...
describe('DefaultSecurityPolicy', () => {
	...
	it('falls back to default signing when custom policy is non-boolean', async () => {
		...
		const outboundSpy = jest.spyOn(policy as any, 'shouldSignOutboundRequest').mockReturnValue(true);
	...
	it('signs outbound sensitive operations when enabled', async () => {
		...
		const sensitivitySpy = jest
			.spyOn(policy as unknown as { isSensitiveOperation: (envelope: FameEnvelope) => boolean }, 'isSensitiveOperation')
	...
	it('returns channel encryption options when channel should be used', async () => {
		...
		const channelSpy = jest
			.spyOn(policy as unknown as { shouldUseChannelEncryption: (...args: any[]) => Promise<boolean> }, 'shouldUseChannelEncryption')
			.mockResolvedValue(true);
	...
	it('evaluates channel encryption helper across scenarios', async () => {
		...
		expect(
			await (desiredLevelPolicy as unknown as {
				shouldUseChannelEncryption: (
					envelope: FameEnvelope,
					context?: FameDeliveryContext,
					nodeLike?: NodeLike
	...
	it('falls back to upstream key request when no key is found', async () => {
		...
		jest
			...
			.mockResolvedValue(false);
		jest
			.spyOn(policy as unknown as { lookupRecipientEncryptionKey: (...args: any[]) => Promise<[string, Uint8Array]> }, 'lookupRecipientEncryptionKey')
	...
	it('looks up recipient keys across derived paths and participants', async () => {
		...
		const [kid] = await (policy as unknown as {
			lookupRecipientEncryptionKey: (
				address: string,
				nodePath?: string
			) => Promise<[string, Uint8Array]>;
	...
	it('fetches participant-level keys when direct and path lookups are empty', async () => {
		...
		const result = await (policy as unknown as {
			lookupRecipientEncryptionKey: (
				address: string,
				nodePath?: string
			) => Promise<[string, Uint8Array]>;
	...
	it('supports FameAddress-like objects during recipient key resolution', async () => {
		...
		const [kid, keyBytes] = await (policy as unknown as {
			lookupRecipientEncryptionKey: (
				address: string,
				nodePath?: string
			) => Promise<[string, Uint8Array]>;
	...
	it('throws when recipient address lacks participant component', async () => {
		...
		it('falls back to default key provider when none is configured', async () => {
			...
			const getProviderSpy = jest.spyOn(keyModule, 'getKeyProvider').mockReturnValue(providerMock as any);
			const policy = makePolicy();
	...
	it('skips invalid local crypto provider keys', async () => {
		...
		setCryptoProvider({
			getJwks: () => ({
				keys: [
					{ use: 'enc', kty: 'OKP', crv: 'X25519', kid: 'bad-local' },
				],
			}),
		});
	...
	it('treats address conversion failures as non-local', () => {
		...
		const badAddress = { toString: () => { throw new Error('boom'); } } as any;
	...
	it('decides response crypto level using mirror and escalation rules', async () => {
		...
		expect(
			await basePolicy.decideResponseCryptoLevel(CryptoLevel.PLAINTEXT, dataResponse)
		...
		expect(
			await escalatePolicy.decideResponseCryptoLevel(CryptoLevel.CHANNEL, dataResponse)
	...
	it('prefers sealed crypto for sensitive operations when enabled', async () => {
		...
		expect(await policy.decideOutboundCryptoLevel(makeEnvelope({ to: 'peer@/path' }), makeContext())).toBe(
			CryptoLevel.SEALED
		);
	...
	it('determines signature requirement based on frame type and inbound policy', () => {
		...
		expect(policy.isSignatureRequired(makeEnvelope({ frame: { type: 'KeyRequest' } as any }))).toBe(true);
		expect(policy.isSignatureRequired(makeEnvelope({ frame: { type: 'NodeAttach' } as any }))).toBe(false);

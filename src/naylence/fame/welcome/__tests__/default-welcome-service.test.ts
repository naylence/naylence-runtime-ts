...
function createMockContext(options?: {
  ...
  const transportProvisioner: TransportProvisioner & {
    ...
    provision: jest.fn(
      async () =>
        ({
          connectionGrant: { type: "WebSocketConnectionGrant", url: "wss://example" },
          cleanupHandle: null,
          metadata: null,
...
describe("DefaultWelcomeService", () => {
  ...
  it("throws when logicals are invalid", async () => {
    const context = createMockContext();
    const hello = createHelloFrame({ logicals: ["invalid logical with spaces"] });

    await expect(context.service.handleHello(hello)).rejects.toThrow("Invalid logical format");
  ...
  it("throws when placement rejects node", async () => {
    ...
    await expect(context.service.handleHello(createHelloFrame())).rejects.toThrow("No capacity");

...
> = async (fastify: FastifyInstance, options: NodeWelcomeRouterOptions) => {
  ...
  fastify.post(`${prefix}/hello`, async (request: HelloRequest, reply) => {
    ...
    if (welcomeService.authorizer) {
      const authResult =
        await welcomeService.authorizer.authenticate(authHeader);
      if (!authResult) {
        logger.warning('client_authentication_failed', {
          authorizerType:
            welcomeService.authorizer.constructor?.name ?? 'UnknownAuthorizer',
        });
    ...
    try {
      ...
      if (parsed.frame?.type !== 'NodeHello') {
        throw new HelloValidationError(
          'Envelope must contain a NodeHello frame'
        );
      }
    ...
    try {
      const welcomeFrame: NodeWelcomeFrame =
        await welcomeService.handleHello(helloFrame);

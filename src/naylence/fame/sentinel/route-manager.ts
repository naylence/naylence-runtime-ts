...
export class RouteManager extends TaskSpawner {
  ...
  public readonly _downstream_addresses_routes = new Map<string, AddressRouteInfo>();
  public readonly _downstream_addresses_legacy = new Map<string, AddressRouteInfo>();
  ...
  private readonly pendingCleanupControllers = new Map<string, AbortController>();
  ...
  public async start(): Promise<void> {
    ...
    this.spawn(() => this.janitorLoop(), { name: `route-janitor-${this.getId()}` });
  }
  ...
  public async registerDownstreamRoute(segment: string, route: FameConnector): Promise<void> {
    await this._routesLock.runExclusive(async () => {
  ...
  public async registerPeerRoute(segment: string, route: FameConnector): Promise<void> {
    await this._routesLock.runExclusive(async () => {
      this.cancelPendingCleanup(segment);
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (this.isTransientError(error)) {
              ...
              await delay(backoff, this.stopController.signal).catch(() => undefined);
              backoff *= 2;
              continue;
            }
  ...
  private async expireRouteLater(segment: string, delayMs: number): Promise<void> {
    if (delayMs > 0) {
      try {
  ...
  private async removeDownstreamRoute(
    ...
    await this.removeRoute(segment, this.downstreamRoutes, this._downstream_route_store, options);
  }
  ...
  private async removePeerRoute(
    ...
    await this.removeRoute(segment, this._peer_routes, this._peer_route_store, options);
  }
  ...
  private buildAuthorizedContext(options: {
    ...
    const security = options.authorization ? { authorization: options.authorization } : undefined;

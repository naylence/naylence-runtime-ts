...
export class CompositeLoadBalancingStrategy implements LoadBalancingStrategy {
  ...
  constructor(strategies: readonly LoadBalancingStrategy[]) {
    if (!strategies.length) {
      throw new Error('CompositeLoadBalancingStrategy requires at least one strategy');
    }

    this.strategies = [...strategies];
  }

  public choose(poolKey: unknown, segments: readonly string[], envelope: FameEnvelope): string | null {
    if (!segments.length) {
      return null;
    }

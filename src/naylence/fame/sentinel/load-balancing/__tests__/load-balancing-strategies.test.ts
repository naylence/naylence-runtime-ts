...
const createEnvelope = (overrides: Partial<FameEnvelope> = {}): FameEnvelope => {
  return {
    id: 'env-1',
...
describe('RandomLoadBalancingStrategy', () => {
  ...
  it('selects a segment using injected RNG', () => {
    ...
    expect(strategy.choose('pool', ['a', 'b', 'c'], createEnvelope())).toBe('c');
  });

  it('clamps RNG results to valid range', () => {
    const strategy = new RandomLoadBalancingStrategy({ rng: () => 1.5 });
    expect(strategy.choose('pool', ['first', 'second'], createEnvelope())).toBe('second');
  });
});
...
describe('CompositeLoadBalancingStrategy', () => {
  ...
  const createMockStrategy = (
    impl: (poolKey: unknown, candidates: readonly string[], env: FameEnvelope) => string | null
  ): LoadBalancingStrategy & { choose: jest.Mock } => ({
    choose: jest.fn((poolKey: unknown, candidates: readonly string[], env: FameEnvelope) =>
  ...
  it('tries strategies sequentially until one succeeds', () => {
    ...
    const composite = new CompositeLoadBalancingStrategy([strategyA, strategyB]);
...
describe('Load balancing strategy factories', () => {
  ...
  it('RandomLoadBalancingStrategyFactory creates a random strategy', async () => {
    ...
    const strategy = await factory.create({ type: 'RandomLoadBalancingStrategy' });
  ...
  it('RoundRobinLoadBalancingStrategyFactory creates a round-robin strategy', async () => {
    ...
    const strategy = await factory.create({ type: 'RoundRobinLoadBalancingStrategy' });
  ...
  it('StickyLoadBalancingStrategyFactory requires stickiness manager', async () => {
    ...
    await expect(factory.create({ type: 'StickyLoadBalancingStrategy' })).rejects.toThrow(
      /stickinessManager/
    );

    const strategy = await factory.create({ type: 'StickyLoadBalancingStrategy' }, { stickinessManager });
  ...
  it('LoadBalancingProfileFactory creates strategies for built-in profiles', async () => {
    ...
    const randomStrategy = await factory.create({ type: 'LoadBalancingProfile', profile: PROFILE_NAME_RANDOM });
    expect(randomStrategy).toBeInstanceOf(RandomLoadBalancingStrategy);

    const rrStrategy = await factory.create({ type: 'LoadBalancingProfile', profile: PROFILE_NAME_ROUND_ROBIN });
    expect(rrStrategy).toBeInstanceOf(RoundRobinLoadBalancingStrategy);

    const hrwStrategy = await factory.create({ type: 'LoadBalancingProfile', profile: PROFILE_NAME_HRW });
    ...
    const devStrategy = await factory.create({ type: 'LoadBalancingProfile', profile: PROFILE_NAME_DEVELOPMENT });
  ...
  it('LoadBalancingStrategyFactory.createLoadBalancingStrategy defaults to HRW strategy', async () => {
    const strategy = await LoadBalancingStrategyFactory.createLoadBalancingStrategy();

...
import { LOAD_BALANCING_STRATEGY_FACTORY_BASE, LoadBalancingStrategyFactory, type LoadBalancingStrategyConfig } from './load-balancing-strategy-factory.js';
...
export class HRWLoadBalancingStrategyFactory extends LoadBalancingStrategyFactory {
  ...
  public async create(
    ...
    const strategy = new HRWLoadBalancingStrategy({ stickyAttribute: finalConfig.stickyAttribute ?? null });
    return strategy;
  }
  ...
  private normalizeConfig(
    ...
    if (typeof config.stickyAttribute === 'string' || config.stickyAttribute === null || config.stickyAttribute === undefined) {
      return {
        type: 'HRWLoadBalancingStrategy',

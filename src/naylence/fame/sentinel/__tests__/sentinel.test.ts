...
describe('Sentinel', () => {
    ...
    await Promise.all(
      sentinels.map(async (sentinel) => {
        const sentinelAny = sentinel as unknown as {
          lifecycleTasks?: { shutdownTasks?: (options?: unknown) => Promise<void> };
  ...
  function createSentinel(options: SentinelOptions = {}): Sentinel {
    const securityManager = options.securityManager ?? createMockSecurityManager();
  ...
  it('enforces attach timeout for pending origin connectors', async () => {
    ...
    try {
      ...
      const lifecycleTasks = (sentinel as any).lifecycleTasks as
        ...
        | undefined;
      await lifecycleTasks?.shutdownTasks({ cancelHanging: true }).catch(() => undefined);
    }
  });

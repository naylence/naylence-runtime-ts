...
const delayMock = (jest.requireMock("../../util/task-utils.js") as { delay: jest.Mock })
  .delay as jest.Mock;
...
describe("RouteManager", () => {
  ...
  it("cancels scheduled cleanup when route is re-registered", async () => {
    ...
    await (
      manager as unknown as { removeDownstreamRoute: (segment: string) => Promise<void> }
  ...
  it("restores routes by retrying transient failures and propagating authorization", async () => {
    ...
    expect(loggerMock.warning).toHaveBeenCalledWith("route_restore_missing_config", {
  ...
  it("logs janitor loop errors and exits gracefully", async () => {
    ...
    const scanSpy = jest
      .spyOn(
        manager as unknown as { scanStoreForExpirations: (...args: unknown[]) => Promise<void> },
  ...
  it("handles cleanup when task signal states vary", async () => {
    ...
    expect(loggerMock.debug).toHaveBeenCalledWith("connector_cleanup_cancelled", {
      segment: "segment",
    });
    expect(
      (
  ...
  it("ignores non-error stop failures during safeStop", async () => {
    ...
    await (manager as unknown as { safeStop: (conn: FameConnector) => Promise<void> }).safeStop(
      connector
    );

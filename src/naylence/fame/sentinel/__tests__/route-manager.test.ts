import { DeliveryOriginType, FameResponseType, type FameConnector, type FameEnvelope } from 'naylence-core';
...
const delayMock = (jest.requireMock('../../util/task-utils.js') as { delay: jest.Mock }).delay as jest.Mock;

function createRouteStore(initial: Record<string, RouteEntry> = {}): RouteStore {
  let data = { ...initial };
  return {
...
describe('RouteManager', () => {
  ...
  it('aborts pending cleanup when re-registering a downstream route', async () => {
    ...
    (manager as unknown as { pendingCleanupControllers: Map<string, AbortController> }).pendingCleanupControllers.set(
      'segment',
      controller
    );
    ...
    expect(
      (manager as unknown as { pendingCleanupControllers: Map<string, AbortController> }).pendingCleanupControllers.has(
        'segment'
      )
    ).toBe(false);
  ...
  const downstreamRoutes = manager._downstream_addresses_routes as Map<string, AddressRouteInfo>;
  downstreamRoutes.set('svc@/child', { segment: 'child' } as AddressRouteInfo);
  manager._downstream_addresses_legacy.set('svc@/child', { segment: 'child' } as AddressRouteInfo);
    manager._peer_addresses_routes.set('svc@/child', 'child');
    manager._pools.set('pool', new Set(['child']));

    await (manager as unknown as {
      removeDownstreamRoute: (segment: string, options?: { stop?: boolean }) => Promise<void>;
  ...
  it('cancels scheduled cleanup when route is re-registered', async () => {
    jest.useFakeTimers();
    const store = createRouteStore();
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: store, cleanupDelayMs: 100 });
    const first = createConnectorStub();
    const second = createConnectorStub();

    await manager.registerDownstreamRoute('child', first);
    await (manager as unknown as { removeDownstreamRoute: (segment: string) => Promise<void> }).removeDownstreamRoute(
      'child'
    );
  ...
  const store = createRouteStore(entries);
    ...
    const manager = new RouteManager({ deliver, routeStore: store, getId: () => 'rm' });

    const { ConnectorFactory } = createConnectorModule();

    type DeliveryHandler = (env: FameEnvelope, ctx: unknown) => Promise<unknown>;
    ...
    expect(loggerMock.warning).toHaveBeenCalledWith('route_restore_missing_config', { segment: 'missing-config' });
    expect(loggerMock.debug).toHaveBeenCalledWith('skipping_expired_route', { segment: 'expired-route' });
    ...
    expect(deliver).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        fromSystemId: 'recoverable-route',
        security: expect.objectContaining({ authorization: expect.objectContaining({ sub: 'node-123' }) }),
      })
    );
  ...
  it('auto-expires routes discovered in the route store', async () => {
    ...
    await (manager as unknown as {
      scanStoreForExpirations: (store: RouteStore, now: Date, kind: 'peer') => Promise<void>;
  ...
  it('parses authorization metadata and handles corrupt entries', () => {
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: createRouteStore() });
    ...
    const parsed = (manager as unknown as {
      parseAuthorization: (metadata: Record<string, unknown> | null) => unknown;
    }).parseAuthorization(metadata) as Record<string, unknown>;

    expect(parsed?.sub).toBe('subject');
    expect(parsed?.scopes).toEqual(['scope']);

    const empty = (manager as unknown as {
      parseAuthorization: (metadata: Record<string, unknown> | null) => unknown;
    }).parseAuthorization(null);

    expect(empty).toBeNull();

    const invalid = (manager as unknown as {
      parseAuthorization: (metadata: Record<string, unknown> | null) => unknown;
    }).parseAuthorization({ authenticated: 'yes' } as unknown as Record<string, unknown>);

    expect(invalid).toBeNull();
    expect(loggerMock.error).toHaveBeenCalledWith('corrupt_route_metadata', expect.any(Object));
  ...
  it('creates connectors via fallback resource when factory is unavailable', async () => {
    ...
    const created = await (manager as unknown as {
      createConnector: (config: Record<string, unknown>) => Promise<FameConnector>;
  ...
  it('defaults cleanup delay when configured with non-finite values', async () => {
    ...
    const actualDelay = (manager as unknown as { cleanupDelayMs: number }).cleanupDelayMs;
    ...
    const sanitizedDelay = (zeroManager as unknown as { cleanupDelayMs: number }).cleanupDelayMs;
  ...
  it('classifies transient errors based on type and message', () => {
    ...
    const asTransient = (manager as unknown as {
  ...
  it('schedules delayed cleanup when removing peer route', async () => {
    jest.useFakeTimers();
    const store = createRouteStore();
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: store, cleanupDelayMs: 25 });
    const connector = createConnectorStub();
    ...
    try {
      ...
      expect(connector.stop).toHaveBeenCalledTimes(1);
      expect(
        (manager as unknown as { pendingCleanupControllers: Map<string, AbortController> })
  ...
  it('logs cleanup delay failures when delay rejects', async () => {
    ...
    await (manager as unknown as {
      cleanupConnector: (segment: string, connector: FameConnector, delayMs: number) => Promise<void>;
  ...
  it('expires routes later and warns when deletion fails', async () => {
    ...
    try {
      ...
      const expirePromise = (manager as unknown as {
      ...
      expect(loggerMock.debug).toHaveBeenCalledWith('expired_route', { route: 'segment' });
    } finally {
      jest.useRealTimers();
      await manager.shutdownTasks({ cancelHanging: true });
    }
  ...
  it('logs janitor loop errors and exits gracefully', async () => {
    ...
    const scanSpy = jest
      .spyOn(manager as unknown as { scanStoreForExpirations: (...args: unknown[]) => Promise<void> }, 'scanStoreForExpirations')
  ...
  it('warns when downstream route deletion fails', async () => {
    const store = createRouteStore();
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: store, cleanupDelayMs: 0 });
    const connector = createConnectorStub();
    await manager.registerDownstreamRoute('segment', connector);
    (store.delete as jest.Mock).mockRejectedValueOnce(new Error('delete failed'));
  ...
  it('logs warning when auto-expire deletion fails', async () => {
    ...
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: store });
    manager._peer_routes.set('peer-1', connector);
    (store.delete as jest.Mock).mockRejectedValueOnce(new Error('cannot delete'));
  ...
  it('logs pending route stop failure when safe stop rejects', async () => {
    ...
    const safeStopSpy = jest
      .spyOn(manager as unknown as { safeStop: (conn: FameConnector) => Promise<void> }, 'safeStop')
  ...
  it('logs fatal restore failures for non-transient errors', async () => {
    ...
    const store = createRouteStore(entries);
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: store, getId: () => 'rm' });
    const { ConnectorFactory } = createConnectorModule();
  ...
  it('skips non-expired routes while scanning store', async () => {
    ...
    await (manager as unknown as {
      scanStoreForExpirations: (store: RouteStore, now: Date, kind: 'downstream') => Promise<void>;
  ...
  it('processes janitor loop iterations until aborted', async () => {
    ...
    try {
      const loopPromise = (manager as unknown as { janitorLoop: () => Promise<void> }).janitorLoop();
      await flushAllTimers();

      expect(scanSpy).toHaveBeenCalledWith(manager._downstream_route_store, expect.any(Date), 'downstream');
      expect(scanSpy).toHaveBeenCalledWith(manager._peer_route_store, expect.any(Date), 'peer');

      (manager as unknown as { stopController: AbortController }).stopController.abort();
  ...
  it('handles cleanup when task signal states vary', async () => {
    const manager = new RouteManager({ deliver: jest.fn(), routeStore: createRouteStore(), cleanupDelayMs: 10 });
    const connector = createConnectorStub();
  ...
  let captured: ((signal?: AbortSignal) => Promise<void>) | undefined;
    const spawnSpy = jest
      ...
      .mockImplementation((task) => {
        ...
        return { id: 'test-task' } as unknown as ReturnType<RouteManager['spawn']>;
      });
    ...
    expect(loggerMock.debug).toHaveBeenCalledWith('connector_cleanup_cancelled', { segment: 'segment' });
    expect(
      (manager as unknown as { pendingCleanupControllers: Map<string, AbortController> })
  ...
  it('restores routes without expiration and avoids zero-delay scheduling', async () => {
    ...
    const spawnSpy = jest.spyOn(manager as unknown as { spawn: RouteManager['spawn'] }, 'spawn');
    ...
    try {
      ...
      const noExpireConnector = manager.downstreamRoutes.get('no-expire') as FameConnector;
      ...
      expect(deliver).toHaveBeenCalledWith(
        envelope,
        expect.objectContaining({ security: undefined, fromSystemId: 'no-expire' })
      );
  ...
  it('logs restore failures with non-error values', async () => {
    ...
    const transientSpy = jest
      .spyOn(manager as unknown as { isTransientError: (error: unknown) => boolean }, 'isTransientError')
  ...
  it('purges references when expiring routes without connectors', async () => {
    ...
    const downstream = manager._downstream_addresses_routes as Map<string, AddressRouteInfo>;
    downstream.set('match', { segment: 'target' } as AddressRouteInfo);
    downstream.set('other', { segment: 'other' } as AddressRouteInfo);
    const legacy = manager._downstream_addresses_legacy as Map<string, AddressRouteInfo>;
  ...
  it('handles cleanup delay rejections with non-error reasons', async () => {
    ...
    const spawnSpy = jest
      ...
      .mockImplementation((task, options) => {
        captured = task as (signal?: AbortSignal) => Promise<void>;
        return { id: options?.name ?? 'task' } as unknown as ReturnType<RouteManager['spawn']>;
      });
  ...
  it('ignores non-error stop failures during safeStop', async () => {
    ...
    await (manager as unknown as { safeStop: (conn: FameConnector) => Promise<void> }).safeStop(connector);
  ...
  it('handles missing connectors during expiration scanning', async () => {
    ...
    await (manager as unknown as {
      scanStoreForExpirations: (store: RouteStore, now: Date, kind: 'downstream' | 'peer') => Promise<void>;
    }).scanStoreForExpirations(store, new Date(), 'downstream');
  ...
  it('parses legacy authorization metadata variants', () => {
    ...
    const parsedNumeric = (manager as unknown as {
      parseAuthorization: (metadata: Record<string, unknown> | null) => unknown;
    }).parseAuthorization(numericExpiryMetadata) as Record<string, unknown> | null;
    ...
    const parsedDate = (manager as unknown as {
      parseAuthorization: (metadata: Record<string, unknown> | null) => unknown;

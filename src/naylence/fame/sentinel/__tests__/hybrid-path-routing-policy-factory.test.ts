...
const { createResource: createResourceMock } = jest.requireMock("naylence-factory") as {
  createResource: jest.MockedFunction<(typeof import("naylence-factory"))["createResource"]>;
};
...
describe("HybridPathRoutingPolicyFactory", () => {
  ...
  const createStrategySpy = jest.spyOn(LoadBalancingStrategyFactory, "createLoadBalancingStrategy");
  ...
  it("creates a strategy via resource when config supplies one", async () => {
    ...
    expect(createResourceMock).toHaveBeenCalledWith(LOAD_BALANCING_STRATEGY_FACTORY_BASE, {
      type: "custom",
    });
    expect(createStrategySpy).not.toHaveBeenCalled();
    expect(HybridPathRoutingPolicyMock).toHaveBeenCalledWith({
  ...
  it("rejects when config type is not HybridPathRoutingPolicy", async () => {
    await expect(
      factory.create({ type: "OtherPolicy", loadBalancingStrategy: null } as any)
    ).rejects.toThrow(
  ...
  it("rejects when loadBalancingStrategy is not an object", async () => {
    ...
    ).rejects.toThrow("loadBalancingStrategy must be an object or null when provided");

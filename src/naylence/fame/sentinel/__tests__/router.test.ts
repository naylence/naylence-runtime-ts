import { type FameEnvelope, type FameAddress, type EnvelopeFactory } from 'naylence-core';
...
function createState(options?: Partial<ConstructorParameters<typeof RouterState>[0]>): RouterState {
...
describe('RoutingAction implementations', () => {
  ...
  it('Drop emits nack and logs drop', async () => {
    ...
    expect(loggerMock.debug).toHaveBeenCalledWith('dropped_envelope', expect.any(Object));
  });
  ...
  it('DeliverLocal routes to provided recipient', async () => {
    ...
    expect(router.deliverLocal).toHaveBeenCalledWith('svc@/node-1/local', envelope, undefined);
  });
  ...
  it('ForwardChild forwards successfully', async () => {
    ...
    expect(router.forwardToRoute).toHaveBeenCalledWith('child-segment', envelope, undefined);
  });
  ...
  it('ForwardChild rethrows non-transport errors', async () => {
    ...
    await expect(action.execute(envelope, router as unknown as any, state, undefined)).rejects.toThrow('boom');
  ...
  it('ForwardPeer handles transport close errors without nack for ack frames', async () => {
    ...
    const state = createState();
    const envelope = { frame: { type: 'DeliveryAck' } } as unknown as FameEnvelope;
  ...
  it('ForwardPeer rethrows non-transport errors', async () => {
    ...
    await expect(action.execute(envelope, router as unknown as any, state, undefined)).rejects.toThrow('fail');
...
describe('emitDeliveryNack', () => {
  ...
  it('returns early when nack should not be emitted', async () => {
    ...
    await emitDeliveryNack(envelope, router as unknown as any, state, 'NO_ROUTE');
  ...
  it('logs warning when envelope factory missing', async () => {
    ...
    expect(loggerMock.warning).toHaveBeenCalledWith('router_missing_envelope_factory', expect.any(Object));
  });
  ...
  it('forwards nack to child segment when route exists', async () => {
    ...
    expect(router.forwardToRoute).toHaveBeenCalledWith('child-a', expect.any(Object), expect.any(Object));
  });
  ...
  it('forwards nack to peer segment when available', async () => {
    ...
    expect(router.forwardToPeer).toHaveBeenCalledWith('peer-seg', expect.any(Object), expect.any(Object));
  });
  ...
  it('forwards nack upstream when no child or peer match', async () => {
    ...
    expect(router.forwardUpstream).toHaveBeenCalledWith(expect.any(Object), expect.any(Object));
  });
  ...
  it('logs warning when forwarding the nack fails', async () => {
    ...
    await emitDeliveryNack(envelope, failingRouter as unknown as any, state, 'NO_ROUTE');

    expect(loggerMock.warning).toHaveBeenCalledWith('nack_forward_failed', expect.objectContaining({ error: 'send failed' }));
  });
  ...
  it('creates secure accept nack frames when secure open envelopes fail', async () => {
    ...
    await emitDeliveryNack(secureEnvelope, router as unknown as any, state, 'HANDSHAKE_FAIL');

    expect(state.envelopeFactory?.createEnvelope).toHaveBeenCalledWith(expect.objectContaining({
...
describe('router utility functions', () => {
  it('normalizes route sources from maps and records', () => {
    ...
    expect(mapState.downstreamAddressRoutes.get('addr-object')).toBe('segment-map');
    ...
    expect(recordState.downstreamAddressRoutes.get('svc@/node-r/local')).toBe('segment-record');

...
function createRoutingNode(overrides: Partial<RoutingNodeLike & { forwardToRoute?: any }> = {}) {
  ...
  return {
    ...
    ...overrides,
  } as unknown as RoutingNodeLike & { envelopeFactory: { createEnvelope: jest.Mock } };
}

function createBindingManager(overrides: Partial<BindingManager> = {}) {
  const getBinding = jest.fn<Binding | undefined, [FameAddress | string]>(() => undefined);
  return {
    getBinding,
    ...overrides,
...
describe('KeyFrameHandler', () => {
  ...
  it('returns false when address lacks separator', async () => {
    const handler = new KeyFrameHandler({
      ...
      bindingManager: createBindingManager(),
      acceptKeyAnnounceParent: jest.fn(),
      keyManager: createKeyManager({ getKeysForPath: jest.fn(async () => [] as KeyRecord[]) }),
      correlationMap: createCorrelationMap(),
    });

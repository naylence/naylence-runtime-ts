...
import { RouterState, Drop, DeliverLocal, ForwardChild, ForwardUp } from '../router.js';
...
describe('CapabilityAwareRoutingPolicy', () => {
  ...
  expect(deliverLocal).toHaveBeenCalledWith(localAddress, envelope, undefined);
  });
  ...
  it('forwards to child segment selected by load-balancing strategy when capabilities match', async () => {
    ...
    const policy = new CapabilityAwareRoutingPolicy({ loadBalancingStrategy: stubStrategy });
    ...
    expect(stubStrategy.choose).toHaveBeenCalledWith(['analytics', 'ingest'], ['child-42'], envelope);

    const forwardToRoute = jest.fn();
    await action.execute(envelope, { forwardToRoute } as any, state, null);
  expect(forwardToRoute).toHaveBeenCalledWith(chosenSegment, envelope, undefined);
  });
  ...
  it('falls back to provider segments when resolver returns non-local address', async () => {
    ...
    const action = await policy.decide(envelope, state, null);
    expect(action).toBeInstanceOf(ForwardChild);
    expect(stubStrategy.choose).toHaveBeenCalledWith(['analytics'], ['child-11'], envelope);
  });
  ...
  it('forwards upstream when load balancer declines to choose but parent exists', async () => {
    ...
    const action = await policy.decide(envelope, state, null);
    expect(action).toBeInstanceOf(ForwardUp);
    expect(stubStrategy.choose).toHaveBeenCalledWith(['metrics'], ['child-5'], envelope);
  });

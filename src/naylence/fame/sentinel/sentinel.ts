...
export class Sentinel extends FameNode implements RoutingNodeLike {
	...
	override async deliver(envelope: FameEnvelope, context?: FameDeliveryContext): Promise<void> {
		const processedEnvelope = await this.dispatchEnvelopeEvent('onDeliver', this, envelope, context);
		...
		if (
			frameType &&
			['AddressBindAck', 'AddressUnbindAck', 'CapabilityAdvertiseAck', 'CapabilityWithdrawAck'].includes(
				frameType
			)
		) {
		...
		if (frameType === 'DeliveryAck') {
			await this.deliveryTracker.onEnvelopeDelivered(SYSTEM_INBOX, processedEnvelope, context);
			await this.resolvePendingBind(processedEnvelope, (processedEnvelope.frame as NodeAttachAckFrame)?.ok ?? true);
			return;
		}
		...
		const action: RoutingAction = await this.routingPolicy.decide(processedEnvelope, state, context);
	...
	async forwardToRoute(
		...
		try {
			processed = await this.dispatchEnvelopeEvent('onForwardToRoute', this, nextSegment, envelope, context);
			if (!processed) {
				return;
			}
			...
			await this.emitDeliveryNack(processed, { code: 'CHILD_UNREACHABLE', context: context ?? null });
				return;
			}
	...
	async forwardToPeer(
		...
		const processed = await this.dispatchEnvelopeEvent('onForwardToPeer', this, peerSegment, envelope, context);
		if (!processed) {
			return;
		}
		...
		if (!connector) {
			...
			await this.emitDeliveryNack(processed, { code: 'PEER_UNREACHABLE', context: context ?? null });
			return;
		}
	...
	override async forwardUpstream(envelope: FameEnvelope, context?: FameDeliveryContext): Promise<void> {
		...
		try {
			...
			const sessionManager = (this as unknown as { _sessionManager: SessionManager | null })._sessionManager;
	...
	async createOriginConnector(options: OriginConnectorOptions): Promise<FameConnector> {
		...
		if (timeoutMs !== null && timeoutMs > 0) {
			...
			this.lifecycleTasks.spawn(async (taskSignal) => {
	...
	async removeDownstreamRoute(segment: string, { stop = true }: { stop?: boolean } = {}): Promise<void> {
		if (stop) {
	...
	private originMatches(
		...
		return Boolean(context && context.originType === originType && context.fromSystemId === segment);
	}

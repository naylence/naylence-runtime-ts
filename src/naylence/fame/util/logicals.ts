...
export function validateLogicalSegment(segment: string): [boolean, string | null] {
  ...
  if (!/^[a-zA-Z0-9-]+$/.test(segment)) {
    return [false, `Path segment '${segment}' must contain only alphanumeric characters and hyphens`];
  }
...
export function validateLogical(logical: string): [boolean, string | null] {
  ...
  if (segments.length === 0) {
    return logical === '/' ? [true, null] : [false, 'Logical must contain at least one non-empty segment'];
  }
...
export function validateHostLogical(hostLogical: string): [boolean, string | null] {
  ...
  if (hostLogical.includes('*')) {
    ...
    if (!DNS_HOSTNAME_PATTERN.test(baseDomain)) {
      return [false, `Host logical '${hostLogical}' base domain '${baseDomain}' is not a valid DNS hostname`];
    }
    ...
    for (const label of labels) {
      if (!DNS_LABEL_PATTERN.test(label)) {
        return [false, `Host logical '${hostLogical}' contains invalid label '${label}' in base domain`];
      }
    }
...
export function validateHostLogicals(hostLogicals: string[] | null | undefined): [boolean, string | null] {

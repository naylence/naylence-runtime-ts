...
describe("logical segment validation", () => {
  it.each([
    ...
    ["a".repeat(64), [false, "Path segment '" + "a".repeat(64) + "' exceeds 63 characters"]],
    [
      "invalid!chars",
      [false, "Path segment 'invalid!chars' must contain only alphanumeric characters and hyphens"],
    ],
    ["-start", [false, "Path segment '-start' cannot start or end with hyphen"]],
    ["end-", [false, "Path segment 'end-' cannot start or end with hyphen"]],
    ["double--dash", [false, "Path segment 'double--dash' cannot contain consecutive hyphens"]],
...
describe("logical validation", () => {
  ...
  it("rejects empty or missing leading slash", () => {
    ...
    expect(validateLogical("foo")).toEqual([false, "Logical 'foo' must start with '/'"]);
  });
...
describe("logical hostname conversions", () => {
  ...
  it("converts logicals to hostnames with error handling", () => {
    ...
    expect(() => logicalToHostname("foo")).toThrow("Logical 'foo' cannot start without '/'");
...
describe("host logical validation", () => {
  ...
  it("validates wildcard host logicals", () => {
    ...
    const baseDomainLabels = ["a".repeat(63), "b".repeat(63), "c".repeat(63), "d".repeat(61)];
  ...
  it("validates non-wildcard host logicals", () => {
    ...
    const nearLimitLabels = ["b".repeat(63), "c".repeat(63), "d".repeat(63), "e".repeat(61)];
  ...
  it("validates collections of host logicals", () => {
    ...
    expect(validateHostLogicals(["example.com", "*.example.com"])).toEqual([true, null]);
...
describe("URI helpers", () => {
  ...
  it("transforms wildcard patterns to DNS constraints", () => {
    ...
    expect(logicalPatternsToDnsConstraints(["*.a", "b"])).toEqual([".a", "b"]);
  });
});

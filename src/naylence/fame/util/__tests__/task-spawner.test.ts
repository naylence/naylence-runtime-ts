...
describe('TaskSpawner', () => {
  ...
  describe('Task Management', () => {
    ...
    test('should disable autoCleanup when configured', async () => {
      ...
      const task = noCleanupSpawner.spawn(async () => {
        await delay(10);
        return 'result';
    ...
    test('should provide task result and error getters', async () => {
      ...
      const successTask = spawner.spawn(async () => {
        await delay(10);
      ...
      const failedTask = spawner.spawn(async () => {
        await delay(10);
  ...
  describe('Error Handling', () => {
    ...
    test('should propagate envelope context to spawned tasks', async () => {
      ...
      const getCurrentEnvelopeSpy = jest.spyOn(require('../envelope-context'), 'getCurrentEnvelope')
        .mockReturnValue(mockEnvelope);
      
      // Mock withEnvelopeContextAsync to verify it gets called with the right context
      const withEnvelopeContextAsyncSpy = jest.spyOn(require('../envelope-context'), 'withEnvelopeContextAsync')

...
describe('TaskSpawner', () => {
  ...
  describe('Task Management', () => {
    ...
    test('should find tasks by ID', () => {
      const task = spawner.spawn(async () => delay(10), {
        name: 'findable-task',
      });
    ...
    test('should cancel all tasks', async () => {
      const task1 = spawner.spawn(async (signal) => delay(1000, signal), {
        name: 'task1',
      });
      const task2 = spawner.spawn(async (signal) => delay(1000, signal), {
        name: 'task2',
      });
  ...
  describe('Graceful Shutdown', () => {
    ...
    test('should capture non-cancellation errors in lastSpawnerError during shutdown', async () => {
      ...
      expect(spawner.lastSpawnerError?.message).toBe(
        'Connection timeout during shutdown'
      );
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should propagate envelope context to spawned tasks', async () => {
      ...
      expect(withEnvelopeContextAsyncSpy).toHaveBeenCalledWith(
        mockEnvelope,
        expect.any(Function)
      );
    ...
    test('should handle WebSocket close during receive errors', async () => {
      ...
      await expect(task.promise).rejects.toThrow(
        'WebSocket closed during receive'
      );
      expect(task.isFailed()).toBe(true);

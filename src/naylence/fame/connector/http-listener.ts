...
export class HttpListener extends TransportListener {
	...
	async createRouter(): Promise<HttpRouter> {
		const plugin: FastifyPluginAsync = async (instance) => {
			...
			instance.post<{ Params: { childId: string }; Body: unknown }>('/downstream/:childId', async (request, reply) => {
	...
	private async _handleUpstreamIngress(request: FastifyRequest<{ Body: unknown }>, reply: FastifyReply): Promise<any> {
	...
	private _getExistingConnector(childId: string): FameConnector | null {
		...
		const routeManager = (node as unknown as { routeManager?: RouteManager | undefined }).routeManager;
		if (!routeManager) {
			return null;
		}
	...
	private _isNodeAttachFrame(frame: FameEnvelope['frame']): frame is NodeAttachFrame {
		return Boolean(frame && typeof frame === 'object' && (frame as { type?: string }).type === 'NodeAttach');
	}
	...
	private _handleIngressError(error: unknown, reply: FastifyReply, direction: 'upstream' | 'downstream', childId?: string) {
		if (error instanceof Error) {

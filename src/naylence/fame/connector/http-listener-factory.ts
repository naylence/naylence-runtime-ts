...
function normalizeConfig(
  ...
  const portValue = typeof record.port === 'number' && Number.isFinite(record.port) ? record.port : 0;
...
export class HttpListenerFactory extends TransportListenerFactory<HttpListenerFactoryConfig> {
  ...
  public async create(
    ...
    const httpServer =
      options?.httpServer ?? (await DefaultHttpServer.getOrCreate({ host: normalized.host, port: normalized.port }));

    let authorizer = options?.authorizer ?? null;
    if (!authorizer && normalized.authorizer) {
      authorizer = (await AuthorizerFactory.createAuthorizer(normalized.authorizer, {
        validate: false,
      })) ?? null;
    }

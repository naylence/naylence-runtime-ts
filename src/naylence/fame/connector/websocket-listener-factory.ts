...
import { TransportListenerFactory, TRANSPORT_LISTENER_FACTORY_BASE_TYPE } from './transport-listener-factory.js';
...
function normalizeConfig(
  ...
  const hostValue = typeof record.host === 'string' && record.host.trim().length > 0 ? record.host : '0.0.0.0';

  let portValue: number | undefined;
  if (typeof record.port === 'number' && Number.isFinite(record.port)) {
    portValue = record.port;
  } else {
    const envPort = typeof process !== 'undefined' ? process.env?.[ENV_WEBSOCKET_LISTENER_PORT] : undefined;
...
export class WebSocketListenerFactory extends TransportListenerFactory<WebSocketListenerFactoryConfig> {
  ...
  public async create(
    ...
    if (!authorizer && normalized.authorizer) {
      authorizer =
        (await AuthorizerFactory.createAuthorizer(normalized.authorizer, { validate: false })) ?? null;
    }

    const httpServer = await DefaultHttpServer.getOrCreate({ host: normalized.host, port: normalized.port });

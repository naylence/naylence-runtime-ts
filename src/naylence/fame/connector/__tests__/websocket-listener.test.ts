...
import { WebSocketListener, getWebsocketConnector, getWebsocketListenerInstance } from '../websocket-listener.js';
...
function createHttpServerStub(overrides: Partial<HttpServer> & Record<string, unknown> = {}): HttpServer {
...
function createSocketStub(): { socket: any; close: jest.Mock; send: jest.Mock } {
  const close = jest.fn();
  const send = jest.fn();
  const socket = {
...
describe('WebSocketListener grants and lifecycle', () => {
  ...
  test('asCallbackGrant returns null when base url unavailable', () => {
    const httpServer = createHttpServerStub({ actualBaseUrl: null, actualHost: null, actualPort: null });
    const listener = new WebSocketListener({ httpServer });
  ...
  test('onNodeInitialized registers router for routing nodes', async () => {
    ...
    const listener = new WebSocketListener({ httpServer });
    const node = { createOriginConnector: jest.fn(), publicUrl: 'https://public.example', id: 'node-1' };

    await listener.onNodeInitialized(node as any);

    expect(includeRouter).toHaveBeenCalledWith(expect.any(Function), { prefix: '/fame/v1/attach' });
  ...
  test('onNodeInitialized requires routing node implementation', async () => {
    ...
    await expect(listener.onNodeInitialized({} as any)).rejects.toThrow('WebSocketListener requires a RoutingNodeLike node instance');
  });

  test('onNodeStopped resets state and releases default server', async () => {
    const releaseSpy = jest.spyOn(DefaultHttpServer, 'release').mockResolvedValue();
    const listener = new WebSocketListener({ httpServer: createHttpServerStub() });
  ...
  test('createRouter handler prefers connection socket when present', async () => {
    const listener = new WebSocketListener({ httpServer: createHttpServerStub() });
    const internals = listener as any;
    const attachSpy = jest.spyOn(internals, '_handleWebSocketAttach').mockResolvedValue(undefined);
    const plugin = await listener.createRouter();
    const getMock = jest.fn();

    await plugin({ get: getMock } as any, {} as any);
    const wsRegistration = getMock.mock.calls.find((call) => call[0] === '/ws/:downstreamOrPeer/:systemId');
    const handler = wsRegistration?.[2];

  expect(handler).toBeDefined();
  expect(typeof handler).toBe('function');
    const connection = { socket: { id: 'socket-1' } };
    const request = { url: '/fame/v1/attach/ws/downstream/system-1', headers: {} } as any;
...
describe('WebSocketListener attachment handling', () => {
  function createListener(overrides: { authorizer?: Authorizer } = {}): WebSocketListener {
    return new WebSocketListener({ httpServer: createHttpServerStub(), ...overrides });
  }

  function createRequestStub(url: string, params?: Record<string, unknown>): any {
    return {
      url,
      headers: {},
      raw: { url },
  ...
  test('rejects attachment when params cannot be resolved', async () => {
    ...
    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/invalid'));
  ...
  test('rejects when node is not initialized', async () => {
    ...
    const listenerInternal = listener as any;
    const { socket } = createSocketStub();
    jest.spyOn(listenerInternal, '_resolveAttachParams').mockReturnValue({ downstreamOrPeer: 'downstream', systemId: 'sys-1' });
    const closeSpy = jest.spyOn(listenerInternal, '_closeSocket');

    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/ws/downstream/sys-1'));

    expect(closeSpy).toHaveBeenCalledWith(socket, 1011, 'Listener not initialized');
  });
  ...
  test('rejects when origin type is invalid', async () => {
    ...
    jest.spyOn(listenerInternal, '_resolveAttachParams').mockReturnValue({ downstreamOrPeer: 'invalid', systemId: 'sys-1' });
    const closeSpy = jest.spyOn(listenerInternal, '_closeSocket');

    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/ws/invalid/sys-1'));
  ...
  test('rejects when system id is missing', async () => {
    ...
    const { socket } = createSocketStub();
    listenerInternal._node = { id: 'node-1' };
    jest.spyOn(listenerInternal, '_resolveAttachParams').mockReturnValue({ downstreamOrPeer: 'downstream', systemId: '' });
    const closeSpy = jest.spyOn(listenerInternal, '_closeSocket');

    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/ws/downstream/'));
  ...
  test('rejects when system id matches node id', async () => {
    ...
    expect(closeSpy).toHaveBeenCalledWith(socket, 1008, 'Self attachment not allowed');
  });
  ...
  test('delegates to connector when authentication succeeds', async () => {
    ...
    const { socket } = createSocketStub();
    listenerInternal._node = { id: 'node-1' };
    jest.spyOn(listenerInternal, '_resolveAttachParams').mockReturnValue({ downstreamOrPeer: 'downstream', systemId: 'peer-1' });
    const authSpy = jest.spyOn(listenerInternal, '_authenticateConnection').mockResolvedValue({} as AuthorizationContext);
    const connectorMock = { waitUntilClosed: jest.fn().mockResolvedValue(undefined) };
    const createConnectorSpy = jest.spyOn(listenerInternal, '_createWebSocketConnector').mockResolvedValue(connectorMock as any);
    ...
    expect(createConnectorSpy).toHaveBeenCalledWith(expect.objectContaining({ systemId: 'peer-1' }));
  ...
  test('handles authentication errors via attachment error handler', async () => {
    ...
    jest.spyOn(listenerInternal, '_authenticateConnection').mockRejectedValue(error);
    const handleErrorSpy = jest.spyOn(listenerInternal, '_handleAttachmentError').mockResolvedValue(undefined);

    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/ws/downstream/peer-1'));
  ...
  test('delegates to connector without authorization when authentication returns undefined', async () => {
    ...
    listenerInternal._node = { id: 'node-1' };
    const { socket } = createSocketStub();
    jest.spyOn(listenerInternal, '_resolveAttachParams').mockReturnValue({ downstreamOrPeer: 'downstream', systemId: 'peer-2' });
    jest.spyOn(listenerInternal, '_authenticateConnection').mockResolvedValue(undefined);
    const createConnectorSpy = jest.spyOn(listenerInternal, '_createWebSocketConnector').mockResolvedValue({ waitUntilClosed: jest.fn().mockResolvedValue(undefined) } as any);

    await listenerInternal._handleWebSocketAttach(socket, createRequestStub('/ws/downstream/peer-2'));
...
describe('WebSocketListener helpers', () => {
  ...
  test('resolves params from url when request params missing', () => {
    ...
    expect(internals._resolveAttachParams(request)).toEqual({ downstreamOrPeer: 'downstream', systemId: 'peer-1' });
  });
  ...
  test('resolves attach params directly from request object', () => {
    ...
    expect(internals._resolveAttachParams(request)).toEqual({ downstreamOrPeer: 'peer', systemId: 'peer-42' });
  });
  ...
  test('resolveAttachParams returns null when ws segments missing', () => {
    const listener = new WebSocketListener({ httpServer: createHttpServerStub() });
    const internals = access(listener);
    const request = { url: '/fame/v1/attach/downstream', headers: {}, raw: { url: '/fame/v1/attach/downstream' } } as any;
  ...
  test('authenticate connection bypasses authorization when not configured', async () => {
    ...
    await expect(internals._authenticateConnection('', 'peer-1')).resolves.toBeUndefined();
  });
  ...
  test('authenticate connection succeeds when authorizer returns context', async () => {
    ...
    const listener = new WebSocketListener({ httpServer: createHttpServerStub(), authorizer });
    const internals = access(listener);

    await expect(internals._authenticateConnection('token', 'peer-1')).resolves.toEqual({ subject: 'abc' });
  ...
  test('authenticate connection throws when authorizer denies', async () => {
    ...
    await expect(internals._authenticateConnection('token', 'peer-1')).rejects.toThrow('Authentication failed');
  });
  ...
  test('authenticate connection wraps unexpected errors', async () => {
    ...
    await expect(internals._authenticateConnection('token', 'peer-1')).rejects.toThrow('Authorization error');
  });

  test('resolve authorizer prefers constructor override', async () => {
    const override: Authorizer = { authenticate: jest.fn(), authorize: jest.fn() };
    const listener = new WebSocketListener({ httpServer: createHttpServerStub(), authorizer: override });
    const internals = access(listener);
    internals._node = { securityManager: { authorizer: { authenticate: jest.fn(), authorize: jest.fn() } } };
  ...
  test('resolve authorizer uses node security manager when available', async () => {
    const nodeAuthorizer: Authorizer = { authenticate: jest.fn(), authorize: jest.fn() };
    const listener = new WebSocketListener({ httpServer: createHttpServerStub() });
    const internals = access(listener);
  ...
  test('createWebSocketConnector wraps node connectors', async () => {
    ...
    const node = {
      createOriginConnector: jest.fn(async () => new WebSocketConnector(socket, { type: 'websocket' })),
    };
    ...
    expect(node.createOriginConnector).toHaveBeenCalledWith(expect.objectContaining({ systemId: 'peer-1' }));
  });
  ...
  test('createWebSocketConnector throws when node not initialized', async () => {
    ...
    await expect(internals._createWebSocketConnector({
      systemId: 'peer-1',
  ...
  test('createWebSocketConnector forwards authorization context', async () => {
    ...
    const node = {
      createOriginConnector: jest.fn(async (options: any) => {
        ...
        expect(options.connectorConfig.authorizationContext).toBe(authorization);
  ...
  test('handleAttachmentError sends ack frame for authentication issues', async () => {
    ...
    const authListener = new WebSocketListener({ httpServer: createHttpServerStub(), authorizer });
    ...
    expect(closeSpy).toHaveBeenCalledWith(socket, 1008, 'Authentication failed');
  });
  ...
  test('handleAttachmentError string errors are stringified before closing', async () => {
    ...
    const internals = access(listener);
    const { socket } = createSocketStub();
    const sendAckSpy = jest.spyOn(internals, '_sendAckAndClose').mockResolvedValue(undefined);
  ...
  test('sendAckAndClose transmits ack payload when socket open', async () => {
    ...
    await internals._sendAckAndClose(socket, { ok: true, reason: 'fine' }, 1000, 'done');
  ...
  test('closeSocket logs but suppresses close errors', () => {
    ...
    expect(() => internals._closeSocket({ readyState: WebSocketState.OPEN, send: jest.fn(), close }, 1000, 'reason')).not.toThrow();
  });
});
...
describe('WebSocketListener exports', () => {
  test('getWebsocketListenerInstance returns the most recent listener', () => {
    ...
    const second = new WebSocketListener({ httpServer: createHttpServerStub({ host: 'remote-host' }) });

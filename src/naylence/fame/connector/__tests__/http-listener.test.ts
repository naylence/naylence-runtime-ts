...
jest.mock('naylence-core', () => {
  const actual = jest.requireActual<typeof import('naylence-core')>('naylence-core');
...
describe('HttpListener', () => {
  ...
  beforeEach(() => {
    ...
    selectCallbackGrant = jest
      .spyOn(grantPolicyModule.defaultGrantSelectionPolicy, 'selectCallbackGrant')
      .mockImplementation(() => ({
        grant: {
  ...
  const createStubServer = (baseUrl: string | null): HttpServer & { includeRouter: jest.Mock } => ({
    host: '127.0.0.1',
    port: 8080,
    isRunning: true,
  ...
  describe('asCallbackGrant', () => {
    ...
    it('returns grant with default auth when reverse auth config is null', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://localhost:3000') });
    ...
    it('uses reverse auth config when present', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://public') });
      (listener as any)._reverseAuthConfig = { type: 'CustomAuth', token: 'abc' };
  ...
  describe('onNodeInitialized', () => {
    it('throws when node is not routing capable', async () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://local') });
      const nonRoutingNode = { publicUrl: 'http://local' } as RoutingNodeLike;

      await expect(listener.onNodeInitialized(nonRoutingNode as any)).rejects.toThrow(
    ...
    it('registers router only once and refreshes reverse auth config', async () => {
      const authorizer: Authorizer = {
        ...
        createReverseAuthorizationConfig: jest.fn().mockResolvedValue({ type: 'Reverse', enabled: true }),
      } as unknown as Authorizer;

      const listener = new HttpListener({ httpServer: createStubServer('http://host'), authorizer });
      ...
      expect((listener as any)._reverseAuthConfig).toEqual({ type: 'Reverse', enabled: true });
      expect(authorizer.createReverseAuthorizationConfig).toHaveBeenCalledTimes(1);
      expect(listener.httpServer.includeRouter).toHaveBeenCalledTimes(1);
      expect(mockLogger.debug).toHaveBeenCalledWith('http_routes_registered', expect.any(Object));
    });
  ...
  describe('onNodeStopped', () => {
    it('releases DefaultHttpServer instances', async () => {
      const fakeDefault = Object.create(DefaultHttpServer.prototype) as HttpServer;
      ...
      const releaseSpy = jest.spyOn(DefaultHttpServer, 'release').mockResolvedValue(undefined);

      const listener = new HttpListener({ httpServer: fakeDefault as HttpServer });
      await listener.onNodeStopped({} as RoutingNodeLike);

      expect(releaseSpy).toHaveBeenCalledWith({ host: '127.0.0.1', port: 8080 });
      releaseSpy.mockRestore();
    });

    it('does not release non-default HTTP servers', async () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://listener') });
      const releaseSpy = jest.spyOn(DefaultHttpServer, 'release').mockResolvedValue(undefined);
  ...
  describe('grant conversion', () => {
    it('converts HTTP grants', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
    ...
    it('converts WebSocket grants', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
      const wsSpy = jest.spyOn(WebSocketGrantModule, 'websocketGrantToConnectorConfig');
      const grant = {
    ...
    it('delegates to custom connector config converter when available', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
      const grant = { type: 'CUSTOM', toConnectorConfig: jest.fn(() => ({ type: 'custom' })) };
      const result = (listener as any)._grantToConnectorConfig(grant);
    ...
    it('throws for unsupported grant types', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
      expect(() => (listener as any)._grantToConnectorConfig({ type: 'UNKNOWN' })).toThrow(
        'Unsupported grant type: UNKNOWN'
      );
    });
  });
  ...
  describe('_parseEnvelope', () => {
    it('handles buffers, strings, and objects', () => {
      ...
      const buffer = Buffer.from(JSON.stringify({ frame: { type: 'NodeAttach' }, test: true }));
      expect((listener as any)._parseEnvelope(buffer)).toEqual({ frame: { type: 'NodeAttach' }, test: true });

      const jsonString = JSON.stringify({ frame: { type: 'Message' } });
      expect((listener as any)._parseEnvelope(jsonString)).toEqual({ frame: { type: 'Message' } });

      expect((listener as any)._parseEnvelope({ frame: { type: 'Other' } })).toEqual({ frame: { type: 'Other' } });
    });

    it('returns null and logs when JSON parsing fails', () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
      expect((listener as any)._parseEnvelope('{')).toBeNull();
      expect(mockLogger.warning).toHaveBeenCalledWith('http_listener_envelope_parse_failed', expect.any(Object));
    });
  ...
  describe('_authenticateRequest', () => {
    it('returns undefined when no authorizer is configured', async () => {
      ...
      await expect((listener as any)._authenticateRequest(undefined)).resolves.toBeUndefined();
      expect(mockLogger.debug).toHaveBeenCalledWith('http_ingress_no_authorization');
    });

    it('throws when authentication fails or throws non-Error', async () => {
      const authorizer: Authorizer = {
        authenticate: jest.fn().mockResolvedValueOnce(null).mockRejectedValueOnce('boom'),
      } as unknown as Authorizer;
      const listener = new HttpListener({ httpServer: createStubServer('http://host'), authorizer });

      await expect((listener as any)._authenticateRequest('token')).rejects.toThrow('Authentication failed');
      await expect((listener as any)._authenticateRequest('token')).rejects.toThrow('boom');
    });
  });
  ...
  describe('_handleIngressError', () => {
    it('distinguishes authentication errors', () => {
      ...
      (listener as any)._handleIngressError(new Error('Authentication failed'), reply, 'upstream');
      expect(reply.code).toHaveBeenCalledWith(401);
      expect(mockLogger.error).toHaveBeenCalledWith('http_upstream_ingress_error', {
        childId: undefined,
    ...
    it('handles unknown error values', () => {
      ...
      (listener as any)._handleIngressError('boom', reply, 'downstream', 'child-1');
      expect(reply.code).toHaveBeenCalledWith(500);
      expect(mockLogger.error).toHaveBeenCalledWith('http_downstream_ingress_error', {
        childId: 'child-1',
        error: 'boom',
      });
    });
  ...
  describe('_refreshReverseAuthConfig', () => {
    it('uses default config when authorizer missing or throws', async () => {
      const listener = new HttpListener({ httpServer: createStubServer('http://host') });
      const nodeWithoutAuthorizer = { securityManager: null } as unknown as RoutingNodeLike;
      ...
      const failingAuthorizer: Authorizer = {
        authenticate: jest.fn(),
        createReverseAuthorizationConfig: jest.fn().mockRejectedValue(new Error('nope')),
      } as unknown as Authorizer;
      const listenerWithAuth = new HttpListener({ httpServer: createStubServer('http://host'), authorizer: failingAuthorizer });
      const nodeWithManager = { securityManager: { authorizer: failingAuthorizer } } as unknown as RoutingNodeLike;

      await (listenerWithAuth as any)._refreshReverseAuthConfig(nodeWithManager);
      expect((listenerWithAuth as any)._reverseAuthConfig).toEqual({ type: 'NoAuth' });
      expect(mockLogger.warning).toHaveBeenCalledWith('reverse_auth_config_failure', expect.any(Object));
    });

    it('stores reverse config when available', async () => {
      const authorizer: Authorizer = {
        authenticate: jest.fn(),
        createReverseAuthorizationConfig: jest.fn().mockResolvedValue({ type: 'Issued' }),
      } as unknown as Authorizer;
      const listener = new HttpListener({ httpServer: createStubServer('http://host'), authorizer });
      const node = { securityManager: { authorizer } } as unknown as RoutingNodeLike;
  ...
  describe('ingress handlers', () => {
    ...
    it('validates upstream envelope parsing and connector availability', async () => {
      ...
      await (listener as any)._handleUpstreamIngress({ body: {}, headers: {} }, missingConnectorReply);
    ...
    it('delivers upstream messages and includes security context', async () => {
      ...
      expect(message.context.security?.authorization).toEqual({ principal: 'node' });
    ...
    it('handles downstream ingress including attach frames and missing connectors', async () => {
      ...
      const node: RoutingNodeLike & { routeManager: any; publicUrl: string | null } = {
        publicUrl: 'http://public',
        upstreamConnector: connector,
        createOriginConnector: jest.fn().mockResolvedValue(connector),
        routeManager: {
        ...
        securityManager: { authorizer },
      } as unknown as RoutingNodeLike & { routeManager: any; publicUrl: string | null };
      (listener as any)._node = node;

      const { reply: notInitialized } = createReply();
      (listener as any)._node = null;
      await (listener as any)._handleDownstreamIngress({ params: { childId: 'child' }, body: {} }, notInitialized);
      expect(notInitialized.code).toHaveBeenCalledWith(503);

      (listener as any)._node = node;
      const { reply: mismatchReply } = createReply();
      const attachEnvelope = { frame: { type: 'NodeAttach', systemId: 'other' } };
      ...
      expect(mockLogger.warning).toHaveBeenCalledWith('child_id_mismatch', expect.any(Object));
      ...
      expect(downstreamPush).toHaveBeenCalledWith(expect.any(FameChannelMessage));
      expect(selectCallbackGrant).toHaveBeenCalledWith(expect.any(grantPolicyModule.GrantSelectionContext));
  const attachMessage = downstreamPush.mock.calls[downstreamPush.mock.calls.length - 1]?.[0] as any;
      expect(attachMessage.context.security?.authorization).toEqual({ principal: 'node' });
      ...
      expect(mockLogger.warning).toHaveBeenCalledWith('no_connector_for_child', { childId: 'no-connector' });
    ...
    const downstreamMessage = downstreamPush.mock.calls[downstreamPush.mock.calls.length - 1]?.[0] as any;
        expect(downstreamMessage.context.security?.authorization).toEqual({ principal: 'node' });

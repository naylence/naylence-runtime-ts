...
describe('routes helper utilities', () => {
  ...
  it('parses basic credentials from authorization header', () => {
    ...
    expect(
      parseBasicCredentials(
        `Basic ${Buffer.from(':missing').toString('base64')}`
      )
    ).toBeUndefined();
  });
  ...
  it('resolves audience with precedence overrides', () => {
    ...
    expect(
      resolveAudience(' requested ', baseClient, config, cryptoProvider)
    ).toBe('requested');
    expect(
      resolveAudience(
        undefined,
        { ...baseClient, audience: 'client' },
        config,
        cryptoProvider
      )
    ).toBe('client');
    ...
    expect(resolveAudience(undefined, baseClient, config, cryptoProvider)).toBe(
      'config-default'
    );
    expect(
      resolveAudience(
    ...
    expect(
      resolveAudience(undefined, baseClient, configWithoutDefault, null)
    ).toBeUndefined();
  });
  ...
  it('sanitizes JWKS output and rejects invalid input', () => {
    ...
    const sanitized = sanitizeJwks({
      getJwks: () => ({ keys: [original] }),
    } as CryptoProvider)!;
  ...
  it('builds base and absolute URLs using proxy headers', () => {
    ...
    expect(buildAbsoluteUrl('https://edge.example/api', '/')).toBe(
  ...
  it('resolves issuers and aggregates scopes', () => {
    expect(
      resolveIssuer({ issuer: 'https://issuer' } as CryptoProvider, 'fallback')
    ).toBe('https://issuer');
    expect(resolveIssuer(null, 'fallback')).toBe('fallback');
    const scopes = aggregateScopes([
      baseClient,
      { ...baseClient, scopes: ['admin', 'read'] },
    ]);
  ...
  it('resolves token issuers and handles errors', () => {
    ...
    expect(
      resolveTokenIssuer({ getTokenIssuer: () => issuer } as CryptoProvider)
    ).toBe(issuer);
    ...
    expect(
      resolveTokenIssuer({ getTokenIssuer: () => undefined } as CryptoProvider)
    ).toBeNull();
  });
});

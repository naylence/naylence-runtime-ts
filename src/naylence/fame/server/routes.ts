  ...
  const registrar: FameServerRouteRegistrar = async (instance, config) => {
    ...
    instance.get(config.routes.openIdConfiguration, async (request, reply) => {
      ...
      const payload = {
        ...
        scopes_supported:
          scopesSupported.length > 0 ? scopesSupported : undefined,
      };
    ...
    instance.post(config.routes.token, async (request, reply) => {
      ...
      const basicCredentials = parseBasicCredentials(
      ...
      const clientSecret =
        body.client_secret ?? basicCredentials?.clientSecret ?? '';
      ...
      if (!validateScopes(requestedScopes, client)) {
        ...
        return sendOAuthError(reply, 400, {
          error: 'invalid_scope',
          error_description:
            'Requested scopes are not permitted for this client',
        });
      }
      ...
      const expiresIn =
        (await inferExpiresIn(accessToken)) ?? DEFAULT_EXPIRES_IN_SECONDS;
...
export async function inferExpiresIn(
  token: string
...
export function sanitizeJwks(
  ...
  const normalizedKeys = keys
    .filter((key): key is Record<string, unknown> =>
...
export function resolveTokenIssuer(
  cryptoProvider: CryptoProvider | null
): TokenIssuer | null {

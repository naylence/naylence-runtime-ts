...
function createEnvelopeFuture(expectedResponseType: FameResponseType): EnvelopeFuture<FameEnvelope> {
...
export class DefaultDeliveryTracker extends TaskSpawner implements NodeEventListener {
  ...
  async onNodeInitialized(node: NodeLike): Promise<void> {
    ...
    this.inboxDlq = await this.storageProvider.getKeyValueStore(TrackedEnvelope, '__delivery_inbox_dlq');
  }
  ...
  async track(
    ...
    const tracked = await this.lock.runExclusive(async () => {
      ...
      if (expectedResponseType & FameResponseType.REPLY || expectedResponseType & FameResponseType.STREAM) {
        if (existingEnvId) {
      ...
      if (options.retryPolicy && options.retryPolicy.maxRetries > 0) {
        try {
          const firstDelayMs = Math.max(0, Math.trunc(options.retryPolicy.nextDelayMs(1)));
          firstCheckpointMs = Math.min(overallTimeoutMs, firstDelayMs);
        } catch (error) {
          logger.warning('tracker_retry_policy_error', {
            envp_id: envelope.id,
  ...
  async updateTrackedEnvelope(envelope: TrackedEnvelope): Promise<void> {
    ...
    if (envelope.mailboxType === MailboxType.OUTBOX) {
      throw new Error(`Updating tracked envelopes of mailbox type ${MailboxType.OUTBOX} is not supported`);
    }
    const inbox = this.ensureInbox();
    await inbox.update(envelope.originalEnvelope.id, envelope);
  }
  ...
  async onNack(envelope: FameEnvelope, context?: FameDeliveryContext): Promise<void> {
    ...
    await this.lock.runExclusive(async () => {
      const future = this.ackFutures.get(tracked.originalEnvelope.id);
      if (future && future.reject) {
        const reason = ackFrame.reason ?? 'unknown';
        future.reject(new Error(`Envelope nacked: ${reason}`));
      }
    });
    await this.markDoneSince(this.ackFutures, tracked.originalEnvelope.id, this.ackDoneSince);
  ...
  async addToInboxDlq(trackedEnvelope: TrackedEnvelope, reason: string | null = null): Promise<void> {
  ...
  async cleanup(): Promise<void> {
    ...
    for (const timer of timers) {
      ...
      try {
        ...
        if (!(error instanceof TaskCancelledError)) {
          logger.debug('tracker_timer_cancelled_with_error', {
            error: error instanceof Error ? error.message : String(error),
          });
        }
      }
    ...
    if (this.futuresSweeper) {
      ...
      try {
        ...
        if (!(error instanceof TaskCancelledError)) {
          logger.debug('tracker_sweeper_cancelled_with_error', {
            error: error instanceof Error ? error.message : String(error),
          });
        }
      }
  ...
  private async scheduleTimer(
    ...
    await this.lock.runExclusive(async () => {
      ...
      const task = this.spawn(async (signal) => {
        try {
          const nowMs = Date.now();
  ...
  private async dequeueWithTimeout(queue: AsyncQueue<unknown>, timeoutMs: number): Promise<unknown> {
    let timeoutId: ReturnType<typeof setTimeout> | undefined;
    const timeoutPromise = new Promise<never>((_, reject) => {
      timeoutId = setTimeout(() => reject(new Error('stream timeout waiting for next item')), timeoutMs);
    });
  ...
  private async sendAck(envelope: FameEnvelope): Promise<void> {
    if (!this.node) {
      logger.error('cannot_send_ack_no_node', { envp_id: envelope.id });
      return;
    }
    if (!envelope.replyTo) {

...
describe('AdmissionProfileFactory', () => {
  ...
  it('creates the direct profile by default', async () => {
    ...
    expect((config as Record<string, unknown>).type).toBe('DirectAdmissionClient');

    const grants =
      (config as { connection_grants?: Array<Record<string, unknown>> }).connection_grants ?? [];
  ...
  it('maps welcome profile to welcome service client', async () => {
    ...
    const [config] = spy.mock.calls[0];
    expect((config as Record<string, unknown>).type).toBe('WelcomeServiceClient');

    const { auth, supported_transports: transports } = config as Record<string, any>;
    expect(transports).toEqual(['websocket']);
    expect(auth?.token_provider?.client_id).toBe(Expressions.env('FAME_ADMISSION_CLIENT_ID'));
  });
  ...
  it('supports noop profile aliases', async () => {
    ...
    const [config] = spy.mock.calls[0];
    expect((config as Record<string, unknown>).type).toBe('NoopAdmissionClient');
    expect((config as Record<string, any>).auto_accept_logicals).toBe(true);
  });

  it('throws for unknown profiles', async () => {
    const factory = new AdmissionProfileFactory();
    await expect(factory.create({ profile: 'unknown-profile' })).rejects.toThrow(
      /Unknown admission profile/
    );
  });
});

...
async function ensureModulesLoaded() {
  ...
  await (jest as any).unstable_mockModule('../../../security/keys/attachment-key-validator', () => {
    class MockKeyValidationError extends Error {
      ...
      constructor(code: string, message: string, options: { kid?: string | null; details?: Record<string, unknown> | null } = {}) {
        super(message);
...
describe('DefaultNodeAttachClient', () => {
  ...
  function createConnector()
    : { connector: jest.Mocked<Mutable<FameConnector>>; handlerRef: { current: FameEnvelopeHandler | null } } {
  ...
  function createAckEnvelope(options: {
  ...
  it('wraps key validation errors from validator', async () => {
    ...
    const client = new DefaultNodeAttachClient({ attachmentKeyValidator: validator as any, timeoutMs: 50 });
  ...
  it('propagates connector.send errors and completes upstream event with error', async () => {
    ...
    const dispatchEnvelopeEvent = jest.fn(async (
      event: string,
      _node: unknown,
  ...
  it('awaitAck times out when no ack arrives', async () => {
    ...
    await expect((client as any).awaitAck(connector)).rejects.toThrow('Timeout waiting for NodeAttachAck');
  });
});

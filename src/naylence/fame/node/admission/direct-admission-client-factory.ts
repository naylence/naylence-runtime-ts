...
function normalizeConfig(
  ...
  const source = config as DirectAdmissionClientConfig &
    Record<string, unknown>;

  const connectionGrantsRaw =
    source.connectionGrants ?? source.connection_grants;
  if (!Array.isArray(connectionGrantsRaw) || connectionGrantsRaw.length === 0) {
    throw new Error(
      'DirectAdmissionClient configuration must include at least one connection grant'
    );
  }

...
function createNodeStub(overrides: Partial<NodeLike> = {}): NodeLike {
  ...
  const dispatchEvent = jest.fn<
    Promise<void>,
    Parameters<NodeLike['dispatchEvent']>
  >().mockResolvedValue();
...
describe('UpstreamSessionManager', () => {
  ...
  let onWelcome: jest.MockedFunction<(frame: NodeWelcomeFrame) => Promise<void>>;
  let onAttach: jest.MockedFunction<(info: AttachInfo, connector: FameConnector) => Promise<void>>;
...
//   });
  ...
  test('connectCycle throws when admission client is unavailable', async () => {
    ...
    await expect((manager as any).connectCycle()).rejects.toThrow('Admission client is required to attach upstream');
  });
  ...
  test('start throws when ready event never sets', async () => {
    ...
    await expect(manager.start()).rejects.toThrow('Upstream session manager failed to attach');
  });
  ...
  test('start is a no-op when already running', async () => {
    ...
    const spawnSpy = jest.spyOn(manager as unknown as { spawn: UpstreamSessionManager['spawn'] }, 'spawn');
    await manager.start();
  ...
  test('start resolves immediately when waitUntilReady is false', async () => {
    ...
    await expect(manager.start({ waitUntilReady: false })).resolves.toBeUndefined();
  ...
  test('stop cancels running task and stops connector', async () => {
    ...
    (manager as any).fsmTask = createTaskStub(Promise.resolve(), { cancel: cancelSpy });
  ...
  test('awaitReady respects timeout', async () => {
    ...
    await expect(promise).rejects.toThrow('Timed out waiting for upstream ready');
  ...
  test('start waits for the ready event when waitUntilReady is true', async () => {
    ...
    const spawnSpy = jest
      .spyOn(manager as unknown as { spawn: UpstreamSessionManager['spawn'] }, 'spawn')
      .mockImplementation((..._args: unknown[]) => task as unknown as SpawnedTask<unknown>);
  ...
  test('send throws when queue is full', async () => {
    ...
    for (let i = 0; i < UpstreamSessionManager.TX_QUEUE_MAX; i += 1) {
      queue.push({ id: `env-${i}`, frame: { type: 'Data', payload: null } } as unknown as FameEnvelope);
    }

    await expect(manager.send({ id: 'overflow', frame: { type: 'Data', payload: null } } as FameEnvelope)).rejects.toThrow(
      'Upstream message queue is full'
    );
  });
  ...
  test('fsmLoop retries transport close errors before stopping', async () => {
    ...
    const backoffSpy = jest.spyOn(manager as any, 'applyBackoff').mockImplementation(async () => {
      (manager as any).stopEvent.set();
      return UpstreamSessionManager.BACKOFF_INITIAL;
    });

    await expect((manager as any).fsmLoop()).resolves.toBeUndefined();
    expect(backoffSpy).toHaveBeenCalledWith(UpstreamSessionManager.BACKOFF_INITIAL);
  });
  ...
  test('fsmLoop rethrows task cancellation errors from connect cycle', async () => {
    ...
    await expect((manager as any).fsmLoop()).rejects.toBeInstanceOf(TaskCancelledError);
  });
  ...
  test('applyBackoff doubles delay and caps at maximum', async () => {
    const manager = createManager();
    const sleepSpy = jest.spyOn(manager as any, 'sleepWithStop').mockResolvedValue(undefined);
    const nextDelay = await (manager as any).applyBackoff(UpstreamSessionManager.BACKOFF_CAP);
  ...
  test('heartbeatLoop throws when heartbeat acknowledgement is missed', async () => {
    ...
    try {
      ...
      const graceMs =
        UpstreamSessionManager.HEARTBEAT_INTERVAL * 1000 * UpstreamSessionManager.HEARTBEAT_GRACE;
      (manager as any).lastHeartbeatAckTime = Date.now() - graceMs - 1;
      jest.advanceTimersByTime(UpstreamSessionManager.HEARTBEAT_INTERVAL * 1000);
      await expect(loopPromise).rejects.toThrow('missed heartbeat acknowledgement');
    } finally {
      jest.runOnlyPendingTimers();
      jest.clearAllTimers();
      jest.useRealTimers();
  ...
  test('heartbeatLoop rethrows when wait is cancelled', async () => {
    ...
    await expect((manager as any).heartbeatLoop(connector, stopEvt)).rejects.toBeInstanceOf(
      TaskCancelledError
    );
  });
  ...
  test('heartbeatLoop stops when stop event triggers during wait', async () => {
    ...
    await expect((manager as any).heartbeatLoop(connector, stopEvt)).resolves.toBeUndefined();
  });
  ...
  const completionCalls = (node.dispatchEnvelopeEvent as jest.Mock).mock.calls;
  expect(completionCalls.some((call) => call[0] === 'onForwardUpstreamComplete')).toBe(true);
  });
  ...
  test('getNodeAttachGrant returns matching grant and null otherwise', () => {
    ...
    expect((manager as any).getNodeAttachGrant([{ purpose: 'other' }])).toBeNull();
  ...
  test('connectCycle throws when node attach grant is missing', async () => {
    const manager = createManager();
    const welcome = createWelcomeFrame();
    welcome.frame.connectionGrants = [{ purpose: 'other' } as Record<string, any>];
    admissionClient.hello.mockResolvedValue(welcome);
  ...
  test('connectCycle flushes buffered frames when queue is populated', async () => {
    ...
    (manager as any).messageQueue.push(
      createFameEnvelope({ frame: { type: 'Data', payload: { value: 1 } } as any })
    );
  ...
  test('waitForFailureOrStop propagates task errors', async () => {
    ...
    await expect((manager as any).waitForFailureOrStop(tasks, stopEvt)).rejects.toBe(error);
  });

  test('waitForFailureOrStop ignores task cancellation errors', async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    const tasks: SpawnedTask<void>[] = [createTaskStub(Promise.reject(new TaskCancelledError('cancelled')) )];

    await expect((manager as any).waitForFailureOrStop(tasks, stopEvt)).resolves.toBeUndefined();
  });

  test('messagePumpLoop sends queued messages and stops gracefully', async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    const envelope = createFameEnvelope({ frame: { type: 'Data', payload: { hello: 'world' } } as any });
  ...
  test('messagePumpLoop handles message too large errors', async () => {
    ...
    const handleSpy = jest.spyOn(manager as any, 'handleMessageTooLarge').mockResolvedValue(undefined);
  ...
  test('messagePumpLoop requeues messages when transport closes', async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    const envelope = createFameEnvelope({ frame: { type: 'Data', payload: {} } as any });
    await manager.send(envelope);
    ...
    await expect((manager as any).messagePumpLoop(connector, stopEvt)).rejects.toThrow('closed');
  ...
  test('messagePumpLoop stops when takeMessage is cancelled', async () => {
    ...
    await expect((manager as any).messagePumpLoop(connector, stopEvt)).resolves.toBeUndefined();
  });
  ...
  test('messagePumpLoop rethrows unexpected send errors', async () => {
    ...
    await expect((manager as any).messagePumpLoop(connector, stopEvt)).rejects.toBe(error);
  });
  ...
  test('takeMessage returns null when abort signal is already triggered', async () => {
    ...
    await expect((manager as any).takeMessage(stopEvt, controller.signal)).resolves.toBeNull();
  });
  ...
  test('handleMessageTooLarge sends nack when reply information is present', async () => {
    ...
    jest.spyOn(FameFabric, 'current').mockReturnValue({ send: fabricSend } as unknown as FameFabric);
  ...
  test('handleMessageTooLarge swallows errors when nack send fails', async () => {
    ...
    await expect((manager as any).handleMessageTooLarge(envelope, 'fail')).resolves.toBeUndefined();
  ...
  test('expiryGuard waits when no expiry data present', async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    const waitSpy = jest.spyOn(manager as any, 'waitEvent').mockResolvedValue(undefined);
    const welcome = createWelcomeFrame();
  ...
  test('expiryGuard stops connector before expiry', async () => {
    ...
    jest.useFakeTimers();
    const promise = (manager as any).expiryGuard(connector, welcome, info, stopEvt);
    jest.runOnlyPendingTimers();
    jest.clearAllTimers();
    await promise;
    expect(connector.stop).toHaveBeenCalled();
  ...
  test('expiryGuard does not stop connector when stop event triggers early', async () => {
    ...
    const welcome = createWelcomeFrame();
    const soon = Date.now() + (UpstreamSessionManager.JWT_REFRESH_SAFETY - 1) * 1000;
  ...
  test('makeHeartbeatEnabledHandler handles heartbeat acknowledgements and downstream forwarding', async () => {
    ...
    const ackEnvelope = createFameEnvelope({
      frame: { type: 'NodeHeartbeatAck', routingEpoch: 'epoch-2' } as NodeHeartbeatAckFrame,
      id: 'ack-1',
      corrId: 'corr',
    });
  ...
  test('makeHeartbeatEnabledHandler ignores NodeAttachAck frames', async () => {
    ...
    const attachAck = createFameEnvelope({ frame: { type: 'NodeAttachAck' } as any });
  ...
  test('waitEvent forwards AbortSignal when provided', async () => {
    const manager = createManager();
    const event = { wait: jest.fn().mockResolvedValue(undefined) } as unknown as AsyncEvent;
    const controller = new AbortController();
  ...
  test('makeHeartbeatEnabledHandler populates context defaults when provided', async () => {
    ...
    const envelope = createFameEnvelope({ frame: { type: 'Data', payload: { value: 42 } } as any });
    ...
    expect(context.security?.authorization).toEqual(connector.authorizationContext);
  ...
  test('makeHeartbeatEnabledHandler warns when epoch changes without callback', async () => {
    ...
    expect(warningSpy).toHaveBeenCalledWith('parent_epoch_changed', { epoch: 'epoch-2' });
    expect(downstream).not.toHaveBeenCalled();
  });
});

...
function createNodeStub(overrides: Partial<NodeLike> = {}): NodeLike {
  const dispatchEnvelopeEvent = jest
    .fn<Promise<FameEnvelope | null>, Parameters<NodeLike["dispatchEnvelopeEvent"]>>()
...
describe("UpstreamSessionManager", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(ConnectorFactory, "createConnector").mockResolvedValue(connector);
  ...
  test("send throws when queue is full", async () => {
    ...
    await expect(
      manager.send({ id: "overflow", frame: { type: "Data", payload: null } } as FameEnvelope)
  ...
  test("fsmLoop applies backoff and retries after prior success", async () => {
    ...
    const connectMock = jest.spyOn(manager as any, "connectCycle").mockImplementation(async () => {
      (manager as any).stopEvent.set();
  ...
  test("fsmLoop retries transport close errors before stopping", async () => {
    const manager = createManager();
    jest.spyOn(manager as any, "connectCycle").mockRejectedValue(new FameTransportClose("closed"));
  ...
  test("fsmLoop rethrows task cancellation errors from connect cycle", async () => {
    const manager = createManager();
    jest.spyOn(manager as any, "connectCycle").mockRejectedValue(new TaskCancelledError("cancel"));
  ...
  test("heartbeatLoop rethrows when wait is cancelled", async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    jest.spyOn(manager as any, "waitEvent").mockRejectedValue(new TaskCancelledError("cancelled"));
  ...
  test("heartbeatLoop stops when stop event triggers during wait", async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    jest.spyOn(manager as any, "waitEvent").mockImplementation(async (...args: unknown[]) => {
  ...
  test("connectCycle prepares crypto provider when assigned path is present", async () => {
    ...
    Object.assign(node, {
      cryptoProvider: { prepareForAttach: prepare } as unknown as CryptoProvider,
    });
  ...
  test("waitForFailureOrStop cancels tasks when stop event fires", async () => {
    const manager = createManager();
    const stopEvt = new AsyncEvent();
    const tasks: SpawnedTask<void>[] = [createTaskStub(new Promise<void>(() => undefined))];

...
describe('StreamingResponseHandler', () => {
  ...
  let deliverFn: jest.Mock<Promise<void>, [FameEnvelope, FameDeliveryContext | undefined]>;
  ...
  const generatedContext = { generated: true } as unknown as FameDeliveryContext;

  beforeEach(() => {
  deliverFn = jest.fn<Promise<void>, [FameEnvelope, FameDeliveryContext | undefined]>(async () => {});
  deliverWrapper = jest.fn<DeliverFn, []>(() => deliverFn);
    ...
    handler = new StreamingResponseHandler(deliverWrapper, envelopeFactory, responseContextManager);
  });
  ...
  describe('handleStreamingFameMessageResponses', () => {
    it('skips invalid responses and forwards valid ones with proper context', async () => {
      ...
      const requestContext = { origin: 'origin' } as unknown as FameDeliveryContext;

      await handler.handleStreamingFameMessageResponses(createResponses(), requestEnvelope, requestContext);

      expect(deliverFn).toHaveBeenCalledTimes(2);
      expect(deliverWrapper).toHaveBeenCalledTimes(2);
      expect(responseContextManager.ensureResponseMetadata).toHaveBeenCalledTimes(2);
      expect(responseContextManager.createResponseContext).toHaveBeenCalledTimes(1);
  ...
  describe('handleStreamingResponse', () => {
      ...
      const iterator = {
        next: jest.fn(() => {
          if (idx < iteratorValues.length) {
            return Promise.resolve({ value: iteratorValues[idx++], done: false });
          }
          return Promise.resolve({ value: undefined, done: true });
        }),
      } as AsyncIterator<unknown>;

      const sendSpy = jest.spyOn(handler as unknown as { sendRpcResponse: jest.Mock }, 'sendRpcResponse');
      sendSpy.mockResolvedValue(undefined);

      const requestEnvelope = { id: 'req-iter', frame: { type: 'Data', payload: null } } as unknown as FameEnvelope;
      const requestContext = { ctx: true } as unknown as FameDeliveryContext;

      await handler.handleStreamingResponse(iterator, requestEnvelope, requestContext, 'reply-to', 'request-123');
    ...
    it('sends error payload when iteration throws an Error', async () => {
      ...
      await handler.handleStreamingResponse(
        iterable,
        { id: 'req-error', frame: { type: 'Data', payload: null } } as unknown as FameEnvelope,
        undefined,
        'reply-to',
        'req-error'
      );
    ...
    it('sends error payload using stringified message when iteration throws non-error', async () => {
      ...
      await handler.handleStreamingResponse(
        iterable,
        { id: 'req-string', frame: { type: 'Data', payload: null } } as unknown as FameEnvelope,
        undefined,
        'reply-to',
        'req-string'
      );
  ...
  describe('sendRpcResponse', () => {
    it('creates error responses with trace metadata', async () => {
      ...
      const responseEnvelope = { id: 'response-env' } as unknown as FameEnvelope;
      envelopeFactory.createEnvelope.mockReturnValue(responseEnvelope);

      const errorPayload = { code: 42, message: 'boom', data: { detail: true } };

      await (handler as unknown as { sendRpcResponse: Function }).sendRpcResponse(
        { error: errorPayload },
        requestEnvelope,
        requestContext,
        'reply-target',
      ...
      expect(deliverFn).toHaveBeenCalledWith(responseEnvelope, generatedContext);
    });
    ...
    it('creates success responses without optional metadata', async () => {
      ...
      const responseEnvelope = { id: 'response-env-2' } as unknown as FameEnvelope;
      envelopeFactory.createEnvelope.mockReturnValue(responseEnvelope);

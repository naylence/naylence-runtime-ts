...
const { __mockLogger: loggerMock } = jest.requireMock(
  '../../util/logging.js'
) as {
  __mockLogger: {
    debug: jest.Mock;
...
function createAdmissionClient(
  overrides: Partial<AdmissionClient> = {}
): AdmissionClient {
  return {
    hasUpstream: true,
    hello: jest.fn(),
...
function createManager(
  overrides: Partial<RootSessionManagerOptions> = {}
): RootSessionManager {
...
describe('RootSessionManager', () => {
    ...
    it('provides default welcome and failure handlers', async () => {
      ...
      await (
        manager as unknown as {
          onWelcome: (frame: NodeWelcomeFrame) => Promise<void>;
        }
      ...
      await (
        manager as unknown as {
          onAdmissionFailed: (error: Error) => Promise<void>;
        }
      ).onAdmissionFailed(new Error('fatal'));

      expect(loggerMock.error).toHaveBeenCalledWith(
        'root_admission_failed_permanently',
        {
          error: 'fatal',
        }
      );
  ...
  describe('start', () => {
    it('returns early when already running', async () => {
      ...
      (
        manager as unknown as { admissionTask: SpawnedTask<void> | null }
      ).admissionTask = existingTask;
    ...
    it('waits for readiness when admission loop resolves', async () => {
      ...
      const admissionLoop = jest
        .spyOn(
          manager as unknown as {
            admissionLoop: (signal?: AbortSignal) => Promise<void>;
          },
          'admissionLoop'
        )
  ...
  describe('stop', () => {
    it('cancels background tasks and clears references', async () => {
      ...
      (
        manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }
      ).expiryGuardTask = expiryTask;
  ...
  describe('performAdmission', () => {
    it('initializes identity, stores welcome, and prepares crypto', async () => {
      ...
      expect(
        (manager as unknown as { currentWelcome: NodeWelcomeFrame | null })
          .currentWelcome
      ).toBe(welcomeFrame);
    });
    ...
    it('returns welcome frame without invoking crypto when path missing', async () => {
      ...
      (admissionClient.hello as jest.Mock).mockResolvedValue({
        frame: welcomeFrame,
      });
  ...
  describe('handleEpochChange', () => {
    ...
    it('logs when epoch handler is missing', async () => {
      ...
      expect(loggerMock.debug).toHaveBeenCalledWith(
        'epoch_change_ignored_no_handler',
        {
          epoch: 'ignored',
        }
      );
  ...
  describe('backoff and sleep helpers', () => {
    ...
    it('returns immediately when sleep duration is non-positive', async () => {
      ...
      await expect(
        (
          manager as unknown as {
            sleepWithStop: (delay: number) => Promise<void>;
          }
        ).sleepWithStop(0)
    ...
    it('clears timers when stop event fires during sleep', async () => {
      ...
      (
        manager as unknown as { stopEvent: { set: () => void } }
      ).stopEvent.set();
    ...
    it('caps exponential backoff at the configured maximum', async () => {
      ...
      expect(sleepSpy).toHaveBeenCalledWith(
  ...
  describe('expiry handling', () => {
    it('starts expiry guard when none is running', async () => {
      ...
      await (
        manager as unknown as {
          startExpiryGuard: (frame: NodeWelcomeFrame) => Promise<void>;
        }
    ...
    it('cancels expiry guard when stop event wins the race', async () => {
      ...
      expect(expiryTask.cancel).toHaveBeenCalled();
      (
        manager as unknown as { stopEvent: { clear: () => void } }
      ).stopEvent.clear();
      (
        manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }
      ).expiryGuardTask = null;
    });
    ...
    it('handles task cancellation during expiry wait', async () => {
      const manager = createManager();
      const cancelledPromise = Promise.reject(
        new TaskCancelledError('cancelled')
      );
    ...
    it('waits for stop event when no expiry timestamp configured', async () => {
      ...
      await expect(
        (
          manager as unknown as {
            expiryGuard: (frame: NodeWelcomeFrame) => Promise<void>;
          }
        ).expiryGuard({
          systemId: 'sys',
        } as NodeWelcomeFrame)
  ...
  describe('identity and task utilities', () => {
    ...
    it('ignores TaskCancelledError when consuming task', async () => {
      ...
      const task = createTaskStub(
        'cancelled',
        Promise.reject(new TaskCancelledError('cancelled'))
      );

      await expect(
        (
          manager as unknown as {
            consumeTask: (task: SpawnedTask<void>) => Promise<void>;
          }
        ).consumeTask(task)
      ).resolves.toBeUndefined();
    });
  ...
  describe('abort and timeout helpers', () => {
    it('rejects instantly when signal already aborted', async () => {
      ...
      await expect(
        (
          manager as unknown as {
            waitForAbort: (signal: AbortSignal) => Promise<never>;
          }
  ...
  describe('admissionLoop', () => {
    ...
    it('retries admissions and reports failure when never successful', async () => {
      ...
      await expect(
        (
          manager as unknown as { admissionLoop: () => Promise<void> }
        ).admissionLoop()
      ).rejects.toThrow('fail');

      expect(onAdmissionFailed).toHaveBeenCalledWith(
    ...
    it('exhausts retries and rethrows when failure handler missing', async () => {
      ...
      expect(backoffSpy).toHaveBeenCalled();
      const lastWarning =
        loggerMock.warning.mock.calls[loggerMock.warning.mock.calls.length - 1];
      expect(lastWarning?.[0]).toBe('root_admission_failed');
      expect(lastWarning?.[1]).toEqual(
        expect.objectContaining({ will_retry: false })
      );
    });
    ...
    it('logs when maximum retries exceeded after prior success', async () => {
      ...
      jest
        .spyOn(manager, 'performAdmission')
        .mockRejectedValue(new Error('late-fail'));
      const backoffSpy = jest
        .spyOn(
          manager as unknown as {
            applyBackoff: (
      ...
      expect(backoffSpy).toHaveBeenCalled();
      expect(loggerMock.error).toHaveBeenCalledWith(
        'root_admission_max_attempts_exceeded',
        {
    ...
    it('propagates task cancellation errors', async () => {
      ...
      jest
        .spyOn(manager, 'performAdmission')
        .mockRejectedValue(new TaskCancelledError('stop'));

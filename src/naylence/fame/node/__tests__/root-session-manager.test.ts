...
describe('RootSessionManager', () => {
  describe('createForRootSentinel', () => {
    it('provides default welcome and failure handlers', async () => {
      const manager = RootSessionManager.createForRootSentinel(createNode(), createAdmissionClient(), ['logical']);
      createdManagers.push(manager);
      ...
      await (manager as unknown as { onWelcome: (frame: NodeWelcomeFrame) => Promise<void> }).onWelcome(welcomeFrame);
      ...
      expect(loggerMock.debug).toHaveBeenCalledWith('received_admission_grant', { purpose: 'ingress' });

      await (manager as unknown as { onAdmissionFailed: (error: Error) => Promise<void> }).onAdmissionFailed(
        new Error('fatal')
      );

      expect(loggerMock.error).toHaveBeenCalledWith('root_admission_failed_permanently', { error: 'fatal' });
    });
  });
  ...
  describe('start', () => {
    it('returns early when already running', async () => {
      ...
      (manager as unknown as { admissionTask: SpawnedTask<void> | null }).admissionTask = existingTask;
      const spawnSpy = jest.spyOn(manager, 'spawn');

      await manager.start();

      expect(spawnSpy).not.toHaveBeenCalled();
      expect((manager as unknown as { admissionTask: SpawnedTask<void> | null }).admissionTask).toBe(existingTask);
    });

    it('waits for readiness when admission loop resolves', async () => {
      const manager = createManager();
      const admissionLoop = jest
        .spyOn(manager as unknown as { admissionLoop: (signal?: AbortSignal) => Promise<void> }, 'admissionLoop')
  ...
  describe('stop', () => {
    it('cancels background tasks and clears references', async () => {
      ...
      (manager as unknown as { admissionTask: SpawnedTask<void> | null }).admissionTask = admissionTask;
      (manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }).expiryGuardTask = expiryTask;

      await manager.stop();

      expect(admissionTask.cancel).toHaveBeenCalled();
      expect(expiryTask.cancel).toHaveBeenCalled();
      expect((manager as unknown as { admissionTask: SpawnedTask<void> | null }).admissionTask).toBeNull();
      expect((manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }).expiryGuardTask).toBeNull();
    });
  });
  ...
  describe('awaitReady', () => {
    ...
    it('throws when admission finishes without readiness', async () => {
      ...
      await expect(manager.awaitReady()).rejects.toThrow('Root session manager terminated before ready');
    });
    ...
    it('rejects when wait timeout elapses first', async () => {
      ...
      await expect(manager.awaitReady(0)).rejects.toThrow('Timed out waiting for root session manager readiness');
  ...
  describe('performAdmission', () => {
    it('initializes identity, stores welcome, and prepares crypto', async () => {
      ...
      expect((manager as unknown as { currentWelcome: NodeWelcomeFrame | null }).currentWelcome).toBe(welcomeFrame);
    });
  ...
  describe('handleEpochChange', () => {
    ...
    it('logs when epoch handler is missing', async () => {
      ...
      expect(loggerMock.debug).toHaveBeenCalledWith('epoch_change_ignored_no_handler', { epoch: 'ignored' });
    });
  });
  ...
  describe('backoff and sleep helpers', () => {
    it('applies exponential backoff with jitter', async () => {
      const manager = createManager();
      const sleepSpy = jest
        .spyOn(manager as unknown as { sleepWithStop: (delay: number, signal?: AbortSignal) => Promise<void> }, 'sleepWithStop')
        .mockResolvedValue(undefined);
      jest.spyOn(Math, 'random').mockReturnValue(0.25);

      const nextDelay = await (manager as unknown as { applyBackoff: (delay: number, signal?: AbortSignal) => Promise<number> }).applyBackoff(4);
    ...
    it('returns immediately when sleep duration is non-positive', async () => {
      ...
      await expect((manager as unknown as { sleepWithStop: (delay: number) => Promise<void> }).sleepWithStop(0)).resolves.toBeUndefined();
    });

    it('clears timers when stop event fires during sleep', async () => {
      jest.useFakeTimers();
      const manager = createManager();
      (manager as unknown as { stopEvent: { set: () => void; clear: () => void } }).stopEvent.clear();
      const sleepPromise = (manager as unknown as { sleepWithStop: (delay: number) => Promise<void> }).sleepWithStop(10);
    ...
    it('allows timers to resolve naturally without clearing again', async () => {
      ...
      const sleepPromise = (manager as unknown as { sleepWithStop: (delay: number) => Promise<void> }).sleepWithStop(1);
    ...
    it('caps exponential backoff at the configured maximum', async () => {
      ...
      const result = await (manager as unknown as { applyBackoff: (delay: number, signal?: AbortSignal) => Promise<number> }).applyBackoff(
        RootSessionManager.BACKOFF_CAP
      );
  ...
  describe('expiry handling', () => {
    it('starts expiry guard when none is running', async () => {
      ...
      await (manager as unknown as { startExpiryGuard: (frame: NodeWelcomeFrame) => Promise<void> }).startExpiryGuard(welcome);

      expect(spawnSpy).toHaveBeenCalledWith(expect.any(Function), {
        name: expect.stringContaining('root-expiry-guard-'),
      });
      expect((manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }).expiryGuardTask).toBe(spawnedTask);
    });

    it('replaces existing expiry guard tasks', async () => {
      const manager = createManager();
      const previousTask = createTaskStub('previous');
      (manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }).expiryGuardTask = previousTask;
      ...
      expect((manager as unknown as { expiryGuardTask: SpawnedTask<void> | null }).expiryGuardTask).toBe(newTask);
    });

    it('returns false when no expiry guard task exists', async () => {
      const manager = createManager();

      const result = await (manager as unknown as { waitForExpiryOrStop: () => Promise<boolean> }).waitForExpiryOrStop();
    ...
    it('waits for stop event when no expiry timestamp configured', async () => {
      ...
      await expect(
        (manager as unknown as { expiryGuard: (frame: NodeWelcomeFrame) => Promise<void> }).expiryGuard({
          systemId: 'sys',
        } as NodeWelcomeFrame)
    ...
    it('logs when admission expiry triggers refresh', async () => {
      ...
      await (manager as unknown as { expiryGuard: (frame: NodeWelcomeFrame) => Promise<void> }).expiryGuard({
        systemId: 'sys',
    ...
    it('stops expiry guard when abort signal triggers', async () => {
      ...
      await expect(
        (manager as unknown as { expiryGuard: (frame: NodeWelcomeFrame, signal?: AbortSignal) => Promise<void> }).expiryGuard(
          {
            systemId: 'sys',
            expiresAt: new Date(Date.now() + 60000).toISOString(),
          } as NodeWelcomeFrame,
  ...
  describe('identity and task utilities', () => {
    it('generates identity when node id missing', () => {
      ...
      (manager as unknown as { initializeRootIdentityIfNeeded: () => void }).initializeRootIdentityIfNeeded();
    ...
    it('ignores TaskCancelledError when consuming task', async () => {
      ...
      await expect((manager as unknown as { consumeTask: (task: SpawnedTask<void>) => Promise<void> }).consumeTask(task)).resolves.toBeUndefined();
    });
    ...
    it('logs unexpected errors when consuming task', async () => {
      ...
      await (manager as unknown as { consumeTask: (task: SpawnedTask<void>) => Promise<void> }).consumeTask(task);
  ...
  describe('abort and timeout helpers', () => {
    it('rejects instantly when signal already aborted', async () => {
      ...
      await expect(
        (manager as unknown as { waitForAbort: (signal: AbortSignal) => Promise<never> }).waitForAbort(controller.signal)
    ...
    it('rejects when abort signal fires later', async () => {
      const manager = createManager();
      const controller = new AbortController();
      const waitPromise = (manager as unknown as { waitForAbort: (signal: AbortSignal) => Promise<never> }).waitForAbort(
        controller.signal
      );
      controller.abort();
    ...
    it('rejects after configured timeout', async () => {
      jest.useFakeTimers();
      const manager = createManager();
      const waitPromise = (manager as unknown as { waitWithTimeout: (timeout: number) => Promise<void> }).waitWithTimeout(5);
      jest.runAllTimers();
    ...
    it('returns unresolved promise when no signal supplied', async () => {
      const manager = createManager();
      const promise = (manager as unknown as { waitForAbort: (signal?: AbortSignal) => Promise<never> }).waitForAbort();
  ...
  describe('admissionLoop', () => {
    it('marks ready and may refresh before returning', async () => {
      ...
      (manager as unknown as { onWelcome: (frame: NodeWelcomeFrame) => Promise<void> }).onWelcome = onWelcome;
      const startExpiryGuard = jest
        .spyOn(manager as unknown as { startExpiryGuard: (frame: NodeWelcomeFrame) => Promise<void> }, 'startExpiryGuard')
        .mockResolvedValue(undefined);
      const waitForExpiryOrStop = jest
        .spyOn(manager as unknown as { waitForExpiryOrStop: () => Promise<boolean> }, 'waitForExpiryOrStop')
        .mockResolvedValueOnce(true);

      await (manager as unknown as { admissionLoop: (signal?: AbortSignal) => Promise<void> }).admissionLoop();
    ...
    it('retries admissions and reports failure when never successful', async () => {
      ...
      const backoffSpy = jest
        .spyOn(manager as unknown as { applyBackoff: (delay: number, signal?: AbortSignal) => Promise<number> }, 'applyBackoff')
        .mockImplementation(async (delay) => delay);

      await expect((manager as unknown as { admissionLoop: () => Promise<void> }).admissionLoop()).rejects.toThrow('fail');
    ...
    it('throws fame connect errors without invoking failure handler when absent', async () => {
      const manager = createManager();
      jest.spyOn(manager, 'performAdmission').mockRejectedValue(new FameConnectError('connect-fail'));
      const backoffSpy = jest
        .spyOn(manager as unknown as { applyBackoff: (delay: number, signal?: AbortSignal) => Promise<number> }, 'applyBackoff')
        .mockImplementation(async (delay) => delay);

      await expect((manager as unknown as { admissionLoop: () => Promise<void> }).admissionLoop()).rejects.toBeInstanceOf(
        FameConnectError
      );
    ...
    it('exhausts retries and rethrows when failure handler missing', async () => {
      ...
      await expect((manager as unknown as { admissionLoop: () => Promise<void> }).admissionLoop()).rejects.toThrow('fatal');

...
import { TrackedEnvelope, EnvelopeStatus } from '../../delivery/tracked-envelope.js';
...
function setupManager(options: { useRealChannelPolling?: boolean } = {}): ManagerSetup {
  ...
  const deliveryTracker: MockDeliveryTracker = {
    listInbound: jest.fn<Promise<TrackedEnvelope[]>, []>().mockResolvedValue([]),
    ...
    onEnvelopeHandled: jest.fn<Promise<void>, [TrackedEnvelope]>().mockResolvedValue(),
  ...
  const rpcClientManagerSpies = {
    invoke: jest.spyOn(rpcClientManagerInstance, 'invoke').mockResolvedValue('rpc-result'),
    invokeStream: jest
      .spyOn(rpcClientManagerInstance, 'invokeStream')
      .mockResolvedValue((async function* () {})()),
    cleanup: jest.spyOn(rpcClientManagerInstance, 'cleanup').mockResolvedValue(undefined),
  };
...
describe('EnvelopeListenerManager', () => {
  ...
  it('processes cached envelopes during recovery', async () => {
    ...
    const tracked = createTrackedEnvelope('svc', EnvelopeStatus.FAILED_TO_HANDLE, 1);
  ...
  it('skips recovery when no cached envelopes are present', async () => {
    ...
    const pendingEnvelopes: Map<string, TrackedEnvelope[]> = (manager as any).pendingRecoveryEnvelopes;
    const pendingServices: Set<string> = (manager as any).pendingRecoveryServices;
  ...
  it('retries handler according to retry policy and succeeds after retry', async () => {
    ...
    const envelope = createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED).originalEnvelope;
    const tracked = createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED);
    const retryPolicy = new RetryPolicy({ maxRetries: 2, baseDelayMs: 0, jitterMs: 0 });
    const handler = jest
      .fn()
  ...
  it('marks envelope as failed when retries exhausted', async () => {
    ...
    const envelope = createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED).originalEnvelope;
    const tracked = createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED);
    const retryPolicy = new RetryPolicy({ maxRetries: 1, baseDelayMs: 0, jitterMs: 0 });
  ...
  it('runs handler only for envelopes that need processing', async () => {
    ...
    deliveryTracker.onEnvelopeDelivered
      .mockResolvedValueOnce(createTrackedEnvelope('svc', EnvelopeStatus.HANDLED))
      .mockResolvedValueOnce(createTrackedEnvelope('svc', EnvelopeStatus.FAILED_TO_HANDLE));
  ...
  it('continues recovery when handler keeps failing', async () => {
    const { manager, deliveryTracker } = setupManager();
    const tracked = createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED);
    const failingHandler = jest.fn().mockRejectedValue(new Error('recover fail'));

    await (manager as any).recoverServiceEnvelopes('svc', [tracked], failingHandler);
  ...
  it('returns early from recovery when no handler is registered', async () => {
    const { manager, deliveryTracker } = setupManager();
    const pendingEnvelopes: Map<string, TrackedEnvelope[]> = (manager as any).pendingRecoveryEnvelopes;
    pendingEnvelopes.set('svc', [createTrackedEnvelope('svc', EnvelopeStatus.RECEIVED)]);
  ...
  it('falls back to default polling timeout when null provided', async () => {
    ...
    await manager.listen('svc', jest.fn().mockResolvedValue(null), { pollTimeoutMs: null });
  ...
  it('replaces existing listener when listen is called twice', async () => {
    ...
    existing.listener.stop = jest.fn(existing.listener.stop.bind(existing.listener));
  ...
  it('wires deliver helpers for internal components', async () => {
    ...
    const deliverFn = (manager as any).deliver as (env: FameEnvelope, ctx?: FameDeliveryContext) => Promise<void>;
    ...
    const streamingDeliver = (manager as any).streamingResponseHandler.deliverWrapper();
    ...
    const channelDeliver = (manager as any).channelPollingManager.deliverWrapper();
  ...
  it('delivers to registered address and rejects unknown addresses', async () => {
    ...
    await expect(
      manager.deliverToAddress({ toString: () => 'other@node' } as any, responseEnvelope)
  ...
  it('delegates RPC listening and invocation helpers', async () => {
    const { manager, rpcServerHandlerSpy, rpcClientManagerSpies, channelPollingMock } = setupManager();
    ...
    rpcServerHandlerSpy.mockResolvedValue({ envelope: createFameEnvelope({
      id: 'resp',
      frame: { type: 'Data', payload: {} },
    }), context: undefined });

    const rpcHandler = jest.fn().mockResolvedValue('rpc-ok');
    const address = await manager.listenRpc('svc-rpc', rpcHandler, { pollTimeoutMs: 1000 });

    expect(address.toString()).toContain('svc-rpc');
    expect(rpcServerHandlerSpy).toHaveBeenCalled();

    await manager.invoke({ targetAddr: address, method: 'do', params: {}, timeoutMs: 2000 });
    ...
    await manager.invoke({ capabilities: ['cap'], method: 'cap', params: {}, timeoutMs: 1500 });
    expect(rpcClientManagerSpies.invoke).toHaveBeenCalledWith({
      capabilities: ['cap'],
      method: 'cap',
      params: {},
    ...
    await manager.invokeStream({ targetAddr: address, method: 'stream', params: {}, timeoutMs: 500 });
    ...
    await manager.invokeStream({ capabilities: ['cap'], method: 'stream', params: {}, timeoutMs: 400 });
    expect(rpcClientManagerSpies.invokeStream).toHaveBeenCalledWith({
      capabilities: ['cap'],
      method: 'stream',
      params: {},
  ...
  it('returns null RPC handler response when server returns null', async () => {
    ...
    channelPollingMock.mockImplementation(async (_svc, _channel, handler) => {
      await handler(
        createFameEnvelope({ id: 'rpc-null', frame: { type: 'Data', payload: {} } }),
        undefined
      );
    });
  ...
  it('invokes RPC stream client with default timeout when none provided', async () => {
    ...
    await manager.invokeStream({ targetAddr: address, method: 'stream', params: {} });

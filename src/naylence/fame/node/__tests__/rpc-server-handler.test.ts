...
describe("RPCServerHandler", () => {
  ...
  it("returns undefined when frame is not a Data frame", async () => {
    ...
    const result = await handler.handleRpcRequest(envelope, undefined, jest.fn(), "service");
  ...
  it("swallows parse errors and returns undefined", async () => {
    const { handler } = createHandler();
    const parseSpy = jest.spyOn(core, "parseRequest").mockImplementationOnce(() => {
  ...
  it("wraps handler errors with a default internal error shape", async () => {
    ...
    await handler.handleRpcRequest(createDataEnvelope({}), undefined, handlerFn, "service");

...
type KeyManagementHandlerMocks = {
  ...
  maybeRequestSigningKey: jest.Mock<Promise<void>, [string, DeliveryOriginType, string]>;
  queuePendingEncryptionEnvelope: jest.Mock<void, [string, FameEnvelope, FameDeliveryContext]>;
  maybeRequestEncryptionKey: jest.Mock<Promise<void>, [string, DeliveryOriginType, string]>;
...
function createKeyManagementHandlerMock(): {
  ...
  const fns: KeyManagementHandlerMocks = {
    ...
    maybeRequestSigningKey: jest.fn<Promise<void>, [string, DeliveryOriginType, string]>(
      async () => undefined
    ),
    queuePendingEncryptionEnvelope: jest.fn<void, [string, FameEnvelope, FameDeliveryContext]>(),
    maybeRequestEncryptionKey: jest.fn<Promise<void>, [string, DeliveryOriginType, string]>(
      async () => undefined
    ),
...
function createHandler(options?: {
  ...
  return {
    ...
    envelopeVerifierMock:
      envelopeVerifier === options?.envelopeVerifier ? null : defaultVerifierMock,
  };
}
...
describe("EnvelopeSecurityHandler", () => {
  ...
  describe("shouldDecryptEnvelope", () => {
    ...
    it("returns false when neither policy nor headers require decryption", async () => {
      ...
      await expect(handler.shouldDecryptEnvelope(createEnvelope(), createContext())).resolves.toBe(
        false
      );
    });
  });
  ...
  describe("handleFailedChannelEnvelopeCleanup (private)", () => {
    it("logs cleanup attempts without throwing", async () => {
      ...
      await expect(
        (handler as any).handleFailedChannelEnvelopeCleanup("naylence://peer", "handshake-failed")
      ).resolves.toBeUndefined();
    });
  });
});

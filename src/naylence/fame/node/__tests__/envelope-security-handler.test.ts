...
type KeyManagementHandlerMocks = {
  ...
  queuePendingSignedEnvelope: jest.Mock<void, [string, FameEnvelope, FameDeliveryContext]>;
  maybeRequestSigningKey: jest.Mock<
    Promise<void>,
    [string, DeliveryOriginType, string]
  >;
  queuePendingEncryptionEnvelope: jest.Mock<void, [string, FameEnvelope, FameDeliveryContext]>;
...
function createEnvelope(overrides: Partial<FameEnvelope> = {}): FameEnvelope {
  envelopeCounter += 1;
  const frame: DataFrame = (overrides.frame ?? { type: 'Data', payload: {} }) as DataFrame;
  return {
...
function createSecurityPolicyMock(): jest.Mocked<SecurityPolicy> {
  return {
    ...
    classifyMessageCryptoLevel: jest.fn().mockReturnValue(CryptoLevel.PLAINTEXT),
    ...
    decideResponseCryptoLevel: jest.fn().mockResolvedValue(CryptoLevel.PLAINTEXT),
    decideOutboundCryptoLevel: jest.fn().mockResolvedValue(CryptoLevel.PLAINTEXT),
    isSignatureRequired: jest.fn().mockReturnValue(false),
    getUnsignedViolationAction: jest.fn().mockReturnValue(SecurityAction.ALLOW),
    getInvalidSignatureViolationAction: jest.fn().mockReturnValue(SecurityAction.ALLOW),
...
function createKeyManagementHandlerMock(): { instance: KeyManagementHandler; fns: KeyManagementHandlerMocks } {
  const fns: KeyManagementHandlerMocks = {
    hasKey: jest.fn<Promise<boolean>, [string]>(async () => true),
    queuePendingSignedEnvelope: jest.fn<void, [string, FameEnvelope, FameDeliveryContext]>(),
    maybeRequestSigningKey: jest.fn<
      Promise<void>,
      [string, DeliveryOriginType, string]
    >(async () => undefined),
    queuePendingEncryptionEnvelope: jest.fn<void, [string, FameEnvelope, FameDeliveryContext]>(),
...
function createHandler(options?: {
  ...
  keyManagement?: { instance: KeyManagementHandler; fns: KeyManagementHandlerMocks };
  node?: NodeLike;
}) {
  const securityPolicy = options?.securityPolicy ?? createSecurityPolicyMock();
  const encryptionManager =
    options?.encryptionManager === undefined ? createEncryptionManagerMock() : options.encryptionManager;
  const keyManagement = options?.keyManagement ?? createKeyManagementHandlerMock();
  ...
  return {
    ...
    encryptionManager,
    keyManagement,
    envelopeSignerMock: envelopeSigner === options?.envelopeSigner ? null : defaultSignerMock,
    envelopeVerifierMock: envelopeVerifier === options?.envelopeVerifier ? null : defaultVerifierMock,
  };
}
...
describe('EnvelopeSecurityHandler', () => {
  ...
  describe('handleOutboundSecurity', () => {
    it('signs envelopes when the security policy requires signing', async () => {
      ...
      const node = createNode({ sid: 'node-sid', physicalPath: '/physical/test' });
      ...
      expect(signerMock).toHaveBeenCalledWith(envelope, { physicalPath: node.physicalPath });
    ...
    it('throws when signing is required but no signer is configured', async () => {
      ...
      const { handler } = createHandler({ securityPolicy, envelopeSigner: null });
    ...
    it('skips encryption when the envelope already contains encryption headers', async () => {
      ...
      const { handler, securityPolicy: policy, encryptionManager } = createHandler({ securityPolicy });
      const envelope = createEnvelope({ sec: { enc: { alg: 'x' } as unknown } as any });

      const result = await handler.handleOutboundSecurity(envelope, createContext());
    ...
    it('uses response crypto decisions for response envelopes', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.shouldEncryptEnvelope.mockResolvedValue(true);
      securityPolicy.decideResponseCryptoLevel.mockResolvedValue(CryptoLevel.SEALED);
    ...
    it('applies channel encryption when the policy requests channel level', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.shouldEncryptEnvelope.mockResolvedValue(true);
      securityPolicy.decideOutboundCryptoLevel.mockResolvedValue(CryptoLevel.CHANNEL);
    ...
    it('returns true when encryption manager is missing even if encryption is requested', async () => {
      ...
      const result = await handler.handleOutboundSecurity(createEnvelope(), createContext());
    ...
    it('returns true when desired crypto level is plaintext', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.shouldEncryptEnvelope.mockResolvedValue(true);
      securityPolicy.decideOutboundCryptoLevel.mockResolvedValue(CryptoLevel.PLAINTEXT);
    ...
    it('consults response crypto decisions for protocol responses', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.shouldEncryptEnvelope.mockResolvedValue(true);
      securityPolicy.decideResponseCryptoLevel.mockResolvedValue(CryptoLevel.PLAINTEXT);

      const { handler, securityPolicy: policy } = createHandler({ securityPolicy });
      const originalSealed = (handler as any).handleSealedEncryption;
      const originalChannel = (handler as any).handleChannelEncryption;
      ...
      const envelope = createEnvelope();
      const context = createContext({
        meta: { 'message-type': 'protocol-response', 'response-to-id': 'req-123' },
  ...
  describe('handleEnvelopeSecurity', () => {
    ...
    it('retains the highest inbound crypto level seen on the context', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.classifyMessageCryptoLevel.mockReturnValue(CryptoLevel.PLAINTEXT);
    ...
    it('queues signed envelopes while waiting for missing signing keys', async () => {
      ...
      const envelope = createEnvelope({ sec: { sig: { kid: 'kid-123' } } as any });
      const context = createContext({
      ...
      const [, accepted] = await handler.handleEnvelopeSecurity(envelope, context);

      expect(accepted).toBe(false);
      expect(context.security?.inboundWasSigned).toBe(true);
      expect(keyManagement.fns.queuePendingSignedEnvelope).toHaveBeenCalledWith('kid-123', envelope, context);
    ...
    it('applies the unsigned violation action for non-critical frames', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.isSignatureRequired.mockReturnValue(true);
      securityPolicy.getUnsignedViolationAction.mockReturnValue(SecurityAction.REJECT);
  ...
  describe('channel handshake notifications', () => {
    it('notifies the encryption manager when a channel handshake succeeds', async () => {
      ...
      await handler.handleChannelHandshakeComplete('channel-42', 'dest-address');

      expect(encryptionManager.notifyChannelEstablished).toHaveBeenCalledWith('channel-42');
    });

    it('lets the encryption manager handle channel handshake failures when possible', async () => {
      const encryptionManager = createEncryptionManagerMock();
      const { handler } = createHandler({ encryptionManager });
      const originalCleanup = (handler as any).handleFailedChannelEnvelopeCleanup;
      const cleanupMock = jest.fn().mockResolvedValue(undefined);
      (handler as any).handleFailedChannelEnvelopeCleanup = cleanupMock;

      try {
        await handler.handleChannelHandshakeFailed('channel-7', 'addr', 'timeout');
        expect(encryptionManager.notifyChannelFailed).toHaveBeenCalledWith('channel-7', 'timeout');
    ...
    it('falls back to cleanup when no encryption manager is available', async () => {
      ...
      try {
        await handler.handleChannelHandshakeFailed('channel-8', 'addr', 'error');
  ...
  describe('shouldDecryptEnvelope', () => {
    it('returns true when the policy indicates decryption is required', async () => {
      ...
      await expect(handler.shouldDecryptEnvelope(envelope, context)).resolves.toBe(true);
    });
    ...
    it('returns true for channel encrypted envelopes even without policy approval', async () => {
      ...
      securityPolicy.classifyMessageCryptoLevel.mockReturnValue(CryptoLevel.CHANNEL);

      const { handler } = createHandler({ securityPolicy, encryptionManager: null });
      const envelope = createEnvelope({ sec: { enc: { kid: 'abc' } as unknown } as any });

      await expect(handler.shouldDecryptEnvelope(envelope, createContext())).resolves.toBe(true);
    });
    ...
    it('returns false when neither policy nor headers require decryption', async () => {
      ...
      await expect(handler.shouldDecryptEnvelope(createEnvelope(), createContext())).resolves.toBe(false);
    });
  });
  ...
  describe('decryptEnvelope', () => {
    ...
    it('delegates to the encryption manager when present', async () => {
      const encryptionManager = createEncryptionManagerMock();
      const decrypted = createEnvelope({ frame: { type: 'Data', payload: { ok: true } } as DataFrame });
      encryptionManager.decryptEnvelope.mockResolvedValue(decrypted);

      const { handler } = createHandler({ encryptionManager });

      await expect(
        handler.decryptEnvelope(createEnvelope(), { requestAddress: 'naylence://peer' })
  ...
  describe('performEncryption and queue handling', () => {
    it('queues envelopes by recipient key id when encryption manager defers encryption', async () => {
      const encryptionManager = createEncryptionManagerMock();
      encryptionManager.encryptEnvelope.mockResolvedValue(EncryptionResult.queued());
      ...
      let result = false;
      try {
        result = await (handler as any).performEncryption(envelope, context, options);
      } finally {
    ...
    it('handles queueing by request address when provided', async () => {
      ...
      expect(keyManagement.fns.queuePendingEncryptionEnvelope).toHaveBeenCalledWith(
        'naylence://node/peer',
        envelope,
        context
      );
      expect(keyManagement.fns.maybeRequestEncryptionKeyByAddress).toHaveBeenCalledWith(
        'naylence://node/peer',
    ...
    it('skips queueing when channel encryption queueing is handled internally', async () => {
      ...
      await (handler as any).handleEncryptionQueueing(createEnvelope(), createContext({
        originType: DeliveryOriginType.LOCAL,
      ...
      expect(keyManagement.fns.queuePendingEncryptionEnvelope).not.toHaveBeenCalled();
    });
    ...
    it('logs unknown queueing options without throwing', async () => {
      ...
      await (handler as any).handleEncryptionQueueing(
        createEnvelope(),
        createContext({ originType: DeliveryOriginType.LOCAL, fromSystemId: 'system-4' }),
        {}
      );
    ...
    it('applies encrypted envelope data when encryption succeeds', async () => {
      ...
      encryptionManager.encryptEnvelope.mockResolvedValue(EncryptionResult.ok(encryptedEnvelope));
    ...
    it('returns true when encryption is skipped by the manager', async () => {
      const encryptionManager = createEncryptionManagerMock();
      encryptionManager.encryptEnvelope.mockResolvedValue(EncryptionResult.skipped(createEnvelope()));
    ...
    it('treats unknown encryption statuses as non-blocking', async () => {
      const encryptionManager = createEncryptionManagerMock();
      encryptionManager.encryptEnvelope.mockResolvedValue({ status: 'UNKNOWN' as EncryptionStatus } as EncryptionResult);
    ...
    it('requires an origin type when queueing encryption work', async () => {
      ...
      await expect(
        (handler as any).handleEncryptionQueueing(createEnvelope(), createContext({ originType: undefined }), {})
      ).rejects.toThrow('Delivery context must include origin type for encryption queueing');
    });
  ...
  describe('handleSignedEnvelope (private)', () => {
    it('throws when context origin type is missing', async () => {
      const { handler } = createHandler();
      const envelope = createEnvelope({ sec: { sig: { kid: 'kid-1' } } as any });
      const context = createContext({ originType: undefined });

      await expect((handler as any).handleSignedEnvelope(envelope, context)).rejects.toThrow(
        'Context origin type must be provided'
      );
    });
    ...
    it('throws when signature header is missing', async () => {
      ...
      const context = createContext({ originType: DeliveryOriginType.UPSTREAM });

      await expect((handler as any).handleSignedEnvelope(envelope, context)).rejects.toThrow(
        'Signed envelope missing signature header'
      );
    });

    it('throws when verifier is not configured', async () => {
      const { handler } = createHandler({ envelopeVerifier: null });
      const envelope = createEnvelope({ sec: { sig: { kid: 'kid-2' } } as any });
      const context = createContext({ originType: DeliveryOriginType.UPSTREAM });

      await expect((handler as any).handleSignedEnvelope(envelope, context)).rejects.toThrow(
        'EnvelopeVerifier is not configured'
      );
    });
    ...
    it('throws when signature header is missing key identifier', async () => {
      ...
      await expect((handler as any).handleSignedEnvelope(envelope, context)).rejects.toThrow(
        'Signature header missing key identifier'
      );
    });
    ...
    it('throws when signature verification fails', async () => {
      ...
      const envelope = createEnvelope({ sec: { sig: { kid: 'kid-3' } } as any });
      const context = createContext({ originType: DeliveryOriginType.UPSTREAM });

      await expect((handler as any).handleSignedEnvelope(envelope, context)).rejects.toThrow(
        'Envelope signature verification failed for kid=kid-3'
      );
    });

    it('returns true when verification succeeds', async () => {
      const { handler } = createHandler();
      const envelope = createEnvelope({ sec: { sig: { kid: 'kid-4' } } as any });
      const context = createContext({
        originType: DeliveryOriginType.UPSTREAM,
        fromSystemId: 'remote-system',
      });

      await expect((handler as any).handleSignedEnvelope(envelope, context)).resolves.toBe(true);
    });
  });
  ...
  describe('handleSealedEncryption (private)', () => {
    it('returns true when destination is missing', async () => {
      ...
      const result = await (handler as any).handleSealedEncryption(createEnvelope(), createContext());
    ...
    it('wraps channel encryption options into request address', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.getEncryptionOptions.mockResolvedValue({ encryptionType: 'channel' });
      const { handler } = createHandler({ securityPolicy });
      ...
      try {
        await (handler as any).handleSealedEncryption(envelope, createContext());
    ...
    it('falls back to request address when option lookup fails', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.getEncryptionOptions.mockRejectedValue(new Error('lookup failed'));
      const { handler } = createHandler({ securityPolicy });
    ...
    it('uses provided options directly when not channel based', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.getEncryptionOptions.mockResolvedValue({ encryptionType: 'sealed', kid: 'abc' } as any);
      const { handler } = createHandler({ securityPolicy });
      const original = (handler as any).handleToBeEncryptedEnvelopeWithOptions;
      const spy = jest.fn().mockResolvedValue(true);
      (handler as any).handleToBeEncryptedEnvelopeWithOptions = spy;
  ...
  describe('handleChannelEncryption (private)', () => {
    it('returns true when destination is missing', async () => {
      ...
      const result = await (handler as any).handleChannelEncryption(createEnvelope(), createContext());
    ...
    it('delegates to handleToBeEncryptedEnvelopeWithOptions when destination exists', async () => {
      ...
      try {
        await (handler as any).handleChannelEncryption(envelope, createContext());
        expect(spy).toHaveBeenCalledWith(envelope, expect.any(Object), {
          encryptionType: 'channel',
  ...
  describe('handleToBeEncryptedEnvelope (private)', () => {
    ...
    it('passes resolved options to performEncryption', async () => {
      const securityPolicy = createSecurityPolicyMock();
      securityPolicy.getEncryptionOptions.mockResolvedValue({ requestAddress: 'dest' });
      const { handler } = createHandler({ securityPolicy });
      ...
      try {
        await (handler as any).handleToBeEncryptedEnvelope(createEnvelope(), createContext());
  ...
  describe('handleToBeEncryptedEnvelopeWithOptions (private)', () => {
    it('returns early when no encryption manager is present', async () => {
      ...
      const result = await (handler as any).handleToBeEncryptedEnvelopeWithOptions(
        createEnvelope(),
        createContext(),
        {}
      );
  ...
  describe('handleEncryptionQueueing variants (private)', () => {
    it('queues by recipKid option', async () => {
      ...
      await (handler as any).handleEncryptionQueueing(envelope, context, { recipKid: 'kid-1' });
    ...
    it('queues by recip_kid option', async () => {
      ...
      await (handler as any).handleEncryptionQueueing(envelope, context, { recip_kid: 'kid-2' });
    ...
    it('queues by recipientKeyId option', async () => {
      ...
      await (handler as any).handleEncryptionQueueing(envelope, context, { recipientKeyId: 'kid-3' });

...
import type { FameAddress, FameConnector, NodeWelcomeFrame } from "naylence-core";
...
class StubServiceManager implements ServiceManager {
  ...
  public readonly resolveByCapability = jest.fn(() => {
    throw new Error("resolveByCapability not implemented in StubServiceManager");
  });
...
describe("FameNode", () => {
  ...
  it("persists node metadata on startup", async () => {
    ...
    const store = await storageProvider.getKeyValueStore(NodeMetaRecord, NODE_META_NAMESPACE);
  ...
  it("collects callback grants from transport and event listeners without duplicates", () => {
    ...
    expect(grants.filter((grant) => grant.type === "transport")).toHaveLength(1);
  });
  ...
  it("waits for an ack when policy requires it", async () => {
    ...
    try {
      ...
      const ackFrame = await node.send(envelope, undefined, undefined, async (env) => {
  ...
  it("forwards upstream for control frames", async () => {
    ...
    const forwardSpy = jest.spyOn(node as any, "forwardUpstream").mockResolvedValue(undefined);
  ...
  it("routes capability-based envelopes via the service manager", async () => {
    ...
    expect(serviceManager.resolveAddressByCapabilityCalls).toEqual([["test-cap"]]);
  ...
  it("halts delivery when a listener returns null", async () => {
    ...
    const deliverSpy = jest.spyOn((node as any)._envelopeListenerManager, "deliverToAddress");
  ...
  it("delivers local envelopes through the channel polling queue", async () => {
    ...
    try {
      ...
      const sendSpy = jest.spyOn(binding.channel, "send").mockResolvedValue(undefined);
  ...
  it("throws when ack frame type is unexpected", async () => {
    ...
    const stubTracker = {
      ...
      awaitAck: jest
        .fn()
        .mockResolvedValue(
          node.envelopeFactory.createEnvelope({ frame: { type: "Data", payload: {} } })
        ),
    ...
    await expect(node.send(envelope, undefined, undefined, deliverFn)).rejects.toThrow(
      "Expected DeliveryAck frame in acknowledgement"
    );
  ...
  it("tracks upstream acknowledgement frames", async () => {
    ...
    expect(stubTracker.onEnvelopeDelivered).toHaveBeenCalledWith("__sys__", envelope, undefined);
  });
  ...
  it("throws when dispatchEnvelopeEvent lacks an envelope", async () => {
    ...
    await expect((node as any).dispatchEnvelopeEvent("onDeliver")).rejects.toThrow(
  ...
  it("delegates bind and unbind to the binding manager", async () => {
    ...
    const unbindSpy = jest.spyOn(manager, "unbind").mockResolvedValue(undefined);
  ...
  it("delegates envelope listener operations", async () => {
    ...
    const listenSpy = jest.spyOn(manager, "listen").mockResolvedValue(listenAddr);
    const listenRpcSpy = jest.spyOn(manager, "listenRpc").mockResolvedValue(listenAddr);
    ...
    for await (const value of node.invokeByCapabilityStream(["cap"], "method", {}, 25)) {
  ...
  it("dispatchEnvelopeEvent returns the processed envelope", async () => {
    ...
    const spy = jest.spyOn(node as any, "runEnvelopeListeners").mockResolvedValue(modified);

    const result = await (node as any).dispatchEnvelopeEvent("onDeliver", envelope);

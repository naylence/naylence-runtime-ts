...
export class EnvelopeSecurityHandler {
  ...
  public async handleOutboundSecurity(
    ...
    if (shouldSign) {
      ...
      this.envelopeSigner.signEnvelope(envelope, { physicalPath: this.node.physicalPath });
    }
  ...
  public async handleEnvelopeSecurity(
    ...
    if (context && this.securityPolicy) {
      const inboundCryptoLevel = this.securityPolicy.classifyMessageCryptoLevel(envelope, context);
      const existingLevel = context.security?.inboundCryptoLevel as CryptoLevel | undefined;
  ...
  public async shouldDecryptEnvelope(
    ...
    if (
      ...
      (await this.securityPolicy.shouldDecryptEnvelope(envelope, context, this.node))
    ) {
      return true;
    }

    if (envelope.sec?.enc) {
      const cryptoLevel = this.securityPolicy.classifyMessageCryptoLevel(envelope, context);
  ...
  private async handleEncryptionQueueing(
    ...
    if (options.requestAddress) {
      ...
      this.keyManagementHandler.queuePendingEncryptionEnvelope(addressKey, envelope, context);
  ...
  private isSigned(envelope: FameEnvelope, context?: FameDeliveryContext): boolean {

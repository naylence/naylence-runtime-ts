...
afterEach(async () => {
  await Promise.all(
    activeHandlers.map((handler) =>
      handler.shutdownTasks({ gracePeriod: 0, cancelHanging: true, joinTimeout: 0 })
    )
  );
...
function createMockNode(
  ...
  const envelopeFactory = {
    createEnvelope: jest.fn((options: CreateFameEnvelopeOptions) => ({
...
function createHandler(deps: HandlerDeps = {}) {
  ...
  const encryptionManager: MockEncryptionManager | null =
    deps.encryptionManager === null
      ...
      : ({ notifyKeyAvailable: jest.fn(async () => undefined) } as unknown as MockEncryptionManager);
...
function makeKeyAnnounceEnvelope(overrides: {
...
describe('KeyManagementHandler.acceptKeyAnnounce', () => {
  it('ignores envelopes that are not key announcements', async () => {
    ...
    const envelope = makeEnvelope({ type: 'Data' } as unknown as FameEnvelope['frame'], 'env');
  ...
  it('skips key announces when no key manager is configured', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('skipping_key_announce_no_key_manager', {
      envelope_id: 'env',
    });
  });
  ...
  it('throws when delivery context is missing or lacks origin type', async () => {
    ...
    ).rejects.toThrow('Delivery context must include originType for KeyAnnounce');
  });
  ...
  it('adds validated keys, handles correlations, and replays pending envelopes', async () => {
    ...
    expect(manager.notifyKeyAvailable).toHaveBeenCalledWith(
  ...
  it('replays queued encryption envelopes when a new key arrives', async () => {
    ...
    const kid = 'enc-1';
    const envelope = makeEnvelope({ type: 'Data' } as FameEnvelope['frame'], 'pending-env');
    const context = {
      originType: DeliveryOriginType.UPSTREAM,
    ...
    expect(
      (handler as unknown as { pendingEncryptionEnvelopes: Map<string, unknown> }).pendingEncryptionEnvelopes.has(
        kid
      )
    ).toBe(false);
  });
...
describe('KeyManagementHandler retry logic', () => {
  it('kicks off retries for queued envelopes after attachment', async () => {
    const { handler } = createHandler();
    const spy = jest.spyOn(handler, 'maybeRequestSigningKey').mockResolvedValue(undefined);
    ...
    expect(spy).toHaveBeenCalledWith('kid-queue', DeliveryOriginType.UPSTREAM, 'origin');
  });
  ...
  it('uses pending-attachment default when system id is absent', async () => {
    ...
    expect(spy).toHaveBeenCalledWith('kid-no-system', DeliveryOriginType.UPSTREAM, 'pending-attachment');
  });
});
...
describe('KeyManagementHandler signing key requests', () => {
  it('handles physical path lookup failures gracefully', async () => {
    ...
    await handler.maybeRequestSigningKey('kid', DeliveryOriginType.UPSTREAM, 'origin');
  ...
  it('forwards signing key requests upstream', async () => {
    ...
    await handler.maybeRequestSigningKey('kid-up', DeliveryOriginType.UPSTREAM, 'origin');
    await flushAll();
  ...
  it('requires routing support when forwarding to peers', async () => {
    ...
    await expect(
      handler.maybeRequestSigningKey('kid-peer', DeliveryOriginType.PEER, 'peer-1')
    ).rejects.toThrow('Key requests to peers are only supported on routing nodes');
  });
  ...
  it('forwards signing key requests to peers when available', async () => {
    ...
    await handler.maybeRequestSigningKey('kid-peer', DeliveryOriginType.PEER, 'peer-1');
    await flushAll();

    expect(forwardToPeer).toHaveBeenCalledWith(
      'peer-1',
      expect.objectContaining({ frame: expect.objectContaining({ kid: 'kid-peer' }) }),
      expect.any(Object)
    );
  });
  ...
  it('avoids duplicate signing key requests for the same kid', async () => {
    ...
    await handler.maybeRequestSigningKey('kid-dup', DeliveryOriginType.UPSTREAM, 'origin');
    await handler.maybeRequestSigningKey('kid-dup', DeliveryOriginType.UPSTREAM, 'origin');
    await flushAll();
...
describe('KeyManagementHandler encryption key requests', () => {
  it('enforces local origin when requesting encryption keys', async () => {
    ...
    ).rejects.toThrow('Encryption key requests are only supported for local origin');
  });

  it('requests encryption keys by id and by address', async () => {
    const { handler, node } = createHandler();

    await handler.maybeRequestEncryptionKey('kid-enc', DeliveryOriginType.LOCAL, 'local');
    ...
    expect((handler as any).pendingEncryptionKeyRequests.size).toBeGreaterThan(0);
  });

  it('avoids duplicate encryption key requests for the same kid', async () => {
    const { handler, node } = createHandler();

    await handler.maybeRequestEncryptionKey('kid-enc-dup', DeliveryOriginType.LOCAL, 'local');
    await handler.maybeRequestEncryptionKey('kid-enc-dup', DeliveryOriginType.LOCAL, 'local');
    await flushAll();
  ...
  it('avoids duplicate encryption key requests by address', async () => {
    ...
    await handler.maybeRequestEncryptionKeyByAddress(address, DeliveryOriginType.LOCAL, 'local');
    await handler.maybeRequestEncryptionKeyByAddress(address, DeliveryOriginType.LOCAL, 'local');
    await flushAll();
...
describe('KeyManagementHandler deferred handling', () => {
  it('ignores redundant resolve and reject calls on pending requests', async () => {
    ...
    await handler.maybeRequestSigningKey('kid-resolve', DeliveryOriginType.UPSTREAM, 'origin');
    await flushAll();

    const pendingRequests = (handler as any).pendingKeyRequests as Map<string, any>;
    ...
    await handler.maybeRequestSigningKey('kid-reject', DeliveryOriginType.UPSTREAM, 'origin');
  ...
  it('uses Date.now when performance.now is unavailable', async () => {
    ...
    const originalPerformance = globalObj.performance as Performance | undefined;
    delete globalObj.performance;

    try {
      await handler.maybeRequestSigningKey('kid-monotonic', DeliveryOriginType.UPSTREAM, 'origin');
    } finally {
...
describe('KeyManagementHandler sweepKeyRequests', () => {
  it('retries expired requests before eventually failing and cleaning up', async () => {
    ...
    await handler.maybeRequestSigningKey('kid-expire', DeliveryOriginType.UPSTREAM, 'origin');
    await flushAll();
  ...
  pendingMap.set('kid-expire', [{ envelope: pendingEnvelope, context: {} as FameDeliveryContext }]);
    ...
    await expect(request.deferred.promise).rejects.toThrow('signing key fetch failed');
...
describe('KeyManagementHandler.gcKeyRequests', () => {
  it('logs and cleans up pending requests when retries exhaust', async () => {
    ...
    const signingEnvelope = makeEnvelope({ type: 'Data' } as FameEnvelope['frame'], 'sign-env');
    const encryptionEnvelope = makeEnvelope({ type: 'Data' } as FameEnvelope['frame'], 'enc-env');
    ...
    const encPendingsMap = (handler as any).pendingEncryptionEnvelopes as Map<string, any>;
    encPendingsMap.set('encryption-kid', [
      { envelope: encryptionEnvelope, context: {} as FameDeliveryContext },
    ]);

    const correlationMap = (handler as any).correlationToAddress as Map<string, string>;
...
describe('KeyManagementHandler.buildKeyRequestEnvelopeOptions', () => {
  it('includes the current trace id when available', () => {
    ...
    const options = (handler as any).buildKeyRequestEnvelopeOptions(frame, 'corr-123');
...
describe('KeyManagementHandler.getEncryptionKeyNotifier', () => {
  it('returns null when notifyKeyAvailable is missing', () => {
    const { handler } = createHandler({ encryptionManager: {} as EncryptionManager });

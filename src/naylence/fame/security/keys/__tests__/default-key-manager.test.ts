import { DeliveryOriginType, type EnvelopeFactory, type FameEnvelope } from "naylence-core";
...
function createValidEncryptionKey(kid: string, physicalPath: string): KeyRecord {
  return {
    kid,
    kty: "OKP",
    crv: "X25519",
...
function createMockKeyStore() {
  const mocks = {
    ...
    getKeysGroupedByPath: jest.fn(async () => ({}) as Record<string, KeyRecord[]>),
...
describe("DefaultKeyManager", () => {
  ...
  it("throws when announcing without envelope factory", async () => {
    ...
    Object.defineProperty(helper.node as unknown as Record<string, unknown>, "envelopeFactory", {
      value: null,
    });
  ...
  it("announces downstream keys upstream and to peers when sid matches", async () => {
    const { store, mocks } = createMockKeyStore();
    const { node, envelopeFactory, forwardUpstream, forwardToPeers } = createMockNode();
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(node);
  ...
  it("throws when announcing downstream keys without node id for upstream forwarding", async () => {
    const { store, mocks } = createMockKeyStore();
    const helper = createMockNode();
    Object.defineProperty(helper.node as unknown as Record<string, unknown>, "id", {
      value: "",
    });
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(helper.node);
  ...
  it("stores downstream path when physical path empty at root", async () => {
    const { store, mocks } = createMockKeyStore();
    const helper = createMockNode({ physicalPath: "/", hasParent: false, includeRouting: false });
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(helper.node);
  ...
  it("handles key request using fallback path and routes downstream", async () => {
    ...
    const { node, envelopeFactory, forwardToRoute, forwardUpstream } = createMockNode();
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(node);
  ...
  it("returns early when announcing keys without upstream", async () => {
    const { store, mocks } = createMockKeyStore();
    const { node } = createMockNode({ hasParent: false, includeRouting: false });
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(node);
  ...
  it("announces stored keys upstream on demand", async () => {
    const { store, mocks } = createMockKeyStore();
    const { node, forwardUpstream, forwardToPeers, envelopeFactory } = createMockNode();
    const manager = new DefaultKeyManager({ keyStore: store });
    await manager.onNodeStarted(node);

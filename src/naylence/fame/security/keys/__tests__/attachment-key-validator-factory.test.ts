...
class TestAttachmentKeyValidator extends AttachmentKeyValidator {
  public readonly validateKeySpy = jest.fn<Promise<KeyInfo>, [AttachmentKey]>(
    async (key) => {
  ...
  public async validateChildAttachmentLogicals(
    ...
    return await this.validateChildAttachmentLogicalsSpy(
      childKeys,
      authorizedLogicals,
      childId
    );
...
describe('AttachmentKeyValidator', () => {
  ...
  it('throws when key info receives invalid date strings', () => {
    expect(() => new KeyInfo({ expiresAt: 'not-a-date' })).toThrow(
  ...
  it('creates key validation error with metadata copies', () => {
    ...
    const error = new KeyValidationError('expired', 'Key expired', {
      kid: 'kid-42',
      details,
    });
...
describe('AttachmentKeyValidatorFactory', () => {
  it('creates default validator when config omitted', async () => {
    const validator =
      await AttachmentKeyValidatorFactory.createAttachmentKeyValidator();
    expect(validator).toBeInstanceOf(TestAttachmentKeyValidator);
  });

  it('creates validator when config provided', async () => {
    const config: AttachmentKeyValidatorConfig = {
      type: 'TestAttachmentKeyValidator',
    };

    const validator =
      await AttachmentKeyValidatorFactory.createAttachmentKeyValidator(config);
    expect(validator).toBeInstanceOf(TestAttachmentKeyValidator);
  });
});

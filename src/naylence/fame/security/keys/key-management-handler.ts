...
export class KeyManagementHandler extends TaskSpawner {
  ...
  public readonly pendingEncryptionEnvelopes = new Map<string, PendingEnvelope[]>();
  private readonly pendingEncryptionKeyRequests = new Map<string, PendingRequest>();
  ...
  public async stop(): Promise<void> {
    ...
    await delay(100);
    await this.shutdownTasks({ gracePeriod: 10, cancelHanging: true, joinTimeout: 1_000 });
  }
  ...
  public async acceptKeyAnnounce(
    ...
    if (!context.originType) {
      throw new Error('Delivery context must include originType for KeyAnnounce');
    }
  ...
  const toRetry: Array<{ kid: string; origin: DeliveryOriginType; fromSystemId: string }> = [];

    for (const [kid, pending] of this.pendingEnvelopes.entries()) {
      if (!pending || pending.length === 0 || this.pendingKeyRequests.has(kid)) {
        continue;
      }
    ...
    for (const entry of toRetry) {
      await this.maybeRequestSigningKey(entry.kid, entry.origin, entry.fromSystemId);
    }
  }
  ...
  public async maybeRequestSigningKey(
    ...
    if (origin === DeliveryOriginType.UPSTREAM) {
      this.spawn(async () => {
        await this.node.forwardUpstream(envelope, localDeliveryContext(this.node.id));
      }, { name: `send-keyreq-upstream-${kid}` });
    } else if (origin === DeliveryOriginType.PEER) {
      const routingNode = this.node as RoutingNodeLike & {
        forwardToPeer?: (segment: string, env: FameEnvelope, context?: FameDeliveryContext) => Promise<void>;
      };

      const forwardToPeer = routingNode.forwardToPeer?.bind(routingNode);
      if (!forwardToPeer) {
        throw new Error('Key requests to peers are only supported on routing nodes');
      }

      this.spawn(async () => {
        await forwardToPeer(fromSystemId, envelope, localDeliveryContext(this.node.id));
  ...
  public async maybeRequestEncryptionKey(
    ...
    if (origin !== DeliveryOriginType.LOCAL) {
      throw new Error('Encryption key requests are only supported for local origin');
    }
  ...
  public async maybeRequestEncryptionKeyByAddress(
    ...
    if (this.pendingEncryptionKeyRequests.has(addressKey) || !this.node.hasParent) {
      return;
    }
  ...
  private onNewKeyForAddress(address: FameAddress, keys: Array<Record<string, unknown>>): void {
    const addressKey = String(address);
    ...
    try {
      while (this.isStarted && !signal?.aborted) {
        ...
        await this.sweepKeyRequests({
          ...
          now,
          requestType: 'signing',
          onRetry: (kid, request) => this.maybeRequestSigningKey(kid, request.origin, request.fromSystemId),
          onFailure: (kid) => {
        ...
        await this.sweepKeyRequests({
          ...
          onRetry: (kid, request) => this.maybeRequestEncryptionKey(kid, request.origin, request.fromSystemId),
          onFailure: (kid) => {
            ...
            for (const [corrId, addrKey] of Array.from(this.correlationToAddress.entries())) {
              if (addrKey === kid) {
  ...
  private async registerOwnPublicKeys(): Promise<void> {
    ...
    if (jwks?.keys) {
      for (const jwk of jwks.keys) {
        if (nodeJwk && getKeyId(jwk) === getKeyId(nodeJwk) && jwk.use !== 'enc') {
          continue;
        }
  ...
  private getEncryptionKeyNotifier(): ((kid: string) => Promise<void> | void) | null {

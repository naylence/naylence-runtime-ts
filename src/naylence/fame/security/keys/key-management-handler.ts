...
export class KeyManagementHandler extends TaskSpawner {
    ...
    try {
      while (this.isStarted && !signal?.aborted) {
        ...
        await this.sweepKeyRequests({
          ...
          requestType: "signing",
          onRetry: (kid, request) =>
            this.maybeRequestSigningKey(kid, request.origin, request.fromSystemId),
          onFailure: (kid) => {
        ...
        await this.sweepKeyRequests({
          ...
          onRetry: (kid, request) =>
            this.maybeRequestEncryptionKey(kid, request.origin, request.fromSystemId),
          onFailure: (kid) => {
  ...
  private async sweepKeyRequests(options: {
    ...
    const { requestMap, pendingMap, now, requestType, onRetry, onFailure } = options;

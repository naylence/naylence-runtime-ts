...
function ensureEnvelopeFactory(
  factory: EnvelopeFactory | null
): EnvelopeFactory {
  if (!factory) {
    throw new Error(
      'Envelope factory not available - key manager not properly initialized'
    );
  }
  return factory;
}
...
export class DefaultKeyManager implements KeyManager {
  ...
  public async addKeys(options: {
    ...
    if (sid && !skipSidValidation) {
      ...
      if (!keyPath) {
        throw new Error(
          'Unable to determine expected SID for key announcement'
        );
      }
  ...
  public async handleKeyRequest(options: {
    ...
    const envelopeOptions: { frame: KeyAnnounceFrame; corrId?: string } = {
      frame,
    };
    if (correlationId) {
    ...
    if (!nodeId) {
      throw new Error(
        'Node ID not available - key manager not properly initialized'
      );
    }

...
function normalizeConfig(
  ...
  const issuer = typeof source.issuer === 'string' && source.issuer.trim().length > 0 ? source.issuer.trim() : undefined;
  if (!issuer) {
  ...
  let jwksUrl = typeof jwksUrlRaw === 'string' && jwksUrlRaw.trim().length > 0 ? jwksUrlRaw.trim() : undefined;
  if (!jwksUrl) {
    const trimmedIssuer = issuer.replace(/\/+$/, '');
    jwksUrl = `${trimmedIssuer}/.well-known/jwks.json`;
  }

  const algorithm = typeof source.algorithm === 'string' && source.algorithm.trim().length > 0
  ...
  const requireScope = typeof source.requireScope === 'boolean'
    ? source.requireScope
    : typeof source.require_scope === 'boolean'
      ? source.require_scope
      : true;

  const defaultTtlCandidate = typeof source.defaultTtlSec === 'number'
  ...
  const defaultTtlSec = typeof validatedDefaultTtl === 'number' ? validatedDefaultTtl : defaultTtlCandidate;

  const maxTtlCandidate = typeof source.maxTtlSec === 'number'
    ? source.maxTtlSec
  ...
  const reverseAuthCandidate = typeof source.reverseAuthTtlSec === 'number'

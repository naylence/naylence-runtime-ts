...
export class JWTTokenIssuerFactory extends TokenIssuerFactory<JWTTokenIssuerConfig> {
  ...
  const cryptoProvider1 = cryptoProvider ?? null;
    ...
    if (isHmac) {
      ...
      if (!signingKey) {
        throw new Error(`HMAC algorithm ${algorithm} requires explicit 'hmacSecret' configuration`);
      }
    } else {
      signingKey =
...
function normalizeConfig(config: JWTTokenIssuerConfig | Record<string, unknown>): NormalizedJWTTokenIssuerConfig {
  const source = config as JWTTokenIssuerConfig & Record<string, unknown>;

  const issuer = typeof source.issuer === 'string' && source.issuer.trim() !== '' ? source.issuer : undefined;
  if (!issuer) {
    throw new Error('JWTTokenIssuer configuration requires "issuer"');
  }

  const algorithm = (typeof source.algorithm === 'string' && source.algorithm.trim() !== '')
    ? source.algorithm
    : 'EdDSA';
  ...
  const kid = typeof source.kid === 'string' && source.kid.trim() !== '' ? source.kid : undefined;
  const audience = typeof source.audience === 'string' && source.audience.trim() !== '' ? source.audience : undefined;
...
async function resolveSecret(source: SecretSource): Promise<string | undefined> {
  ...
  switch (source.type) {
    ...
    case 'SecretStoreCredentialProvider': {
      ...
      throw new Error(`Secret store credential provider for '${secretName ?? 'unknown'}' is not yet implemented`);
    }
...
function readEnvironmentVariable(varName: string): string {
  if (typeof process === 'undefined' || !process.env) {
    throw new Error(`Environment variables are not accessible in this runtime; cannot read ${varName}`);
  }

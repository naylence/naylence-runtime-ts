...
export class JWKSJWTTokenVerifier implements TokenVerifier {
  ...
  constructor(options: JWKSJWTTokenVerifierOptions) {
    ...
    this.cacheTtlMs = Math.max(1_000, (options.cacheTtlSec ?? DEFAULT_JWKS_CACHE_TTL_SEC) * 1_000);
    this.algorithms = (
  ...
  public async verify(
    ...
    try {
      ...
      const { payload, protectedHeader } = await jose.jwtVerify(token, remoteJwkSet, verifyOptions);

...
export class OAuth2ClientCredentialsTokenProvider implements TokenProvider {
  ...
  constructor(private readonly options: OAuth2ClientCredentialsTokenProviderOptions) {
    if (!options.tokenUrl) {
  ...
  public async getToken(): Promise<Token> {
    if (this.cachedToken && this.isTokenFresh(this.cachedToken)) {
      logger.debug("using_cached_oauth2_token", { token_url: this.options.tokenUrl });
  ...
  private isTokenFresh(token: Token): boolean {
    ...
    const clockSkew = this.options.clockSkewSeconds ?? DEFAULT_CLOCK_SKEW_SECONDS;
  ...
  private async fetchNewToken(): Promise<Token> {
    ...
    const clientId = credentialToString(await this.options.clientIdProvider.get());
    if (!clientId) {
      throw new Error("OAuth2 client ID provider returned empty value");
    }

    const clientSecret = credentialToString(await this.options.clientSecretProvider.get());
    if (!clientSecret) {
  ...
  private resolveExpiresIn(payload: OAuth2TokenResponse): number {
    ...
    if (typeof expiresInCandidate === "number" && Number.isFinite(expiresInCandidate)) {

...
describe('sealed envelope crypto helpers', () => {
	...
	test('decryption fails with wrong private key', () => {
		...
		expect(() => sealedDecrypt(sealed, attacker.privateKey)).toThrow('Failed to decrypt sealed envelope');
	});
	...
	test('encryption output structure matches expectations', () => {
		...
		const nonce = sealed.subarray(SEALED_ENVELOPE_PUBLIC_KEY_LENGTH, SEALED_ENVELOPE_PUBLIC_KEY_LENGTH + SEALED_ENVELOPE_NONCE_LENGTH);
		const ciphertext = sealed.subarray(SEALED_ENVELOPE_PUBLIC_KEY_LENGTH + SEALED_ENVELOPE_NONCE_LENGTH);

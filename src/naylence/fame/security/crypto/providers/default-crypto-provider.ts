...
export class DefaultCryptoProvider implements CryptoProvider {
  ...
  public async createCsr(
    ...
    const trimmedPhysicalPath = assertNonEmptyString(
      physicalPath,
      'physicalPath'
    );
    ...
    try {
      if (this.artifacts.signing.algorithm !== 'EdDSA') {
        throw new Error(
          'CSR creation only supported for Ed25519 signing keys in the default crypto provider'
        );
      }
      ...
      const privateKey = await cryptoImpl.subtle.importKey(
        'pkcs8',
        pemToArrayBuffer(this.signingPrivatePem),
        {
          name: 'Ed25519',
        },
        false,
      ...
      const subjectPkInfo = AsnConvert.parse(
        publicKeyDer,
        SubjectPublicKeyInfo
      );

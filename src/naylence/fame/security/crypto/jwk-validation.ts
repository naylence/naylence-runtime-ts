...
export function validateJwkUseField(jwk: JsonWebKey): string {
  ...
  if (!use) {
    throw new JWKValidationError(`JWK missing required 'use' field: ${String(jwk?.kid ?? 'unknown')}`);
  }

  if (typeof use !== 'string') {
    throw new JWKValidationError(`JWK 'use' field must be a string: ${String(jwk?.kid ?? 'unknown')}`);
  }
...
export function validateJwkStructure(jwk: JsonWebKey): void {
  ...
  if (!kid || typeof kid !== 'string') {
    throw new JWKValidationError("JWK missing required 'kid' field or kid is not a string");
  }

  const kty = jwk.kty;
  if (!kty || typeof kty !== 'string') {
    throw new JWKValidationError(`JWK ${kid} missing required 'kty' field or kty is not a string`);
  }

  const requiredFields = REQUIRED_FIELDS_BY_KTY[kty];
  if (!requiredFields) {
    throw new JWKValidationError(`JWK ${kid} has unsupported key type '${kty}'`);
  }

  const missingFields = Array.from(requiredFields).filter((field) => !(field in jwk));
  ...
  if ((kty === 'EC' || kty === 'OKP')) {
    ...
    if (!crv || typeof crv !== 'string') {
      throw new JWKValidationError(`JWK ${kid} missing required 'crv' field or crv is not a string`);
    }
...
export function validateJwkComplete(jwk: JsonWebKey): string {
  ...
  } else if (kty === 'RSA' || (kty === 'EC' && (crv === 'P-256' || crv === 'P-384' || crv === 'P-521'))) {
    if (use !== 'sig') {
      throw new JWKValidationError(
...
export function filterKeysByUse(keys: JsonWebKey[], use: string): JsonWebKey[] {
  if (!VALID_KEY_USES.has(use)) {
    throw new Error(`Invalid use value '${use}'. Valid values: ${Array.from(VALID_KEY_USES).join(', ')}`);
  }
...
export function validateEncryptionKey(jwk: JsonWebKey): void {
  ...
  if (use !== 'enc') {
    throw new JWKValidationError(`JWK ${String(jwk.kid ?? 'unknown')} is not an encryption key (use=${use})`);
  }
...
export function validateSigningKey(jwk: JsonWebKey): void {
  const use = validateJwkComplete(jwk);
  if (use !== 'sig') {
    throw new JWKValidationError(`JWK ${String(jwk.kid ?? 'unknown')} is not a signing key (use=${use})`);
  }

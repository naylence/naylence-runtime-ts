...
import { canonicalJson, decodeBase64Url, frameDigest, immutableHeaders } from './eddsa-signer-verifier.js';
import { encodeUtf8 } from './eddsa-utils.js';
import { validateSigningKey, JWKValidationError } from '../crypto/jwk-validation.js';
...
function loadPublicKey(jwk: VerifierJwk, signingConfig: SigningConfig): Uint8Array {
  if (jwk.x5c) {
  ...
  const candidate = typeof jwk.x === 'string' ? jwk.x : typeof jwk.crv_x === 'string' ? jwk.crv_x : jwk.pub;
...
export class EdDSAEnvelopeVerifier {
  ...
  public constructor(keyProvider: KeyProvider, options: EdDSAEnvelopeVerifierOptions = {}) {
  ...
  public async verifyEnvelope(
    ...
    const tbs = new Uint8Array(
      encodeUtf8(sid).length + 1 + encodeUtf8(immutable).length + 1 + encodeUtf8(trustedDigest).length
    );

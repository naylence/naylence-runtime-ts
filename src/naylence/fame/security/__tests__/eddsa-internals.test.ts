...
const PRIVATE_KEY_BYTES = Uint8Array.from(
  Buffer.from(
    '8f6c9a4b2d5e7f0182736455aa99bbccddee00112233445566778899aabbccdd',
    'hex'
  )
);
...
describe('eddsa utility helpers', () => {
  ...
  it('rejects malformed Ed25519 private key structures', () => {
    const badPem =
      '-----BEGIN PRIVATE KEY-----\nAAAA\n-----END PRIVATE KEY-----';
    expect(() => parseEd25519PrivateKey(badPem)).toThrow(
      'Unexpected ASN.1 tag'
    );
  });
  ...
  it('rejects ASN.1 structures that end before declaring a length', () => {
    const shortPem =
      '-----BEGIN PRIVATE KEY-----\nMA==\n-----END PRIVATE KEY-----';
    expect(() => parseEd25519PrivateKey(shortPem)).toThrow(
  ...
  it('rejects ASN.1 length encodings with zero length-of-length octets', () => {
    ...
    const pem =
      ...
      '\n-----END PRIVATE KEY-----';
    expect(() => parseEd25519PrivateKey(pem)).toThrow(
  ...
  it('encodes UTF-8 with and without TextEncoder support', async () => {
    ...
    jest.resetModules();
    const { encodeUtf8: fallbackEncodeUtf8 } = await import(
      '../signing/eddsa-utils.js'
    );
    const fallback = fallbackEncodeUtf8('fallback');
    expect(Array.from(fallback)).toEqual(
      Array.from(Buffer.from('fallback', 'utf8'))
    );
    jest.resetModules();
  ...
  it('throws when PEM decoding support is unavailable', () => {
    ...
    expect(() =>
      decodePem('-----BEGIN KEY-----\nAA==\n-----END KEY-----')
    ).toThrow('Base64 decoding is not available in this environment');
...
describe('EdDSA envelope signer and verifier integration', () => {
  ...
  it('throws when signature header fields are malformed', async () => {
    ...
    const signed = signer.signEnvelope(buildEnvelope(), {
      physicalPath: '/malformed',
    });
  ...
  it('throws when key provider returns null for unknown key id', async () => {
    ...
    const signer = new EdDSAEnvelopeSigner({ cryptoProvider });
    const envelope = signer.signEnvelope(buildEnvelope(), {
      physicalPath: '/unknown-key',
    });
  ...
  it('throws when signature verification fails despite correct length', async () => {
    ...
    const signer = new EdDSAEnvelopeSigner({ cryptoProvider });
    const signed = signer.signEnvelope(buildEnvelope(), {
      physicalPath: '/invalid-signature',
    });

    const verifier = new EdDSAEnvelopeVerifier(
      buildKeyProvider('/invalid-signature')
    );
  ...
  it('supports explicit key overrides and certificate-based kid resolution', () => {
    ...
    const signer = new EdDSAEnvelopeSigner({
      ...
      signingConfig: new SigningConfig({
    ...
    const fallbackSigned = fallbackSigner.signEnvelope(buildEnvelope(), {
      physicalPath: 'y',
    });
  ...
  it('throws when crypto provider or key identifiers are missing', () => {
    expect(() => new EdDSAEnvelopeSigner()).toThrow(
      'No crypto provider is configured for signing'
    );
    ...
    const signer = new EdDSAEnvelopeSigner({
      cryptoProvider: incompleteProvider,
    });
    expect(() =>
      signer.signEnvelope(buildEnvelope(), { physicalPath: 'z' })
    ).toThrow('Crypto provider does not expose a signature key id');
  });
  ...
  it('throws when envelope is missing sid', () => {
    ...
    expect(() =>
      signer.signEnvelope(envelope, { physicalPath: '/sidless' })
    ).toThrow('Envelope missing sid');
  });
  ...
  it('preserves existing payload digest values on data frames', () => {
    ...
    const envelope = buildEnvelope();
    const dataFrame = envelope.frame as {
      type: string;
      payload?: unknown;
      pd?: string | null;
    };
    ...
    const signed = signer.signEnvelope(envelope, {
      physicalPath: '/existing-pd',
    });
    expect((signed.frame as { pd?: string | null }).pd).toBe(
      'precomputed-digest'
    );
  });
  ...
  it('throws when crypto provider omits the signing private key', () => {
    ...
    expect(() =>
      signer.signEnvelope(buildEnvelope(), { physicalPath: '/no-key' })
    ).toThrow('Crypto provider does not expose a signing private key');
  });
  ...
  it('validates payload digests and logical checking branches', async () => {
    ...
    const verifier = new EdDSAEnvelopeVerifier(buildKeyProvider(physicalPath));
    await expect(
      verifier.verifyEnvelope(signed, { checkPayload: true })
    ).resolves.toBe(true);
    ...
    await expect(
      verifier.verifyEnvelope(relaxedEnvelope, { checkPayload: false })
    ).resolves.toBe(true);
  });
...
describe('EdDSA factories', () => {
  ...
  it('allows options signingConfig to override argument defaults', async () => {
    ...
    const signer = new EdDSAEnvelopeSigner({ cryptoProvider });
    const envelope = signer.signEnvelope(buildEnvelope(), {
      physicalPath: '/factory-cert-option',
    });

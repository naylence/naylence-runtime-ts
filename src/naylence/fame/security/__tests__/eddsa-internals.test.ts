...
describe('eddsa utility helpers', () => {
  ...
  it('ignores empty and non-string property candidates', () => {
    const result = readStringProperty({ empty: '', number: 42, text: 'ok' }, 'empty', 'number', 'text');
  ...
  it('rejects unsupported PKCS#8 Ed25519 layouts', () => {
    const invalidStructure = new Uint8Array([
      0x30, 0x2e, 0x02, 0x01, 0x00, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x04, 0x22, 0x05, 0x20,
    ...
    expect(() => parseEd25519PrivateKey(invalidPem)).toThrow('Unsupported Ed25519 private key structure');
  });
  ...
  it('rejects PKCS#8 Ed25519 payloads with unexpected lengths', () => {
    ...
    expect(() => parseEd25519PrivateKey(invalidPem)).toThrow('Unexpected Ed25519 private key length');
  });
  ...
  it('throws when no base64url decoder is available', () => {
    ...
    expect(() => decodeBase64Url(base64)).toThrow('No base64 decoder available in this environment');
  ...
  it('throws when UTF-8 encoding support is unavailable', async () => {
    ...
    jest.resetModules();
    const { encodeUtf8: fallbackEncodeUtf8 } = await import('../signing/eddsa-utils.js');
    expect(() => fallbackEncodeUtf8('utf8?')).toThrow('No UTF-8 encoder available in this environment');
    jest.resetModules();
...
describe('EdDSA envelope signer and verifier integration', () => {
  ...
  it('throws when envelope lacks a signature header', async () => {
    ...
    await expect(verifier.verifyEnvelope({ ...buildEnvelope(), sec: {} } as FameEnvelope)).rejects.toThrow(
      'Missing envelope.sec.sig header'
    );
  });
  ...
  it('throws when signature header fields are malformed', async () => {
    ...
    await expect(verifier.verifyEnvelope(missingKid)).rejects.toThrow("Signature header missing 'kid'");
  
    const missingVal = {
      ...signed,
      sec: { sig: { ...signed.sec?.sig, val: '' } },
    } as FameEnvelope;
    await expect(verifier.verifyEnvelope(missingVal)).rejects.toThrow("Signature header missing 'val'");
  });
  ...
  it('throws when DataFrame payload digest is missing', async () => {
    ...
    await expect(verifier.verifyEnvelope(envelope)).rejects.toThrow('DataFrame missing payload digest (pd field)');
  });
  ...
  it('throws when payload digest mismatches after tampering', async () => {
    ...
    await expect(verifier.verifyEnvelope(envelope)).rejects.toThrow('Payload digest mismatch in DataFrame');
  });
  ...
  it('throws when signature verification fails despite correct length', async () => {
    ...
    await expect(verifier.verifyEnvelope(signed)).rejects.toThrow('Envelope signature verification failed');
  ...
  it('throws when JWK lacks usable public key material', async () => {
    ...
    const verifier = new EdDSAEnvelopeVerifier(provider);
    await expect(verifier.verifyEnvelope(signed)).rejects.toThrow('JWK missing public key material');
  });
  ...
  it('throws when envelope is missing sid', () => {
    ...
    expect(() => signer.signEnvelope(envelope, { physicalPath: '/sidless' })).toThrow('Envelope missing sid');
  });
  ...
  it('validates payload digests and logical checking branches', async () => {
    ...
    await expect(verifier.verifyEnvelope(relaxedEnvelope, { checkPayload: false })).resolves.toBe(true);
  });
...
describe('EdDSA factories', () => {
  it('creates signer instances and respects override options', async () => {
    ...
    const signed = (signer as EdDSAEnvelopeSigner).signEnvelope(envelope, { physicalPath: '/factory' });
  ...
  it('uses the crypto provider supplied via options', async () => {
    ...
    const envelope = buildEnvelope();
    const signed = (signer as EdDSAEnvelopeSigner).signEnvelope(envelope, { physicalPath: '/factory-option' });
  ...
  it('requires a key provider for verifier factory', async () => {
    ...
    await expect(factory.create()).rejects.toThrow('EdDSAEnvelopeVerifierFactory requires a key provider');

...
import { createAuthorizationContext, type AuthorizationContext } from 'naylence-core';
...
class StubKeyManager implements KeyManager {
	...
	public getKey = jest.fn(async (): Promise<KeyRecord> => ({ kid: 'stub' } as KeyRecord));
...
class ListenerAuthorizer implements Authorizer, NodeEventListener {
	...
	private createContext(): AuthorizationContext {
		return createAuthorizationContext({ authenticated: true, authorized: true, principal: 'listener' });
	}
}
...
describe('DefaultSecurityManagerFactory', () => {
	...
	it('reuses a provided key manager override', async () => {
		...
		const manager = await SecurityManagerFactory.createSecurityManager({ keyManager });
	...
	it('adds provided authorizer to event listeners when applicable', async () => {
		...
		const manager = await factory.create(null, { authorizer, eventListeners: listeners });

...
describe('DefaultSecurityPolicy alias normalization', () => {
  it('accepts snake_case constructor options', async () => {
    ...
    expect(internals.signing.inbound.signaturePolicy).toBe(
    ...
    expect(customSigningPolicy).toHaveBeenCalledWith(
      envelope,
      undefined,
      undefined
    );
    ...
    await expect(policy.shouldEncryptEnvelope(envelope, context)).resolves.toBe(
      false
    );

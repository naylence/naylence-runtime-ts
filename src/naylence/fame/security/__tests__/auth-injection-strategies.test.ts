...
import { TOKEN_PROVIDER_FACTORY_BASE_TYPE, TokenProviderFactory } from '../auth/token-provider-factory.js';
...
describe('auth injection strategies', () => {
  ...
  it('injects bearer token via setAuthHeader and refreshes before expiry', async () => {
    ...
    const strategy = await new BearerTokenHeaderAuthInjectionStrategyFactory().create({
  ...
  await jest.advanceTimersByTimeAsync(90_000);
    ...
    expect(connector.setAuthHeader).toHaveBeenLastCalledWith('Bearer refreshed');
  ...
  ).rejects.toThrow('BearerTokenHeaderAuthInjectionStrategy requires a tokenProvider configuration');
  });
  ...
  await jest.advanceTimersByTimeAsync(60_000);
    expect(errorSpy).toHaveBeenCalledWith('auth_token_refresh_failed', expect.any(Error));
  ...
  it('schedules minimum refresh when token is already expired', async () => {
    ...
    await jest.advanceTimersByTimeAsync(60_000);
    expect(connector.setAuthHeader).toHaveBeenLastCalledWith('Bearer after-expired');
    await strategy.cleanup();
  });
  ...
  it('handles null tokens from provider gracefully', async () => {
    ...
    await strategy.apply(connector);
    await jest.advanceTimersByTimeAsync(60_000);
    expect(connector.setAuthHeader).toHaveBeenLastCalledWith('Bearer after-null');
    await strategy.cleanup();
  });
  ...
  it('modifies URLs with dynamic query parameters', async () => {
    ...
    const absolute = await strategy.modifyUrl('https://example.com/path?existing=1');
  ...
  it('preserves URL hash fragment when injecting query token', async () => {
    ...
    const modified = await strategy.modifyUrl('https://example.com/path#section');
  ...
  ).rejects.toThrow(/QueryParamAuthInjectionStrategyFactory expects type "QueryParamAuth"/);
  });
  ...
  ).rejects.toThrow('WebSocketSubprotocolAuthInjectionStrategy requires a tokenProvider configuration');
  });
...
class StaticTokenProvider implements TokenProvider {
  ...
  public async getToken(): Promise<Token> {
    const entry = this.responses[this.index] ?? this.responses[this.responses.length - 1];
...
class StaticTokenProviderFactory extends TokenProviderFactory<StaticTokenProviderConfig> {
  ...
  public async create(
    ...
    const candidate = config as StaticTokenProviderConfig & Record<string, unknown>;
    if (candidate.type !== 'StaticTokenProvider') {
      throw new Error('StaticTokenProvider config must declare type StaticTokenProvider');
    }
    ...
    return new StaticTokenProvider([...(sequenceValue as StaticTokenSequenceEntry[])]);
  }
}

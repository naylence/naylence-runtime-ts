...
describe("StaticTokenProvider", () => {
  it("returns configured token and optional expiration", async () => {
    const provider = new StaticTokenProvider({ token: "static-token", expiresAt: 123 });
  ...
  it("creates provider via factory", async () => {
    ...
    const provider = await factory.create({ type: "StaticTokenProvider", token: "factory-token" });
  ...
  it("normalizes Date expiresAt values", async () => {
    ...
    const provider = new StaticTokenProvider({ token: "date-token", expiresAt: date });
  ...
  it("parses ISO string expiresAt values", async () => {
    ...
    const provider = new StaticTokenProvider({ token: "iso-token", expiresAt: iso });
  ...
  it("omits expiresAt when null or undefined", async () => {
    const providerWithNull = new StaticTokenProvider({ token: "null-token", expiresAt: null });
    const providerWithUndefined = new StaticTokenProvider({ token: "undef-token" });
  ...
  it("returns a defensive copy of the token", async () => {
    const provider = new StaticTokenProvider({ token: "copy-token", expiresAt: 1000 });
  ...
  it("throws when token is missing or not a string", () => {
    expect(
      () => new StaticTokenProvider(undefined as unknown as StaticTokenProviderOptions)
    ).toThrow("StaticTokenProvider requires a string token value");

    expect(() => new StaticTokenProvider({ token: 123 as unknown as string })).toThrow(
      "StaticTokenProvider requires a string token value"
    );
  });

  it("throws when expiresAt number is not finite", () => {
    expect(
      () => new StaticTokenProvider({ token: "finite", expiresAt: Number.POSITIVE_INFINITY })
  ...
  it("throws when expiresAt date is invalid", () => {
    ...
    expect(
      () => new StaticTokenProvider({ token: "invalid-date", expiresAt: invalidDate })
    ).toThrow("expiresAt Date must be valid");
  });

  it("throws when expiresAt string cannot be parsed", () => {
    expect(
      () => new StaticTokenProvider({ token: "invalid-string", expiresAt: "not-a-date" })
    ).toThrow("expiresAt string must be ISO-8601 or epoch milliseconds");
  });

  it("throws when expiresAt is of unsupported type", () => {
    expect(
      () => new StaticTokenProvider({ token: "bad-type", expiresAt: true as unknown as number })
...
describe("NoneTokenProvider", () => {
  it("returns empty token far in the future", async () => {
    ...
    expect((token.expiresAt ?? 0) - Date.now()).toBeGreaterThan(10 * 365 * 24 * 60 * 60 * 1000);
  });
...
describe("OAuth2ClientCredentialsTokenProvider", () => {
  ...
  it("caches tokens until expiration and refreshes after", async () => {
    ...
    const fetchImpl = jest.fn(
      async () =>
        responses[callCount++] ?? buildResponse({ access_token: "fallback-token", expires_in: 60 })
    );
  ...
  it("creates provider via factory and performs fetch", async () => {
    ...
    (global as typeof globalThis & { fetch: typeof fetch }).fetch = stubFetch as typeof fetch;
...
describe("SharedSecretTokenVerifier", () => {
  it("returns authorization context for valid token", async () => {
    ...
    const context = await verifier.verify("secret", { expectedAudience: "audience" });
  ...
  it("throws for invalid token", async () => {
    ...
    await expect(verifier.verify("wrong")).rejects.toThrow("Invalid shared secret token");
  });
...
describe("TokenVerifierFactory integration", () => {
  ...
  it("throws when provider returns mismatched token", async () => {
    ...
    await expect(verifier.verify("mismatch")).rejects.toThrow("Invalid shared secret token");
  });

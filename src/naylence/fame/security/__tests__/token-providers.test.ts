...
describe('StaticTokenProvider', () => {
  ...
  it('throws when expiresAt date is invalid', () => {
    ...
    expect(() => new StaticTokenProvider({ token: 'invalid-date', expiresAt: invalidDate })).toThrow(
...
describe('OAuth2ClientCredentialsTokenProvider', () => {
  ...
  it('caches tokens until expiration and refreshes after', async () => {
    ...
    const fetchImpl = jest.fn(async () => responses[callCount++] ?? buildResponse({ access_token: 'fallback-token', expires_in: 60 }));

...
type ManagerInternals = {
  _keyManagementHandler: {
    acceptKeyAnnounce(envelope: FameEnvelope, context: FameDeliveryContext): Promise<void>;
  } | null;
...
describe("DefaultSecurityManager._getKeyAnnounceHandler", () => {
  ...
  it("returns a no-op handler when key management handler is absent", async () => {
    ...
    await expect(delegate({} as FameEnvelope, {} as FameDeliveryContext)).resolves.toBeUndefined();
  });
});

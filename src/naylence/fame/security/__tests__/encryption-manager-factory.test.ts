...
import type { SecureChannelManager, SecureChannelState } from '../encryption/secure-channel-manager.js';
...
describe('Encryption manager primitives', () => {
  ...
  const envelope = { frame: { type: 'DataFrame' } } as unknown as FameEnvelope;
    ...
    expect(EncryptionResult.queued()).toEqual({ status: EncryptionStatus.QUEUED, envelope: undefined });
  });
});
...
describe('EncryptionManagerFactory with registered factory', () => {
  ...
  it('creates an encryption manager with config and dependency bundle', async () => {
    ...
    expect(TestEncryptionManagerFactory.lastConfig).toEqual({ type: 'TestEncryptionManager', label: 'configured' });
...
describe('SecureChannelManagerFactory with registered factory', () => {
  ...
  it('creates a manager using explicit config', async () => {
    ...
    expect(TestSecureChannelManagerFactory.lastConfig).toEqual({ type: 'TestSecureChannelManager' });

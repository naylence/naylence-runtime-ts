...
import { CryptoLevel, SecurityAction, SecurityRequirements } from '../policy/security-policy.js';
...
function createPermissivePolicy(overrides: Partial<SecurityPolicy> = {}): SecurityPolicy {
  const policy = { ...noopPolicy } as SecurityPolicy;
  policy.classifyMessageCryptoLevel = jest.fn(() => CryptoLevel.CHANNEL) as unknown as SecurityPolicy['classifyMessageCryptoLevel'];
  policy.isInboundCryptoLevelAllowed = jest.fn(() => true) as unknown as SecurityPolicy['isInboundCryptoLevelAllowed'];
  policy.isSignatureRequired = jest.fn(() => false);
  policy.shouldVerifySignature = jest.fn(async () => false);
  policy.getUnsignedViolationAction = jest.fn(() => SecurityAction.ALLOW) as unknown as SecurityPolicy['getUnsignedViolationAction'];
  policy.getInboundViolationAction = jest.fn(() => SecurityAction.ALLOW) as unknown as SecurityPolicy['getInboundViolationAction'];
  policy.getInvalidSignatureViolationAction = jest.fn(() => SecurityAction.ALLOW) as unknown as SecurityPolicy['getInvalidSignatureViolationAction'];
...
describe('DefaultSecurityManager helper coverage', () => {
  ...
  it('processes signed local delivery by creating local context and security state', async () => {
    ...
    await expect(manager.onDeliverLocal(node, address, envelope)).resolves.toBe(envelope);

    expect(mockLogger.warning).toHaveBeenCalledWith('deliver_local_missing_payload_digest', {
      envp_id: 'env-data',
    });
  ...
  it('rejects unsigned critical frames from non-local origins', async () => {
    ...
    await expect(manager.onDeliver(createNode(), envelope, context)).resolves.toBeNull();
    expect(mockLogger.error).toHaveBeenCalledWith('critical_frame_unsigned_rejected', expect.any(Object));
  });
  ...
  it('invokes optional lifecycle hooks via node listener helper', async () => {
    ...
    const manager = new DefaultSecurityManager(createPermissivePolicy(), null, null, encryption, keyManager, null, certificateManager);
    const node = createNode();
  ...
  it('normalizes missing delivery context fields before outbound security handling', async () => {
    ...
    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(envelope, expect.objectContaining({
...
describe('DefaultSecurityManager onDeliverLocal enforcement', () => {
  ...
  function setupPolicy(): SecurityPolicy {
    const policy = { ...noopPolicy } as SecurityPolicy;
    policy.classifyMessageCryptoLevel = jest.fn(() => CryptoLevel.PLAINTEXT) as unknown as SecurityPolicy['classifyMessageCryptoLevel'];
    policy.isInboundCryptoLevelAllowed = jest.fn(() => false) as unknown as SecurityPolicy['isInboundCryptoLevelAllowed'];
    policy.getInboundViolationAction = jest.fn(() => SecurityAction.REJECT) as unknown as SecurityPolicy['getInboundViolationAction'];
    policy.isSignatureRequired = jest.fn(() => false);
    policy.shouldVerifySignature = jest.fn(async () => false);
    policy.getUnsignedViolationAction = jest.fn(() => SecurityAction.ALLOW);
    policy.getInvalidSignatureViolationAction = jest.fn(() => SecurityAction.ALLOW);
    return policy;
  }
  ...
  it('rejects inbound messages when crypto level is not allowed', async () => {
    ...
    await expect(manager.onDeliverLocal(createNode(), address, envelope)).resolves.toBeNull();
  ...
  it('sends NACK when crypto violation policy requests nack', async () => {
    const policy = setupPolicy();
    policy.getInboundViolationAction = jest.fn(() => SecurityAction.NACK) as unknown as SecurityPolicy['getInboundViolationAction'];
    const manager = new DefaultSecurityManager(policy);
    const sendNack = jest.spyOn(manager as any, 'sendNack').mockResolvedValue(undefined);
    const envelope = createEnvelope({ id: 'env-nack' });

    await expect(manager.onDeliverLocal(createNode(), address, envelope)).resolves.toBeNull();
    expect(sendNack).toHaveBeenCalledWith(expect.any(Object), envelope, 'crypto_level_violation');
  ...
  it('sends NACK when signature is required but missing', async () => {
    ...
    policy.isInboundCryptoLevelAllowed = jest.fn(() => true) as unknown as SecurityPolicy['isInboundCryptoLevelAllowed'];
    policy.isSignatureRequired = jest.fn(() => true);
    policy.getUnsignedViolationAction = jest.fn(() => SecurityAction.NACK) as unknown as SecurityPolicy['getUnsignedViolationAction'];
    const manager = new DefaultSecurityManager(policy);
    const sendNack = jest.spyOn(manager as any, 'sendNack').mockResolvedValue(undefined);
    const envelope = createEnvelope({ id: 'env-sig' });

    await expect(manager.onDeliverLocal(createNode(), address, envelope)).resolves.toBeNull();
    expect(sendNack).toHaveBeenCalledWith(expect.any(Object), envelope, 'signature_required');
    expect(mockLogger.error).toHaveBeenCalledWith('inbound_message_nacked_unsigned', { envp_id: 'env-sig' });
  });
});
...
describe('DefaultSecurityManager onDeliver dispatch', () => {
  it('rejects when authorizer denies remote envelope', async () => {
    ...
    const manager = new DefaultSecurityManager(policy, null, null, null, null, authorizer);
    const envelope = {
      id: 'env-auth-denied',
      frame: { type: 'Data' },
    } as unknown as FameEnvelope;

    await expect(manager.onDeliver(createNode(), envelope, {
      originType: DeliveryOriginType.PEER,
    } as FameDeliveryContext)).resolves.toBeNull();

    expect(authorizer.authorize).toHaveBeenCalled();
    expect(mockLogger.warning).toHaveBeenCalledWith('envelope_authorization_failed', expect.objectContaining({
  ...
  it('logs and returns null when authorizer throws', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith('envelope_authorization_error', expect.objectContaining({
  ...
  it('uses key frame handler for key requests and stops when handled locally', async () => {
    ...
    const context = { originType: DeliveryOriginType.PEER } as FameDeliveryContext;
  ...
  it('falls back to child key request handler when sentry cannot handle locally', async () => {
    ...
    const handleChild = jest.spyOn(manager as any, 'handleChildKeyRequest').mockResolvedValue(undefined);
    const envelope = {
      id: 'env-keyreq-fallback',
      frame: { type: 'KeyRequest' },
      sec: { sig: 'sig' },
    } as unknown as FameEnvelope;
    const context = { originType: DeliveryOriginType.PEER, fromSystemId: 'sid-1' } as FameDeliveryContext;
...
describe('DefaultSecurityManager.onChildAttach', () => {
  ...
  it('removes stale keys on rebind and adds new child keys', async () => {
    ...
    const manager = new DefaultSecurityManager(noopPolicy, null, null, null, keyManager);
    jest.spyOn(manager as unknown as { _getKeysToProvide(): any }, '_getKeysToProvide').mockReturnValue(null);
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('removed_stale_keys_on_rebind', expect.objectContaining({
      system_id: 'child-1',
      old_path: '/old',
      removed_count: 2,
    }));
    expect(mockLogger.debug).toHaveBeenCalledWith('added_child_attach_keys', expect.objectContaining({
  ...
  it('logs warnings when rebind cleanup fails and policy validation rejects keys', async () => {
    ...
    jest.spyOn(manager as unknown as { _getKeysToProvide(): any }, '_getKeysToProvide').mockReturnValue(ourKeys);

    const sentinelPolicy = {
      validateAttachSecurityCompatibility: jest
        .fn<[boolean, string?], any[]>((input: { peerKeys?: Array<Record<string, unknown>> }) => {
    ...
    expect(mockLogger.warning).toHaveBeenCalledWith('failed_to_remove_stale_keys_on_rebind', expect.objectContaining({
      system_id: 'child-2',
      old_path: '/old',
      error: 'nope',
    }));
    expect(mockLogger.warning).toHaveBeenCalledWith('attach_child_security_validation_failed', expect.objectContaining({
      reason: 'bad-peer',
      child_system_id: 'child-2',
    }));
    expect(mockLogger.warning).toHaveBeenCalledWith('attach_our_security_validation_warning', expect.objectContaining({
      reason: 'bad-ours',
      child_system_id: 'child-2',
    }));
    expect(mockLogger.warning).toHaveBeenCalledWith('attach_missing_signing_key', expect.objectContaining({
      child_system_id: 'child-2',
    }));
    expect(mockLogger.warning).toHaveBeenCalledWith('attach_missing_encryption_key', expect.objectContaining({
      child_system_id: 'child-2',
    }));
  });
  ...
  it('logs when no keys provided but policy requires exchange and handles transport errors', async () => {
    ...
    const sentinelPolicy = {
      validateAttachSecurityCompatibility: jest.fn(() => [true, undefined] as [boolean, string | undefined]),
      requirements: jest.fn(() => ({
        requireSigningKeyExchange: true,
        requireEncryptionKeyExchange: true,
      })),
    ...
    expect(mockLogger.warning).toHaveBeenCalledWith('attach_no_keys_provided', expect.objectContaining({
    ...
    expect(mockLogger.error).toHaveBeenCalledWith('failed_to_add_attach_keys_will_retry_on_epoch_change', expect.objectContaining({
      parent_id: 'child-3',
    }));
  });
});
...
describe('DefaultSecurityManager forwarding flows', () => {
  ...
  it('handles outbound security on upstream forwarding when keys pending', async () => {
    ...
    await expect(manager.onForwardUpstream(node, envelope, context)).resolves.toBeNull();

    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(envelope, expect.objectContaining({
      originType: DeliveryOriginType.LOCAL,
    }));
    expect(mockLogger.debug).toHaveBeenCalledWith('on_forward_upstream_queued_for_keys', { envp_id: 'env-upstream' });
  });
  ...
  it('warns when critical frame cannot be secured while forwarding to route', async () => {
    ...
    await expect(manager.onForwardToRoute(node, 'seg-1', envelope, context)).resolves.toBeNull();

    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(
      envelope,
      expect.objectContaining({ fromSystemId: node.id, originType: DeliveryOriginType.LOCAL })
    );
    expect(mockLogger.warning).toHaveBeenCalledWith('critical_frame_forwarding_failed_missing_keys', expect.objectContaining({
  ...
  it('queues local route forwarding when keys are pending', async () => {
    ...
    await expect(manager.onForwardToRoute(node, 'seg-2', envelope, context)).resolves.toBeNull();

    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(envelope, expect.objectContaining({ originType: DeliveryOriginType.LOCAL }));
    expect(mockLogger.debug).toHaveBeenCalledWith('on_forward_to_route_queued_for_keys', expect.objectContaining({
  ...
  it('errors when forwarding critical frame to peer without security handler', async () => {
    ...
    await expect(manager.onForwardToPeer(node, 'peer-1', envelope, context)).resolves.toBeNull();

    expect(mockLogger.error).toHaveBeenCalledWith('critical_frame_forwarding_failed_no_security_handler', expect.objectContaining({
  ...
  it('queues peer forwarding when local outbound security needs keys', async () => {
    ...
    await expect(manager.onForwardToPeer(node, 'peer-2', envelope, context)).resolves.toBeNull();

    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(envelope, context);
    expect(mockLogger.debug).toHaveBeenCalledWith('on_forward_to_peer_queued_for_keys', expect.objectContaining({
  ...
  await expect(manager.onForwardToPeers(node, envelope, ['peer-a'], undefined, context)).resolves.toBe(envelope);

    expect(handler.handleOutboundSecurity).toHaveBeenCalledWith(envelope, context);
    expect(mockLogger.debug).toHaveBeenCalledWith('on_forward_to_peers_security_processing_complete', expect.objectContaining({
      envp_id: 'env-peers',
    }));
  });
});
...
describe('DefaultSecurityManager welcome and heartbeat handling', () => {
  ...
  it('rethrows certificate validation failures from onWelcome', async () => {
    ...
    const manager = new DefaultSecurityManager(noopPolicy, null, null, null, null, null, certificateManager);
    const welcomeFrame = { system_id: 'child', assigned_path: '/path' } as unknown as NodeWelcomeFrame;

    await expect(manager.onWelcome(welcomeFrame)).rejects.toThrow('certificate validation failed: mismatch');
    expect(mockLogger.error).toHaveBeenCalledWith('child_node_certificate_validation_failed_stopping_node', expect.objectContaining({
      node_id: 'child',
      assigned_path: '/path',
    }));
  });
  ...
  it('logs warning when certificate provisioning fails but continues', async () => {
    ...
    expect(mockLogger.warning).toHaveBeenCalledWith('certificate_provisioning_error_proceeding_without_cert', expect.objectContaining({
      error: 'network issue',
      node_id: 'child',
    }));
  });
  ...
  it('verifies heartbeat signatures when verifier is present', async () => {
    ...
    const manager = new DefaultSecurityManager(noopPolicy, null, envelopeVerifier);
    const envelope = {
      id: 'heartbeat-1',
      sec: { sig: 'sig' },
    } as unknown as FameEnvelope;
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('heartbeat_ack_envelope_verified');
  });
  ...
  it('suppresses warning when requirements lookup throws', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('could_not_determine_verification_policy_allowing_heartbeat', {
  ...
  it('logs failure details when heartbeat signature verification throws', async () => {
    ...
    expect(mockLogger.warning).toHaveBeenCalledWith('heartbeat_envelope_verification_failed', expect.objectContaining({
...
describe('DefaultSecurityManager epoch change handling', () => {
  ...
  it('announces keys upstream when key manager supports it', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('handle_epoch_change_security', { epoch: 'epoch-42' });
  ...
  it('logs when key announcement is skipped due to missing key manager', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('handle_epoch_change_security', { epoch: 'epoch-43' });
    expect(mockLogger.debug).toHaveBeenCalledWith('skipping_key_announcement_no_key_manager');
  });
});
...
describe('DefaultSecurityManager node shutdown handling', () => {
  ...
  it('stops and clears all managed security components', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('stopping_security_components', { node_id: node.id });
...
describe('DefaultSecurityManager welcome short-circuit', () => {
  ...
  it('returns immediately when certificate manager is not configured', async () => {
    ...
    await expect(manager.onWelcome({} as NodeWelcomeFrame)).resolves.toBeUndefined();

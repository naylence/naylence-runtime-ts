...
describe("JWKSJWTTokenVerifier", () => {
  ...
  it("verifies tokens, builds authorization context, and caches JWKS set", async () => {
    ...
    expect(context.grantedScopes).toEqual(expect.arrayContaining(["read", "write"]));
    expect(context.claims.kid).toBe("kid-123");

    expect(createRemoteJWKSetMock).toHaveBeenCalledTimes(1);
    expect(createRemoteJWKSetMock).toHaveBeenCalledWith(new URL("https://example.com/keys"), {
      cacheMaxAge: 1_000,
  ...
  it("wraps unknown non-error rejections", async () => {
    ...
    await expect(verifier.verify("non-error-token")).rejects.toThrow("Token verification failed");
  });
});

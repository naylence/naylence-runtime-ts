...
import { SecretSource, normalizeSecretSource } from '../credential/secret-source.js';
...
describe('SecretSource.normalize', () => {
  it('throws when provided value is null or undefined', () => {
    ...
    expect(() => SecretSource.normalize(undefined as unknown as string)).toThrow(
      'Secret source cannot be null or undefined'
    );
  });
  ...
  it('parses env:// URI strings and validates variable names', () => {
    ...
    expect(config).toEqual({ type: 'EnvCredentialProvider', varName: 'API_TOKEN' });
  ...
  it('delegates through normalizeSecretSource helper', () => {
    ...
    expect(config).toEqual({ type: 'EnvCredentialProvider', varName: 'DELEGATED' });
  });
...
describe('Normalization helpers', () => {
  it('normalizes static credential config variations', () => {
    ...
    expect(
      normalizeStaticConfig({ type: 'StaticCredentialProvider', credentialValue: 'token' })
    ).toEqual({ type: 'StaticCredentialProvider', credentialValue: 'token' });

    expect(
      normalizeStaticConfig({ type: 'StaticCredentialProvider', credential_value: 'legacy' })
    ).toEqual({ type: 'StaticCredentialProvider', credentialValue: 'legacy' });

    expect(() => normalizeStaticConfig({ type: 'StaticCredentialProvider' })).toThrow(
  ...
  it('normalizes environment credential config variations', () => {
    ...
    expect(
      normalizeEnvConfig({ type: 'EnvCredentialProvider', varName: 'TOKEN_VAR' })
    ).toEqual({ type: 'EnvCredentialProvider', varName: 'TOKEN_VAR' });

    expect(
      normalizeEnvConfig({ type: 'EnvCredentialProvider', var_name: 'legacy_var' })
    ).toEqual({ type: 'EnvCredentialProvider', varName: 'legacy_var' });

    expect(() => normalizeEnvConfig({ type: 'EnvCredentialProvider', varName: '' })).toThrow(
      'EnvCredentialProvider requires a non-empty "varName"'
    );
  });
  ...
  it('normalizes secret store credential config variations', () => {
    ...
    expect(
      normalizeSecretStoreConfig({ type: 'SecretStoreCredentialProvider', secretName: 'vault' })
    ).toEqual({ type: 'SecretStoreCredentialProvider', secretName: 'vault' });

    expect(
      normalizeSecretStoreConfig({ type: 'SecretStoreCredentialProvider', secret_name: 'legacy' })
    ).toEqual({ type: 'SecretStoreCredentialProvider', secretName: 'legacy' });

    expect(() =>
      normalizeSecretStoreConfig({ type: 'SecretStoreCredentialProvider', secretName: '' })
    ).toThrow('SecretStoreCredentialProvider requires a non-empty "secretName"');
  });
  ...
  it('normalizes prompt credential config variations', () => {
    ...
    expect(
      normalizePromptConfig({ type: 'PromptCredentialProvider', credentialName: 'apiToken' })
    ).toEqual({ type: 'PromptCredentialProvider', credentialName: 'apiToken' });

    expect(
      normalizePromptConfig({ type: 'PromptCredentialProvider', credential_name: 'legacy' })
    ).toEqual({ type: 'PromptCredentialProvider', credentialName: 'legacy' });

    expect(() =>
      normalizePromptConfig({ type: 'PromptCredentialProvider', credentialName: '' })
...
describe('EnvCredentialProvider', () => {
  ...
  it('throws when instantiated with an empty variable name', () => {
    expect(() => new EnvCredentialProvider('')).toThrow('Environment variable name must not be empty');
  });
});
...
describe('PromptCredentialProvider', () => {
  ...
  it('normalizes non-string resolver results to null', async () => {
    const provider = new PromptCredentialProvider('apiCredential', async () => 0 as unknown as string);
    await expect(provider.get()).resolves.toBeNull();
  });
  ...
  it('falls back to global prompt when available', async () => {
    ...
    expect(promptMock).toHaveBeenCalledWith("Enter credential 'apiCredential': ");
  });
  ...
  it('returns null when no IO transport exists', async () => {
    ...
    globalObject.process = { stdin: null, stdout: null } as unknown as typeof process;
...
describe('CredentialProviderFactory', () => {
  it('creates provider instances based on configuration', async () => {
    const staticProvider = await CredentialProviderFactory.createCredentialProvider({
      type: 'StaticCredentialProvider',
    ...
    const envProvider = await CredentialProviderFactory.createCredentialProvider({
      type: 'EnvCredentialProvider',
      varName: 'API_TOKEN',
    });
  ...
  it('creates default provider when configuration is absent', async () => {
    const provider = await CredentialProviderFactory.createCredentialProvider(null);

...
describe('JWTTokenVerifier', () => {
  ...
  const context = await verifier.verify(token);
    ...
    expect(context.grantedScopes).toEqual(expect.arrayContaining(['one', 'two']));
  ...
  it('enforces required scopes', async () => {
    ...
    await expect(verifier.verify(token)).rejects.toThrow('Token missing required scope');
  });
  ...
  it('validates audience when provided', async () => {
    ...
    await expect(verifier.verify(token, { expectedAudience: 'wrong-audience' })).rejects.toThrow(
      'Invalid audience'
    );

    const context = await verifier.verify(token, { expectedAudience: 'expected-audience' });

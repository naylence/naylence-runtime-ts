...
describe("DefaultSecurityManager.onDeliverLocal", () => {
  ...
  it("nacks inbound envelope when crypto level violation requires nack", async () => {
    ...
    const manager = new DefaultSecurityManager(policy);
    const node = createNode();
    const envelope = createEnvelope({ replyTo: FameAddress.create("reply@/client") });
    ...
    expect(result).toBeNull();
    expect(sendNack).toHaveBeenCalledWith(node, envelope, "crypto_level_violation");
  });
  ...
  it("rejects when signature is required but missing", async () => {
    ...
    const manager = new DefaultSecurityManager(policy);
    const result = await manager.onDeliverLocal(createNode(), address, createEnvelope());
    expect(result).toBeNull();
  ...
  it("nacks when signature verification fails", async () => {
    ...
    expect(result).toBeNull();
    expect(sendNack).toHaveBeenCalledWith(node, envelope, "signature_verification_failed");
  });
  ...
  it("throws when plaintext payload digest mismatches", async () => {
    ...
    await expect(manager.onDeliverLocal(createNode(), address, envelope)).rejects.toThrow(
      "Payload digest mismatch on final delivery"
    );
  });
  ...
  it("decrypts envelope when handler requests decryption", async () => {
    ...
    expect((manager as any)._envelopeSecurityHandler.shouldDecryptEnvelope).toHaveBeenCalled();
  });
  ...
  it("routes secure accept frames to secure channel handler", async () => {
    ...
    const envelope = createEnvelope({ frame: { type: "SecureAccept" } as FameEnvelope["frame"] });
    const result = await manager.onDeliverLocal(createNode(), address, envelope);
...
describe("DefaultSecurityManager.onDeliver", () => {
  it("rejects unsigned critical frame from non-local origin", async () => {
    const policy = createPolicy();
    const manager = new DefaultSecurityManager(policy);
    const envelope = createEnvelope({ frame: { type: "SecureOpen" } as FameEnvelope["frame"] });
    const result = await manager.onDeliver(createNode(), envelope, createContext());
    expect(result).toBeNull();
  });
  ...
  it("nacks unsigned envelope when policy requires signature", async () => {
    ...
    const manager = new DefaultSecurityManager(policy);
    const result = await manager.onDeliver(createNode(), createEnvelope(), createContext());
    expect(result).toBeNull();
  });
  ...
  it("authorizes envelope and stores authorization results in context", async () => {
    ...
    const envelope = createEnvelope({ sec: { sig: { val: "sig-data" } } as FameEnvelope["sec"] });

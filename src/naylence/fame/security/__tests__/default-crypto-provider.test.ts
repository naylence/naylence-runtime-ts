...
describe('DefaultCryptoProvider', () => {
  ...
  it('manages node context, certificates, and JWKS exposure', async () => {
    ...
    expect(provider.getCertificateContext()?.logicals).toEqual([
      'logical.from.node',
    ]);
  });
  ...
  it('rejects unsupported algorithms when key material is missing', async () => {
    await expect(
      DefaultCryptoProvider.create({ algorithm: 'HS256' })
    ).rejects.toThrow('Unsupported signing algorithm: HS256');
  });
  ...
  it('honors RSA algorithm configured via environment variables', async () => {
    const originalEnv =
      typeof process !== 'undefined'
        ? process.env.FAME_CRYPTO_ALGORITHM
        : undefined;

    if (typeof process === 'undefined' || !process.env) {
      throw new Error(
        'process.env is not available for the RSA algorithm test'
      );
    }
  ...
  it('generates HMAC secrets via browser crypto when available', async () => {
    ...
    const getRandomValues = jest
      ...
      .mockImplementation(
        (typedArray: Parameters<typeof webcrypto.getRandomValues>[0]) => {
    ...
    try {
      const provider = await DefaultCryptoProvider.create();
      const hmacCall = getRandomValues.mock.calls.find(
        ([buffer]) => buffer.length === 32
      );
      expect(hmacCall).toBeDefined();
      expect(provider.hmacSecret).toBe(
        Buffer.from(deterministicBytes).toString('base64')
      );
    } finally {

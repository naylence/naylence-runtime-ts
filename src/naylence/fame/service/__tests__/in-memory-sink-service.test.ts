...
import { InMemorySinkService, InMemorySinkServiceFactory, type SinkBindingManager } from '../in-memory-sink-service.js';
...
jest.mock('naylence-core', () => {
  const extractEnvelopeAndContext = jest.fn((message: any) => {
    ...
    if (typeof message === 'object' && message !== null) {
      const envelope = 'envelope' in message ? (message as any).envelope ?? null : (message as any) ?? null;
      const context = 'context' in message ? (message as any).context : undefined;
...
jest.mock('../../channel/in-memory/in-memory-fanout-broker.js', () => {
  ...
  class InMemoryFanoutBroker {
    ...
    constructor(public readonly channel: unknown, public readonly config: unknown) {
      instances.push(this);
    }
  }
...
describe('InMemorySinkService', () => {
  ...
  it('creates sinks using binding manager and starts broker', async () => {
    ...
    const result = await service.createSink({ name: ' sink-one ' } as CreateSinkParams);
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('created_sink', expect.any(Object));
  });
  ...
  it('throws when binding manager does not return binding', async () => {
    ...
    await expect(service.createSink({ name: 'x' } as CreateSinkParams)).rejects.toThrow(
      'Binding manager did not return a binding'
    );
  });

  it('requires sink and subscriber addresses when subscribing', async () => {
    const service = new InMemorySinkService({ bindingManager: createBindingManager(createBinding('sink')) });

    await expect(service.subscribe({ subscriberAddress: 'dest' } as SubscribeParams)).rejects.toThrow(
      'sinkAddress and subscriberAddress are required'
    );

    await expect(service.subscribe({ sinkAddress: 'sink' } as SubscribeParams)).rejects.toThrow(
      'sinkAddress and subscriberAddress are required'
    );
  });
  ...
  it('subscribes consumers, delivers messages, and handles missing envelopes', async () => {
    ...
    await service.subscribe({ sinkAddress: 'sink-subs', subscriberAddress: 'subscriber' });

    const instances = fanoutModule.__getInstances();
    const channel = instances[0].addSubscriber.mock.calls[0][0] as { send: (message: any) => Promise<void> };
    ...
    coreModule.extractEnvelopeAndContext.mockImplementationOnce(() => [null, undefined]);
  ...
  it('propagates delivery errors from channel and logs failure', async () => {
    ...
    await service.subscribe({ sinkAddress: 'sink-error', subscriberAddress: 'subscriber' });
  ...
  it('default deliver sends via FameFabric and validates destination', async () => {
    ...
    const deliverFn = (service as any).deliver as (env: any, ctx?: any) => Promise<void>;
    ...
    (coreModule.FameFabric.current as jest.Mock).mockReturnValueOnce({ send: sendSpy });

    await deliverFn({ id: 'env-dest', to: { toString: () => 'dest-string' } }, { scope: 'ctx' });
  ...
  it('records subscriptions and removes them on unsubscribe', async () => {
    ...
    await service.subscribe({ sinkAddress: 'sink-unsub', subscriberAddress: 'subscriber' });

    const subscriptions: Array<any> = (service as any).subscriptions.get('sink-unsub');
  ...
  it('ignores unsubscribe for unknown subscriptions', async () => {
    ...
    const service = new InMemorySinkService({ bindingManager });
    const coreModule = require('naylence-core');
    const orphan = new coreModule.Subscription({ close: jest.fn() }, { toString: () => 'addr' });
  ...
  it('routes RPC requests to handlers and rejects unknown methods', async () => {
    ...
    const createSpy = jest.spyOn(service, 'createSink').mockResolvedValue(binding.address as any);
    const subscribeSpy = jest.spyOn(service, 'subscribe').mockResolvedValue();

    await service.handleRpcRequest('createSink', { name: 'sink' });
    await service.handleRpcRequest('subscribe', { sinkAddress: 'sink', subscriberAddress: 'sub' });
    ...
    await expect(service.handleRpcRequest('unknown', {})).rejects.toThrow('Unknown RPC method: unknown');
  });
  ...
  it('handles unsubscribe when updated list retains other subscriptions', async () => {
    ...
    await service.subscribe({ sinkAddress: 'sink-multi', subscriberAddress: 'one' });
    await service.subscribe({ sinkAddress: 'sink-multi', subscriberAddress: 'two' });

    const subscriptions: Array<any> = (service as any).subscriptions.get('sink-multi');
...
describe('InMemorySinkServiceFactory', () => {
  it('requires binding manager', () => {
    ...
    expect(() => factory.create({} as any)).toThrow('bindingManager is required to create InMemorySinkService');
  });

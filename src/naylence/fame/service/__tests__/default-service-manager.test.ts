...
describe('DefaultServiceManager', () => {
  ...
  it('registers message services and wraps handler responses', async () => {
    ...
    let servedHandler:
      | ((env: FameEnvelope, ctx?: unknown) => Promise<unknown>)
      | undefined;
  ...
  it('resolves capabilities from local services first', async () => {
    ...
    >;
    const spy = jest
      .spyOn(FameServiceProxy, 'remoteByAddress')
      .mockReturnValue(remoteInstance);
  ...
  it('resolves addresses by capability for local and mapped services', async () => {
    ...
    expect(await manager.resolveAddressByCapability(['delta', 'epsilon'])).toBe(
      serveAddress
    );
    expect(await manager.resolveAddressByCapability(['mapped-cap'])).toBe(
      mappedAddress
    );
  ...
  it('does not restart services when already running', async () => {
    const serve = jest.fn(async () =>
      FameAddress.create('service@/already-started')
    );
    const manager = new DefaultServiceManager({
      invoke: jest.fn(),
      serve,

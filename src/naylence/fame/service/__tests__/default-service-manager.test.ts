...
describe('DefaultServiceManager', () => {
  ...
  it('resolves capabilities from local services first', async () => {
    ...
    const remoteInstance = { remote: true } as unknown as ReturnType<typeof FameServiceProxy.remoteByAddress>;
  ...
  expect(() => manager.resolveByCapability('missing')).toThrow('Capability missing not available');
  });
  ...
  it('rejects non-string capability lookups', () => {
    ...
    expect(() => manager.resolveByCapability(Symbol('cap'))).toThrow('Capability Symbol(cap) not available');
  });
});

...
function normalizeIndexedDBConfig(
  ...
  const normalizedMode = (
    typeof parsed.mode === 'string' ? parsed.mode : 'dx'
  ).toLowerCase();
  ...
  if (parsed.masterKey !== null && parsed.masterKey !== undefined) {
    masterKeyConfig = SecretSource.normalize(
      parsed.masterKey as SecretSourceType
    );
  }
...
export class IndexedDBStorageProviderFactory extends StorageProviderFactory<IndexedDBStorageProviderConfig> {
  ...
  public async create(
    ...
    const { IndexedDBStorageProvider } =
      await getIndexedDbStorageProviderModule();
...
type IndexedDbStorageProviderModule =
  typeof import('./indexeddb-storage-provider.js');

let indexedDbStorageProviderModulePromise: Promise<IndexedDbStorageProviderModule> | null =
  null;

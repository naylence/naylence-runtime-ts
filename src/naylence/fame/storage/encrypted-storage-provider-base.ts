  ...
  private async getRawMasterKey(): Promise<Uint8Array> {
    ...
    if (matchesPattern && key.length >= 16) {
      try {
        ...
        console.warn(
          `[EncryptedStorage] Master key decoded from base64url but has unexpected length ${decoded.length}. Treating as UTF-8 passphrase.`
        );
      } catch (error) {
        // If decoding fails, treat as UTF-8 passphrase
        console.warn(
          `[EncryptedStorage] Master key looks like base64url but failed to decode: ${error instanceof Error ? error.message : String(error)}. Treating as UTF-8 passphrase.`
        );
      }
    }

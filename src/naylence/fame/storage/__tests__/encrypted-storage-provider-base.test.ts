...
describe('StorageAESEncryptionManager', () => {
  ...
  it('throws when ciphertext is shorter than IV length', async () => {
    ...
    await expect(manager.decrypt(shortCiphertext, key)).rejects.toThrow('Ciphertext too short to contain IV');
  });
});
...
describe('EncryptedKeyValueStore', () => {
  ...
  it('lists values while skipping corrupt and non-encrypted entries', async () => {
    ...
    encryptionManager.decrypt.mockClear();
    encryptionManager.decrypt.mockImplementation(async (ciphertext: Uint8Array, _key: Uint8Array) => {
    ...
    await store.set(
      'valid',
      new EncryptedValue({ keyId: 'k', ciphertext: toHex(JSON.stringify({ value: 'ok' })), algorithm: 'algo' })
    );
  ...
  it('supports modelCtor fromJSON, fromJson, deserialize, constructor, and prototype merge', async () => {
    ...
    for (const [Ctor, label] of scenarios) {
      ...
      await store.set(
        label,
        new EncryptedValue({ keyId: 'k', ciphertext: toHex('{"value":"' + label + '"}'), algorithm: 'algo' })
      );
...
describe('EncryptedStorageProviderBase', () => {
  ...
  it('throws when encryption enabled without master key provider', () => {

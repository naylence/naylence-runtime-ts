...
import { TransportProvisionerFactory, TRANSPORT_PROVISIONER_FACTORY_BASE_TYPE } from '../transport-provisioner.js';
...
function makeHelloFrame(overrides: Partial<NodeHelloFrame> = {}): NodeHelloFrame {
  return {
    type: 'NodeHello',
...
function makePlacementDecision(overrides: Partial<PlacementDecision> = {}): PlacementDecision {
  return {
    accept: true,
    assignedPath: '/child',
    ...overrides,
...
describe('WebSocketTransportProvisioner', () => {
  ...
  it('provisions a websocket connection grant when supported', async () => {
    const provisioner = new WebSocketTransportProvisioner({ url: 'ws://localhost:8080/ws' });

    const hello = makeHelloFrame({ supportedTransports: ['websocket', 'http'] });
  ...
  it('throws when websocket transport is not supported', async () => {
    const provisioner = new WebSocketTransportProvisioner({ url: 'ws://example.com/ws' });
    const hello = makeHelloFrame({ supportedTransports: ['http'] });
    const decision = makePlacementDecision();

    await expect(
      provisioner.provision(decision, hello, {})
    ).rejects.toThrow('Unsupported transports: http');
  });

  it('includes auth configuration and ttl metadata when attach token provided', async () => {
    const provisioner = new WebSocketTransportProvisioner({ url: 'ws://example.com/ws', ttlSec: 90 });
    const hello = makeHelloFrame({ supportedTransports: ['websocket'] });
    const decision = makePlacementDecision();

    const result = await provisioner.provision(decision, hello, {}, 'secret-token');
    ...
    const tokenProvider = (grant.auth as Record<string, unknown>).tokenProvider as Record<string, unknown>;
    expect(tokenProvider).toMatchObject({ type: 'StaticTokenProvider', token: 'secret-token' });
...
describe('TransportProvisionerFactory', () => {
  ...
  it('creates websocket provisioner from configuration', async () => {
    const provisioner = await TransportProvisionerFactory.createTransportProvisioner({
